...
- [x] caching for crawled domains, or queries, with TTL and when to refresh. Maybe a unified list of domains and rules on when to refresh. Optimize for speed and precision.

- [x] in the tool calling, allow to edit bash commands before they are executed.
...
- [x] improve CI/Release speed: remove redundant cargo check (clippy is superset), deduplicate audit jobs, cache Vulkan SDK on Linux, use mold linker in CI, use `--workspace` for clippy, add concurrency groups, add `--locked` and `--timings` to all release builds.
