...
- [x] configure sccache + mold linker for faster builds — added `rustc-wrapper = "sccache"` to `[build]` in `.cargo/config.toml` to cache rustc and C/C++ compilation outputs across clean builds; added `[target.aarch64-unknown-linux-gnu]` and `[target.x86_64-unknown-linux-gnu]` sections with `linker = "clang"` and `-fuse-ld=mold` for faster linking; added `protocol-asset` feature to tauri dependency to match `tauri.conf.json` assetProtocol allowlist; clean rebuild time reduced from ~6m to ~2m45s (54% faster)
