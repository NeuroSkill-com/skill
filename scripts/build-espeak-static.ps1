...
#
# Version control:
#   Override the tag with:  $env:ESPEAK_TAG_OVERRIDE="1.51.1"
#
# Requirements:
#   cmake  git  lib.exe (MSVC) -- all available when Visual Studio Build Tools
#   or Visual Studio is installed and a Developer Command Prompt is active.
#
# Usage (from the repository root in a Developer PowerShell for VS):
...
foreach ($tool in @("cmake", "git")) {
    ...
    if (-not $found) {
        Die "'$tool' not found in PATH. Install it (cmake: winget install Kitware.CMake  git: winget install Git.Git) and restart your terminal."
    }
    # Use ${tool} so PowerShell does not treat the colon as a drive reference.
    Write-Host "  ${tool}: $($found.Source)"
}

# lib.exe (MSVC static-library tool) -- try PATH first, then vswhere.
# Avoid ?. (null-conditional) -- it requires PowerShell 7+.
...
# Version
if ($env:ESPEAK_TAG_OVERRIDE) {
    $EspeakTag = $env:ESPEAK_TAG_OVERRIDE
} else {
    $EspeakTag = "1.52.0"
}
...
try {
    ...
    # cmake builds libucd and libSpeechPlayer as separate archives; merge them
    # into one self-contained espeak-ng.lib using MSVC lib.exe.
