...
- **macOS DMG creator**: replaced the custom 800-line `scripts/create-macos-dmg.sh` with a thin wrapper around [`sindresorhus/create-dmg`](https://github.com/sindresorhus/create-dmg) (v8).  The tool handles composed volume icon (app icon overlaid on disk), Retina @2x background, ULFO+APFS format, SLA license agreement (from `license.txt` via `hdiutil udifrez`), and code signing automatically.  The wrapper script signs the `.app`, runs `create-dmg`, renames the output to our convention (`NeuroSkill_<version>_<arch>.dmg`), and optionally notarizes with Apple.  Both `release-mac.yml` and `pr-build.yml` CI workflows install `create-dmg` globally and use the shared script.  Also fixed `assemble-macos-app.sh` Info.plist generation: replaced regex string injection with `plistlib` so `NSHighResolutionCapable` is a proper boolean `<true/>` (not `<string>true</string>`) and added `CFBundleName`, `CFBundleDisplayName`, `LSMinimumSystemVersion`, `NSRequiresAquaSystemAppearance` keys.
