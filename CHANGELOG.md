...
- **macOS DMG creator**: replaced the custom 800-line `scripts/create-macos-dmg.sh` with a 3-phase hybrid approach: (1) [`sindresorhus/create-dmg`](https://github.com/sindresorhus/create-dmg) v8 produces the base DMG with composed volume icon (app icon overlaid on disk), Retina @2x background, ULFO+APFS format, and SLA; (2) post-processing converts to read-write, injects README.md/CHANGELOG.md/LICENSE, overlays version text onto background via Pillow, updates `.DS_Store` icon positions for the extra files, cleans junk/dotfiles; (3) re-compresses to ULFO, re-embeds SLA via `hdiutil udifrez` (using the same Node.js approach as `create-dmg`'s `sla.js`), signs, and optionally notarizes.  Both `release-mac.yml` and `pr-build.yml` CI workflows install `create-dmg` globally and use the shared script.  Also fixed `assemble-macos-app.sh` Info.plist generation: replaced regex string injection with `plistlib` so `NSHighResolutionCapable` is a proper boolean `<true/>` (not `<string>true</string>`) and added `CFBundleName`, `CFBundleDisplayName`, `LSMinimumSystemVersion`, `NSRequiresAquaSystemAppearance` keys.
