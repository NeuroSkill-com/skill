...
- **Add macOS 32 MB main-thread stack size**: added `-Wl,-stack_size,0x2000000` (32 MB) linker flags for both `aarch64-apple-darwin` and `x86_64-apple-darwin` in `.cargo/config.toml`.  The flag was previously listed as done in TODO but was missing from the actual config, leaving macOS at the default 8 MB stack.
