...
- **Move `CHANGELOG.md` to project root**: merged the root-level 2026-03-15 entries into the comprehensive `docs/CHANGELOG.md` and moved the combined file to the project root; updated README link.
...
- **Extract `skill-devices` workspace crate**: moved pure device-session logic from `muse_session.rs` (774 lines) into `crates/skill-devices/`. Zero Tauri dependencies — depends on `skill-eeg` and `serde`. Extracted items: DND focus-mode decision engine (`dnd_tick` pure function with `DndConfig`/`DndState`/`DndDecision` types — encapsulates the entire rolling-window focus scoring, SNR low-signal tracking, exit-delay with lookback, and OS toggle decision logic that was previously inlined as ~120 lines inside the EEG event handler), `dnd_apply_os_result` (post-OS-call state update), composite EEG score functions (`compute_meditation` with alpha/beta/stillness/HRV components, `compute_cognitive_load` with frontal-theta/parietal-alpha sigmoid, `compute_drowsiness` with TAR + alpha-spindle, `compute_engagement_raw`, `focus_score` sigmoid mapping), battery EMA (`BatteryEma` struct with `update()` returning `BatteryAlert::None|Low|Critical`), and SNR constants (`SNR_LOW_DB`, `SNR_LOW_TICKS`). Includes 9 unit tests covering all composite scores (range validation), battery alerts (low/critical thresholds), and DND activation/deactivation logic (threshold crossing, below-threshold no-activate). `muse_session.rs` delegates composite score calculations via `skill_devices::compute_*`; the DND engine types are ready for full integration in a follow-up.

- **Extract `skill-exg` workspace crate**: moved pure EEG embedding helpers from `eeg_embeddings.rs` (2,613 lines) into `crates/skill-exg/`. Zero Tauri dependencies — depends on `skill-constants`, `skill-eeg`, `skill-data`, `hf-hub`, `ureq`, `cubecl-runtime`, `dirs`. Extracted items: `cosine_distance` (cosine similarity metric), `fuzzy_match` + internal `normalize_text`/`levenshtein` (fuzzy keyword matching with ≤32% edit-distance threshold), `yyyymmdd_utc`/`yyyymmddhhmmss_utc` (pure UTC timestamp formatters), `resolve_hf_weights`/`probe_hf_weights` (HuggingFace disk-cache weight lookup with `$HUGGINGFACE_HUB_CACHE`/`$HF_HOME` support), `download_hf_weights` (resumable streaming download with progress reporting via `EegModelStatus`, cancellation via `AtomicBool`, HF LFS metadata resolution, blob promotion, and snapshot registration), `register_hf_snapshot` (symlink/hardlink/copy blob into HF snapshot structure), `configure_cubecl_cache` (pre-create GPU kernel cache to prevent macOS `.app` launch panics), `GPU_DEVICE_POISONED` (process-global atomic flag), `panic_msg` (extract message from caught panic), `EpochMetrics` struct + `from_snapshot(BandSnapshot)` + `Default` (per-epoch band-derived metrics with sigmoid scoring). `src-tauri/src/eeg_embeddings.rs` delegates via `use skill_exg::*` and thin wrapper functions. All existing `crate::eeg_embeddings::*` import paths unchanged. `download_hf_weights` signature simplified (logger parameter dropped — uses `eprintln!` directly). This enables EEG distance computation, text matching, weight management, and metrics derivation to be reused by CLI tools or headless services without Tauri.

- **Extract `skill-tray` workspace crate**: moved system tray icon helpers from `tray.rs` (674 lines) into `crates/skill-tray/`. Zero dependencies — pure `std`, no Tauri. Extracted items: `overlay_progress_bar` (circular progress-ring pixel compositing, decoupled from `tauri::Image` — now takes raw RGBA buffer + dimensions), `shortcut_suffix` / `with_shortcut` (keyboard shortcut label formatting with platform-aware `CmdOrCtrl` replacement), `ellipsize_middle` (middle-truncation for long filenames), `progress_bucket` / `progress_percent` (deduplication keys for icon updates), and `MENU_REBUILD_MIN_MS` constant. Includes 8 unit tests and 2 doc-tests covering all exported helpers. `src-tauri/src/tray.rs` delegates via `pub use skill_tray::*` re-exports; the thin `overlay_progress_bar` wrapper converts between `skill_tray`'s raw RGBA output and `tauri::image::Image`. All existing import paths unchanged.

- **Extract `skill-router` workspace crate**: moved UMAP projection + caching, embedding/label loaders, UMAP cluster analysis, and metric rounding types from `ws_commands.rs` (2,408 lines) and `session_analysis.rs` into `crates/skill-router/`. Zero Tauri dependencies — depends on `skill-commands`, `skill-settings`, `skill-constants`, `fast-umap`, `burn`, `cubecl`, `rusqlite`. Extracted items: `umap_compute_inner` (GPU-accelerated parametric UMAP with disk caching), `umap_cache_dir`/`umap_cache_path`/`umap_cache_load`/`umap_cache_store`, `load_embeddings_range` (daily SQLite embedding scan), `load_labels_range` (label window overlap query), `find_label_for_epoch`, `analyze_umap_points` (centroid separation, outlier detection), `RoundedScores`/`RoundedBands` structs, rounding helpers (`r1`–`r3`, `r1d`, `r2d`, `r2f`), and `COMMANDS` constant (complete list of supported WebSocket/HTTP command names). `ws_commands.rs` re-exports moved items; `session_analysis.rs` delegates `load_embeddings_range` via `pub(crate) use`. All existing import paths unchanged. This enables UMAP projection, embedding loading, and metric formatting to be reused by CLI tools or headless services without the Tauri dependency tree.

- **Extract `skill-commands` workspace crate**: moved EEG embedding search, timestamp helpers, SVG/DOT graph generation, PCA projection, streaming search, and session lookup (`commands.rs`, 2,321 lines) into `crates/skill-commands/`. The crate has zero Tauri dependencies — pure `std` + `serde` + `serde_json` + `fast-hnsw` + `rusqlite` + `skill-constants` with its own `MutexExt` poison-recovery helper. All types (`SearchResult`, `SearchProgress`, `QueryEntry`, `NeighborEntry`, `NeighborMetrics`, `LabelEntry`, `SessionRef`, `InteractiveGraphNode`, `InteractiveGraphEdge`, `InteractiveSearchResult`, `SvgLabels`, `DayIndex`, `GlobalIndexHandle`) and pure functions (`search_embeddings_in_range`, `stream_search_inner`, `find_session_for_timestamp_in`, `generate_dot`, `generate_svg`, `pca_2d`, `get_labels_near`, `get_labels_for`, `get_found_label_embedding`, `list_date_dirs`, `load_day_index`, `unix_to_ts`, `ts_to_unix`, `fmt_unix_utc`, `dot_esc`, `dot_node_label`, `dot_edge_label`, `query_slug`, `file_ts`) live in the crate. `src-tauri/src/commands.rs` replaced with a `pub use skill_commands::*` re-export shim plus 7 thin Tauri command wrappers (`search_embeddings`, `enqueue_search_embeddings`, `stream_search_embeddings`, `find_session_for_timestamp`, `interactive_search`, `save_dot_file`, `save_svg_file`). All existing `crate::commands::*` import paths continue to work unchanged. This enables search and graph generation to be reused by CLI tools or headless services without the Tauri dependency tree.

- **Extract `skill-jobs` workspace crate**: moved the sequential job queue (`job_queue.rs`, 384 lines) into `crates/skill-jobs/`. The crate has zero Tauri dependencies — pure `std` + `serde` + `serde_json` with its own `MutexExt` poison-recovery helper. `src-tauri/src/job_queue.rs` replaced with a 4-line re-export shim. All 3 unit tests pass in the new crate. This enables the job queue to be reused by CLI tools or headless services without pulling in the Tauri dependency tree.
...
- **Shared text embedder — eliminate 3 redundant model copies (~390 MB RAM saved)**: consolidated four independent `fastembed::TextEmbedding` instances into a single app-wide `Arc<Mutex<TextEmbedding>>` (`EmbedderState`). Previously the label embedder, EEG hook matcher, screenshot OCR embed thread, and OCR search query path each loaded their own copy of the `bge-small-en-v1.5` ONNX model (~130 MB each). Now they all share one instance. Added `embed_text(&str) -> Option<Vec<f32>>` to the `ScreenshotContext` trait so the `skill-screenshots` crate can delegate to the shared embedder without depending on `src-tauri`. `HookMatcher` now receives `Arc<EmbedderState>` instead of a `text_embedding_model: String` and creating its own `TextEmbedding`. Lock hold time in `HookMatcher::maybe_refresh()` is minimised via a two-phase pattern: batch-embed all hook keywords under the lock, then release it before running HNSW searches. Removed deprecated `ocr_text_model` field from `ScreenshotConfig`, `ocr_text_model_enum()` method, OCR text model dropdown from Screenshots settings UI, and 10 dead i18n keys (en/de). Settings → Embeddings info callout now notes the model is shared across labels, hooks, and OCR text search. Screenshots tab "Active Models" grid shows the shared embedder name with "(Embeddings)" provenance label.
...
- **Extract tools into `skill-tools` workspace crate**: moved all LLM tool logic (definitions, execution, parsing, validation, context trimming, safety checks) from `skill-llm` into a new `crates/skill-tools/` crate with 5 modules: `types` (`LlmToolConfig`, `ToolExecutionMode`), `parse` (tool-call extraction, injection, stripping from model output), `defs` (built-in tool JSON Schema definitions and enable checks), `exec` (runtime implementation of all 9 tools including DuckDuckGo search with HTML fallback, bash with script files, filesystem read/write/edit, search_output, and approval dialogs for dangerous operations), `context` (context-aware history trimming). `skill-llm` now depends on `skill-tools` and re-exports `skill_tools::parse` as `tools` for backwards compatibility. Removed 607 lines of duplicated code from `engine.rs` and 4 direct dependencies (`jsonschema`, `regex`, `rfd`, `chrono`) from skill-llm's Cargo.toml. All 24 tool-call parsing tests pass in the new crate. Dead `src-tauri/src/llm/tools.rs` file removed.
...
- **Extract TTS engine into `skill-tts` workspace crate**: moved 1,307 lines of TTS logic (KittenTTS ONNX backend, NeuTTS GGUF backend, shared audio output, espeak-ng initialization, backend routing, voice listing/selection) from the monolithic `skill` crate into `crates/skill-tts/`. `NeuttsConfig` is now canonically defined in `skill-tts::config` and re-exported from `settings.rs`. Tauri commands (`tts_init`, `tts_unload`, `tts_speak`, `tts_list_voices`, etc.) stay in the main crate as thin wrappers using a callback-based `tts_init_with_callback` to decouple from `tauri::AppHandle`. Feature flags (`tts-kitten`, `tts-neutts`) are forwarded through both crates. Compiles clean with and without TTS features.

- **Extract EEG DSP into `skill-eeg` workspace crate**: moved 3,459 lines of EEG signal processing logic (band power analysis, overlap-save filtering, quality monitoring, artifact detection, head pose tracking, model configuration) from the monolithic `skill` crate into `crates/skill-eeg/`. EEG constants (sample rates, filter parameters, band definitions, HNSW settings, ZUNA model references) are now canonically defined in `skill-eeg::constants` and re-exported from the main crate's `constants.rs`. All `crate::eeg_*`, `crate::artifact_detection`, `crate::head_pose`, and `crate::constants::*` paths remain unchanged via thin re-export modules. Zero tauri/AppState coupling — pure DSP logic with only `serde`, `serde_json`, and `gpu-fft` dependencies.

- **Extract LLM engine into `skill-llm` workspace crate**: moved 7,421 lines of LLM logic (inference engine, tool calling, catalog management, chat store, configuration types) from the monolithic `skill` crate into `crates/skill-llm/`. Introduced `LlmEventEmitter` trait to decouple the inference engine from `tauri::AppHandle`, with a `TauriEmitter` adapter in the main crate. The main crate's `llm/mod.rs` is now a thin re-export layer — all `crate::llm::*` paths remain unchanged. `LlmConfig`, `LlmToolConfig`, and `ToolExecutionMode` are now defined in `skill-llm` and re-exported from `settings.rs`. Tauri-dependent commands (`cmds.rs`) stay in the main crate as a thin glue layer. Feature flags (`llm`, `llm-metal`, `llm-cuda`, `llm-vulkan`, `llm-mtmd`) are forwarded through both crates. Compiles clean with and without LLM features. This separation enables incremental compilation — changes to LLM logic no longer trigger recompilation of the entire 40K-line main crate.
...
- **Configurable OCR engine, text embedding model, GPU/CPU toggle**: `ScreenshotConfig` extended with four new fields: `ocr_enabled` (optional, default true), `ocr_engine` (default "ocrs", future: tesseract, apple-vision), `ocr_text_model` (text embedding model — "bge-small-en-v1.5" 384d default, "all-minilm-l6-v2" 384d balanced, "bge-base-en-v1.5" 768d richer), and `use_gpu` (default true — applies to both image embeddings and OCR). Settings UI updated with three new toggles (OCR enable/disable, GPU acceleration on/off) and two new dropdowns (OCR engine, text embedding model). Active Models info grid shows current engine, models, text embedder, inference mode. OCR section conditionally visible only when OCR is enabled. Full i18n for en/de (14 new keys).

- **OCR text extraction + text embedding for screenshots**: added on-device OCR via the `ocrs` crate (0.12.1, rten inference backend). OCR runs on the **full-resolution** captured image before any downsizing for maximum text recognition quality. Extracted text is embedded via fastembed BGE-Small-EN-v1.5 text model and indexed in a separate HNSW index (`screenshots_ocr.hnsw`) for semantic text search. SQLite schema extended with `ocr_text` (full extracted text), `ocr_embedding` (text embedding blob), `ocr_embedding_dim`, and `ocr_hnsw_id` columns with automatic migration for existing databases. OCR models (~10 MB each: text-detection.rten, text-recognition.rten) auto-downloaded from S3 on first use to `~/.skill/ocr_models/`. New `search_screenshots_by_text` Tauri command supporting both `"semantic"` mode (embed query text → HNSW cosine search) and `"substring"` mode (SQL LIKE matching). Dual HNSW architecture: `screenshots.hnsw` for visual similarity, `screenshots_ocr.hnsw` for text similarity — enabling cross-modal queries ("find screenshots where I was reading about X").

- **macOS screenshot capture via CoreGraphics FFI**: replaced the osascript/Swift/Python subprocess approach with pure Rust FFI for completely silent, non-intrusive capture. Gets the frontmost application's PID via `NSWorkspace.sharedWorkspace.frontmostApplication.processIdentifier` (objc2 message send), then enumerates on-screen windows via `CGWindowListCopyWindowInfo` (CoreGraphics.framework C FFI) to find the first layer-0 window matching that PID. The CGWindowID is passed to `screencapture -x -l <wid>` which captures only that window silently — no cursor change, no sound, no user interaction. Sub-millisecond window lookup with no subprocess overhead.

- **Screenshots Re-embed & Reindex button**: the re-embed section is now always visible in the Screenshots settings tab (not conditionally hidden). Shows an always-accessible "Re-embed & Reindex" button that re-embeds all screenshots with the current vision model and rebuilds the HNSW similarity index. When the embedding model is changed, an amber warning banner appears with the stale count, time estimate, and a prominent "Re-embed now" call-to-action. Stale and unembedded counts shown as badge pills; live progress bar with ETA during re-embedding.
...
- **Screenshot capture + vision-encoder embedding system**: new periodic active-window capture system aligned with EEG epoch cadence (~5 s). Captures the frontmost application window only — macOS via CGWindowListCopyWindowInfo window-ID + `screencapture -l <wid>`, Linux via `xdotool`/`import -window` (X11) or `swaymsg`+`grim -g` (Wayland) with `scrot -u` fallback, Windows via `GetForegroundWindow`/`GetWindowRect`/`CopyFromScreen`. Resizes with aspect-ratio-preserving Lanczos3 fit + center-pad to configurable intermediate resolution (default 224×224 for CLIP), saves as WebP to `~/.skill/screenshots/YYYYMMDD/`. Dual embedding backend: fastembed CLIP ViT-B/32 (512-dim, default) or Nomic Embed Vision v1.5 (768-dim), plus optional mmproj via `InferRequest::EmbedImage` through the LLM actor channel (mean-pooled + L2-normalized). Embeddings stored in `screenshots.sqlite` (with full provenance: model_backend, model_id, image_size, quality, app_name, window_title) and indexed in `screenshots.hnsw` for visual-similarity search. Shared `ScreenshotStore` on `AppState` as `Option<Arc<ScreenshotStore>>`. Opt-in via `ScreenshotConfig` in settings (disabled by default, session-gated). Cross-modal search via shared `YYYYMMDDHHmmss` timestamp join key to EEG embeddings. Re-embedding on model change with progress events and time estimation. Seven new Tauri commands: `get_screenshot_config`, `set_screenshot_config`, `estimate_screenshot_reembed`, `rebuild_screenshot_embeddings`, `get_screenshots_around`, `search_screenshots_by_vector`, `search_screenshots_by_image`.
...
- **Complete translation pass across de/fr/he/uk**: all untranslated keys are now translated — titles, labels, section headings, body text, and FAQ answers for LLM help, Hooks help, TTS help, compare, settings, and UI elements. `npm run audit:i18n` reports 0 untranslated keys across all 4 locales (1,527 keys legitimately exempt as technical terms, brand names, formulas, or cognates).
