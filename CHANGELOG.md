...
- **macOS DMG creator**: added `scripts/create-macos-dmg.sh` and `npm run tauri:build:mac:dmg` for local DMG creation that bypasses the Tauri CLI entirely.  The DMG includes a @2x Retina background image (app icon + version on dark canvas, generated via Pillow or macOS CoreGraphics), `README.md`, `CHANGELOG.md`, `LICENSE`, and a Finder-configured icon layout (app left, Applications right, docs bottom row).  Signs the `.app` (ad-hoc by default, or with `APPLE_SIGNING_IDENTITY` for distribution), creates a compressed UDZO DMG via `hdiutil`, and optionally notarizes with Apple when `APPLE_ID` / `APPLE_PASSWORD` / `APPLE_TEAM_ID` are set.  Both `release-mac.yml` and `pr-build.yml` CI workflows now use this shared script instead of inline DMG/signing logic.
- **DMG background/icon fix**: the DMG background image and Finder view settings were not visible for two reasons: (1) modern macOS defaults to APFS for `hdiutil create`, but `.DS_Store` background-image settings only work on **HFS+** volumes — fixed by forcing `-fs HFS+`; (2) AppleScript requires Finder automation permission (`-1743` error) — fixed by generating the `.DS_Store` binary directly with Python using `ds_store` + `mac_alias` packages (auto-installed if missing), which writes icon positions, window bounds, icon-view settings, and background image alias without needing any Finder permission.  AppleScript kept as fallback.  Also fixed `assemble-macos-app.sh` Info.plist generation: replaced regex string injection with `plistlib` so `NSHighResolutionCapable` is a proper boolean `<true/>` (not `<string>true</string>`) and added `CFBundleName`, `CFBundleDisplayName`, `LSMinimumSystemVersion`, `NSRequiresAquaSystemAppearance` keys.
- **DMG polish** (from [SO best practices](https://stackoverflow.com/questions/96882) and [`create-dmg`](https://github.com/create-dmg/create-dmg) patterns): `chmod -Rf go-w` to prevent world-writable files, clean macOS junk (`.fseventsd`, `.Trashes`, `.Spotlight-V100`, `._` via `dot_clean`), hide dotfiles with `SetFile -a V`, auto-open on mount (`bless`, arm64-aware), `hdiutil internet-enable` with availability check (removed in macOS 10.15), Retina @2x background, SLA license agreement via `hdiutil udifrez -xml` (same as `create-dmg`), `hdiutil` retry wrapper with exponential backoff for "Resource busy" errors, max zlib compression.
- **DMG volume icon with version badge**: the mounted DMG volume and `.dmg` file now show a custom icon — the app icon with a version badge (e.g. "v0.0.28") overlaid as a rounded pill in the bottom-right corner.  Generated via Pillow at all required `.iconset` sizes (16–1024px), converted to `.icns` with `iconutil`, and set as `.VolumeIcon.icns` with the `SetFile -a C` custom icon flag on the volume root.  Falls back to the plain app icon if Pillow or `iconutil` is unavailable.
