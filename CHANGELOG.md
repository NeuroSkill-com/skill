...
- **DMG polish** (from [SO best practices](https://stackoverflow.com/questions/96882)): clean macOS junk files (`.fseventsd`, `.Trashes`, `.Spotlight-V100`, `._` resource forks via `dot_clean`), hide dotfiles with `SetFile -a V`, auto-open volume window on mount (`bless --openfolder`), auto-eject after drag-install in Safari (`hdiutil internet-enable`), Retina @2x background image (1320×800 @2x + 660×400 @1x), SLA license agreement dialog before mounting (via `Rez` flatten/unflatten, best-effort), max zlib compression (`zlib-level=9`).
