...
pub fn generate_dot(nodes: &[InteractiveGraphNode], edges: &[InteractiveGraphEdge]) -> String {
    ...
    let ts_map: std::collections::HashMap<String, u64> = nodes
        .iter()
    ...
    if !query_row.is_empty() {
        o.push_str(&format!("  {{ rank=source; {query_row} }}\n"));
    }
    if !text_row.is_empty() {
        o.push_str(&format!("  {{ rank=same;   {text_row} }}\n"));
    }
    if !eeg_row.is_empty() {
        o.push_str(&format!("  {{ rank=same;   {eeg_row} }}\n"));
    }
    if !found_row.is_empty() {
        o.push_str(&format!("  {{ rank=same;   {found_row} }}\n"));
    }
    if !ss_row.is_empty() {
        o.push_str(&format!("  {{ rank=sink;   {ss_row} }}\n"));
    }
    o.push('\n');
