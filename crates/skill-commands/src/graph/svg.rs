...
fn default_layer_screenshots() -> String {
    "SCREENSHOTS".into()
}
fn default_legend_screenshot() -> String {
    "Screenshot".into()
}
...
pub fn generate_svg(
    ...
    let max_tl_stack: usize = {
        ...
        for (date, hour, _) in &tl_info {
            *counts
                .entry((tl_day_idx[date.as_str()], tl_hour_idx[hour]))
                .or_insert(0) += 1;
        }
    ...
    let ss_band_top = if has_fl {
        fl_band_bot + BAND_GAP
    } else if has_eeg {
        eeg_band_bot + BAND_GAP
    } else if has_tl {
        tl_band_bot + BAND_GAP
    ...
    if has_fl {
        if fl_has_proj {
            let pairs: Vec<(f32, f32)> = layers[3]
                .iter()
            ...
            let mut raw_pos: Vec<(f64, f64)> = pairs
                .iter()
                .map(|&(px, py)| {
    ...
    // ── Colour helpers ────────────────────────────────────────────────────
    let eeg_ts: Vec<u64> = nodes
        .iter()
        .filter(|n| n.kind == "eeg_point")
...
"##));
    ...
    if has_tl {
        ...
        o.push_str(&format!(
            ...
            tl_band_top + 13.0,
            svg_esc(&labels.layer_text_matches)
        ));
    ...
    if has_fl {
        ...
        o.push_str(&format!(
            "  <rect x=\"0\" y=\"{fl_band_top:.1}\" width=\"{w}\" height=\"{bh:.1}\" \
             ...
             font-weight=\"600\" letter-spacing=\"1\">{}</text>\n",
            fl_band_top + 13.0,
            svg_esc(&labels.layer_found_labels)
        ));
    ...
    if has_ss {
        ...
        o.push_str(&format!(
            "  <rect x=\"0\" y=\"{ss_band_top:.1}\" width=\"{w}\" height=\"{bh:.1}\" \
             ...
             font-weight=\"600\" letter-spacing=\"1\">{}</text>\n",
            ss_band_top + 13.0,
            svg_esc(&labels.layer_screenshots)
        ));
    ...
    // ── Edges ─────────────────────────────────────────────────────────────
    for e in edges {
        let (Some(&(x1, y1)), Some(&(x2, y2))) = (pos.get(&e.from_id), pos.get(&e.to_id)) else {
            continue;
        };
        let dx = x2 - x1;
        let dy = y2 - y1;
        ...
        let dst_h = nodes
            ...
            .map(|n| half_h(&n.kind))
            .unwrap_or(NH / 2.0);
        let sx1 = x1 + dx / len * (src_h + 2.0);
        let sy1 = y1 + dy / len * (src_h + 2.0);
        let sx2 = x2 - dx / len * (dst_h + 9.0);
        let sy2 = y2 - dy / len * (dst_h + 9.0);
    ...
    // ── Nodes ─────────────────────────────────────────────────────────────
    for nd in nodes {
        let Some(&(cx, cy)) = pos.get(&nd.id) else {
            continue;
        };
