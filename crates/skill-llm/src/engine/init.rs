...
pub fn init(
    ...
    let mmproj_path = catalog
        ...
        .filter(|p| {
            let Some(model_repo) = active_model_repo else {
                return true;
            };
        ...
        if config.ctx_size.is_none() {
            ...
            if user_ctx > entry.max_context_length {
                push_log(
                    ...
                    "warn",
                    &format!(
                        "user ctx_size {} exceeds model max {} — capping",
    ...
    push_log(
        ...
        &log_buf,
        "info",
        &format!("starting LLM server — model: {model_name}"),
    );
    ...
    let bundled_skills_dir = exe_dir
        .as_ref()
    ...
    for diag in &skills_result.diagnostics {
        push_log(
            ...
            &log_buf,
            &diag.level,
            &format!("[skills] {}: {}", diag.path, diag.message),
        );
    }
    if n_skills > 0 {
        ...
        push_log(
            ...
            &log_buf,
            "info",
            &format!("discovered {n_skills} skill(s): {}", names.join(", ")),
        );
    }
