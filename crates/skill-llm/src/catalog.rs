...
fn bundled() -> LlmCatalog {
    // The catalog JSON is embedded at compile time — a parse failure here is a
    // build-time bug, not a runtime error.  `expect` is the correct choice.
        ...
        let monitor = std::thread::spawn(move || {
            loop {
                ...
                let (shard_state, shard_pct, shard_msg, shard_cancelled) = {
                    let sp = shard_prog_clone.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
...

                {
                    ...
                    if op.cancelled && !shard_cancelled {
                        ...
                        drop(op);
                        let mut sp = shard_prog_clone.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
                        sp.cancelled       = true;
