...
mod tests {
    ...
    /// active_calibration_id (the other half of the deleted block). Written
    /// as raw JSON text so this cannot silently stop covering the explicit-`[]`
    /// case if the struct ever gains `skip_serializing_if`.
    #[tokio::test]
