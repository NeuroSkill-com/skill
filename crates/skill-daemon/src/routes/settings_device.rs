...
use crate::{
    routes::settings_io::{load_user_settings, patch_settings, patch_settings_ok},
    state::AppState,
};
...
crate::settings_struct!(FilterConfig, get_filter_config, set_filter_config => filter_config);
crate::settings_get_value!(get_storage_format => storage_format);
crate::settings_get_value!(get_embedding_overlap => embedding_overlap_secs);
...
pub(crate) async fn set_notch_preset(
    ...
    let value = req.value;
    let _ = patch_settings_ok(&state, move |s| {
        s.filter_config.notch = value;
    })
    .await;
...
pub(crate) async fn set_storage_format(
    ...
    patch_settings(&state, move |s| {
        s.storage_format = fmt.to_string();
    })
    .await;
...
pub(crate) async fn set_embedding_overlap(
    ...
    patch_settings(&state, move |s| {
        s.embedding_overlap_secs = clamped;
    })
    .await;
...
crate::settings_u64!(
    ...
    set_update_check_interval => update_check_interval_secs
);

crate::settings_struct_get!(skill_settings::OpenBciConfig, get_openbci_config => openbci);

pub(crate) async fn set_openbci_config(
    State(state): State<AppState>,
    Json(config): Json<skill_settings::OpenBciConfig>,
) -> Json<serde_json::Value> {
    let wifi_shield_ip = config.wifi_shield_ip.clone();
    let galea_ip = config.galea_ip.clone();
    let _ = patch_settings_ok(&state, move |s| {
        s.openbci = config;
    })
    .await;
    if let Ok(mut wifi) = state.scanner_wifi_config.lock() {
        wifi.wifi_shield_ip = wifi_shield_ip;
        wifi.galea_ip = galea_ip;
    }
    Json(serde_json::json!({"ok": true}))
}
...
pub(crate) async fn set_device_api_config(
    ...
    let emotiv_client_id = config.emotiv_client_id.clone();
    let emotiv_client_secret = config.emotiv_client_secret.clone();
    let disk_api = config.clone();
    patch_settings(&state, move |s| {
        s.device_api = disk_api;
    })
    .await;
    ...
    if let Ok(mut cortex) = state.scanner_cortex_config.lock() {
        cortex.emotiv_client_id = emotiv_client_id;
        cortex.emotiv_client_secret = emotiv_client_secret;
    }
    Json(serde_json::json!({"ok": true}))
}
