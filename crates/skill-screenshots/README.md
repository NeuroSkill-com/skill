...
| `gif_encode` | Animated GIF encoding from burst-captured frames, representative frame extraction for vision embedding |
...
| `RlxImageEmbedder::from_repo` | Load the RLX vision encoder (nomic-embed-vision-v1.5) |
| `RlxImageEmbedder::embed_bytes` | Embed a PNG image into a vector |
...
The GIF is saved alongside the WebP still. The middle frame of the burst is used as the representative image for vision embedding and OCR, providing better coverage of animated/scrolling content.
...
- `rlx` / `rlx-models` — vision embedding (nomic-embed-vision-v1.5; Metal / CUDA / wgpu / CPU)
- `rlx-ocr` — OCR text detection and recognition
