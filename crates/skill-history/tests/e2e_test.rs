...
fn e2e_full_pipeline() {
    ...
    eprintln!("3. list_sessions_for_local_day({}) -> {} sessions", newest.key, sessions.len());
    ...
    for d in &local_days {
        ...
        if !s.is_empty() {
            ...
            assert!(first.session_start_utc.is_some() || first.session_end_utc.is_some(), 
                "no timestamps for {} {}", d.key, first.csv_file);
        }
    }
    ...
    if let Some(first) = s.first() {
        ...
        eprintln!("5. SessionEntry JSON (first 200 chars): {}", &json[..json.len().min(200)]);
