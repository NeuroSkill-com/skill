...
pub fn set_skills_refresh_interval(
...
) {
    {
        ...
        __a.lock_or_recover()
            .config
            .tools
            .skills_refresh_interval_secs = secs;
    }
...
pub fn get_skills_sync_on_launch(state: tauri::State<'_, Mutex<Box<AppState>>>) -> bool {
    {
        let __a = state.lock_or_recover().llm.clone();
        let __r = __a.lock_or_recover().config.tools.skills_sync_on_launch;
        __r
    }
}
...
pub fn set_skills_sync_on_launch(
    ...
    {
        let __a = state.lock_or_recover().llm.clone();
        __a.lock_or_recover().config.tools.skills_sync_on_launch = enabled;
    }
    crate::save_settings(&app);
}
...
pub async fn sync_skills_now(
    ...
    let skill_dir = state.lock_or_recover().skill_dir.clone();
    let outcome =
        tokio::task::spawn_blocking(move || skill_skills::sync::sync_skills(&skill_dir, 0, None))
            .await
    ...
    match outcome {
        ...
        skill_skills::sync::SyncOutcome::Fresh { .. } => Ok("already up to date".into()),
...
pub fn get_skills_license(state: tauri::State<'_, Mutex<Box<AppState>>>) -> Option<String> {
    let skill_dir = state.lock_or_recover().skill_dir.clone();
    let license_path = skill_dir
        .join(skill_constants::SKILLS_SUBDIR)
        .join("LICENSE");
...
pub fn set_disabled_skills(
    ...
    {
        let __a = state.lock_or_recover().llm.clone();
        __a.lock_or_recover().config.tools.disabled_skills = names;
    }
    crate::save_settings(&app);
    ...
    #[cfg(feature = "llm")]
    {
        let (cell, tools) = {
            let g = state.lock_or_recover();
            {
                let __llm_arc = g.llm.clone();
                let llm = __llm_arc.lock_or_recover();
                (llm.state_cell.clone(), llm.config.tools.clone())
            }
        };
