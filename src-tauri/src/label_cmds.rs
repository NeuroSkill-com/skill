...
use crate::{save_settings_handle, unix_secs, AppState};
...
pub async fn delete_label(
    ...
    tokio::task::spawn_blocking(move || {
        let labels_db = skill_dir.join(crate::constants::LABELS_FILE);
        if !labels_db.exists() {
            return Err("labels db not found".into());
        }
...
pub async fn update_label(
    ...
    if text.is_empty() {
        return Err("label text is empty".into());
    }
    let skill_dir = crate::skill_dir(&state);
    tokio::task::spawn_blocking(move || {
        let labels_db = skill_dir.join(crate::constants::LABELS_FILE);
        ...
        let n = conn
            ...
            .map_err(|e| e.to_string())?;
        if n == 0 {
            return Err(format!("label {label_id} not found"));
        }
        Ok(())
    })
    .await
...
pub fn init_embedder(
    ...
    match build_embedder(model_code, skill_dir) {
        Ok(te) => {
            *embedder.0.lock_or_recover() = Some(te);
        }
...
pub async fn set_embedding_model(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let mut guard = embedder.0.lock_or_recover();
        let Some(te) = guard.as_mut() else { return };
        let Some(store) = skill_data::label_store::LabelStore::open(&sd2) else {
            return;
        };
...
#[tauri::command]
pub async fn get_stale_label_count(
    state: tauri::State<'_, Mutex<Box<AppState>>>,
) -> Result<usize, String> {
...
pub async fn search_labels_by_text(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        Ok(crate::label_index::search_by_text_vec(
...
pub async fn search_labels_by_eeg(
    ...
    let ef = (k * 4).max(64);
    tokio::task::spawn_blocking(move || {
        Ok(crate::label_index::search_by_eeg_vec(
            &eeg_embedding,
            k,
            ef,
            &skill_dir,
...
pub async fn reembed_all_labels(
    ...
    let (skill_dir, model_code) = crate::read_state(&state, |s| {
