...
pub fn enqueue_search_embeddings(
    ...
    queue.submit(estimated_ms, move || {
        let result =
            search_embeddings_in_range(&skill_dir, start_utc, end_utc, k, ef, Some(global_arc));
...
pub async fn interactive_search(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let ss_store = screenshot_store.or_else(|| {
            skill_data::screenshot_store::ScreenshotStore::open(&skill_dir).map(Arc::new)
        });
        ...
        let mut seen_screenshots: std::collections::HashSet<String> =
        ...
        if labels_db.exists() {
            let fl_info: Vec<(usize, i64)> = nodes
                .iter()
                .enumerate()
            ...
            let emb_info: Vec<(usize, Vec<f32>)> = fl_info
                .iter()
                .filter_map(|&(idx, lid)| {
            ...
            for (ni, nd) in nodes.iter().enumerate() {
                match nd.kind.as_str() {
                    "text_label" => {
                        ...
                        if let Some(el) = v.get_mut(0) {
                            *el += jitter;
                        }
                        if let Some(el) = v.get_mut(1) {
                            *el -= jitter * 0.5;
                        }
