...
pub(super) fn search_labels(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    ...
    match mode {
        ...
        other => {
            return Err(format!(
                "invalid mode \"{other}\": must be \"text\", \"context\", or \"both\""
            ))
        }
    }

    let (skill_dir, model_code) = crate::read_state(&app.app_state(), |s| {
        (s.skill_dir.clone(), s.ui.text_embedding_model.clone())
    });

    // Embed the query synchronously — ws_commands are called from a blocking
    // context (tokio spawn_blocking) so this is safe.
    let embedder_arc =
        std::sync::Arc::clone(&*app.state::<std::sync::Arc<crate::label_cmds::EmbedderState>>());
    let label_idx_arc =
        std::sync::Arc::clone(&*app.state::<std::sync::Arc<crate::label_index::LabelIndexState>>());
    ...
    let query_vec: Vec<f32> = {
        ...
        let te = guard
            .as_mut()
            .ok_or("text embedder not initialised — model may still be downloading")?;
        let mut vecs = te
    ...
    let merged: Vec<crate::label_index::LabelNeighbor> = match mode {
        "text" => {
            crate::label_index::search_by_text_vec(&query_vec, k, ef, &skill_dir, &label_idx_arc)
        }
        "context" => {
            crate::label_index::search_by_context_vec(&query_vec, k, ef, &skill_dir, &label_idx_arc)
        }
        _ => {
            let mut text_hits = crate::label_index::search_by_text_vec(
                &query_vec,
                k,
                ef,
                &skill_dir,
            ...
            let mut context_hits = crate::label_index::search_by_context_vec(
                &query_vec,
                k,
                ef,
                &skill_dir,
    ...
    if let (Some(fo), Some(so)) = (first_json.as_object(), second_json.as_object()) {
        for (key, fv) in fo {
            ...
            if let (Some(f), Some(s)) =
                (fv.as_f64(), so.get(key).and_then(serde_json::Value::as_f64))
            {
...
pub(super) fn interactive_search(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    use crate::commands::{
        generate_dot, get_labels_near, list_date_dirs, load_day_index_for, ts_to_unix,
        InteractiveGraphEdge, InteractiveGraphNode,
    };
    ...
    let (skill_dir, model_code, eeg_model_backend, screenshot_store) =
        crate::read_state(&app.app_state(), |s| {
            (
                s.skill_dir.clone(),
    ...
    let ss_store = screenshot_store.or_else(|| {
    ...
    for (ti, tl) in text_labels.iter().enumerate() {
        ...
        let Some(mean_eeg) =
            crate::label_index::mean_eeg_for_window(&skill_dir, tl.eeg_start, tl.eeg_end)
        else {
            continue;
        };
        ...
        for (ep_unix, ep_dist) in &eeg_candidates {
            ...
            if labels_db.exists() {
                ...
                for fl in nearby.iter().take(k_labels) {
                    ...
                    let t_dist =
                        (fl.eeg_start as f32 - *ep_unix as f32).abs() / (reach_seconds as f32);
            ...
            if let Some(ref store) = ss_store {
                let screenshots = store.around_timestamp(*ep_unix as i64, reach_seconds as i32);

                for ss in screenshots.iter().take(k_labels.max(3)) {
                    if !seen_screenshots.insert(ss.filename.clone()) {
                        continue;
                    }

                    let time_dist =
                        (ss.unix_ts as f32 - *ep_unix as f32).abs() / (reach_seconds as f32);
                    ...
                    if relevance < 0.05 && time_dist > 0.5 {
                        continue;
                    }
