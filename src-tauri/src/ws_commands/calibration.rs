...
pub fn create_calibration(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    ...
    let actions: Vec<crate::CalibrationAction> = msg
        .get("actions")
        .and_then(|v| serde_json::from_value(v.clone()).ok())
        .filter(|v: &Vec<_>| !v.is_empty())
        .ok_or_else(|| {
            "\"actions\" must be a non-empty array of {label, duration_secs}".to_string()
        })?;

    let break_secs = msg
        .get("break_duration_secs")
...
pub fn update_calibration(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    ...
    if let Some(actions) = msg
        .get("actions")
        .and_then(|v| serde_json::from_value::<Vec<crate::CalibrationAction>>(v.clone()).ok())
    {
        if !actions.is_empty() {
