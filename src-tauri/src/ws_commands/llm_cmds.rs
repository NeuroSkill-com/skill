...
pub(super) async fn llm_start(app: &AppHandle) -> Result<Value, String> {
    ...
    crate::llm::push_log(
        ...
        &log_buf,
        "info",
        "llm_start command received via WebSocket",
    );
    ...
    match new_state {
        ...
        None => Err("Failed to start LLM server. \
...
pub(super) fn llm_stop(app: &AppHandle) -> Result<Value, String> {
    ...
    if let Some(server_state) = server_state {
        ...
        crate::llm::push_log(
            ...
            &log_buf,
            "info",
            "llm_stop command received via WebSocket",
        );
...
pub(super) fn llm_download(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    let filename = msg["filename"]
        ...
        .to_string();
    crate::llm::cmds::download_llm_model(filename.clone(), app.clone(), app.app_state());
...
pub(super) fn llm_delete(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    let filename = msg["filename"]
        ...
        .to_string();
    crate::llm::cmds::delete_llm_model(filename.clone(), app.clone(), app.app_state());
    Ok(serde_json::json!({ "filename": filename }))
}
...
pub(super) fn llm_select_model(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    let filename = msg["filename"]
        ...
        .to_string();
    crate::llm::cmds::set_llm_active_model(filename.clone(), app.clone(), app.app_state());
    let state = app.app_state();
    let s = state.lock_or_recover();
    let __llm_arc = s.llm.clone();
    let llm = __llm_arc.lock_or_recover();
...
pub(super) fn llm_select_mmproj(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    let filename = msg["filename"].as_str().unwrap_or("").to_string();
    crate::llm::cmds::set_llm_active_mmproj(filename.clone(), app.clone(), app.app_state());
    let state = app.app_state();
    let s = state.lock_or_recover();
    let __llm_arc = s.llm.clone();
    let llm = __llm_arc.lock_or_recover();
...
pub(super) fn llm_pause_download(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    let filename = msg["filename"]
        ...
        .to_string();
    crate::llm::cmds::pause_llm_download(filename.clone(), app.app_state());
    Ok(serde_json::json!({ "filename": filename }))
}
...
pub(super) fn llm_resume_download(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    let filename = msg["filename"]
        ...
        .to_string();
    crate::llm::cmds::resume_llm_download(filename.clone(), app.clone(), app.app_state());
    Ok(serde_json::json!({ "filename": filename }))
}
...
pub(super) fn llm_refresh_catalog(app: &AppHandle) -> Result<Value, String> {
    crate::llm::cmds::refresh_llm_catalog(app.clone(), app.app_state());
    Ok(serde_json::json!({}))
}
...
pub(super) fn llm_downloads(app: &AppHandle) -> Result<Value, String> {
    let items = crate::llm::cmds::get_llm_downloads(app.app_state());
...
pub(super) fn llm_set_autoload_mmproj(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    ...
    crate::llm::cmds::set_llm_autoload_mmproj(enabled, app.clone(), app.app_state());
...
pub(super) fn llm_add_model(app: &AppHandle, msg: &Value) -> Result<Value, String> {
    let repo = msg["repo"]
        .as_str()
        .ok_or_else(|| {
            "llm_add_model: 'repo' field required (string, e.g. \"bartowski/Phi-4-GGUF\")"
                .to_string()
        })?
        .to_string();
    let filename = msg["filename"]
        .as_str()
        .ok_or_else(|| {
            "llm_add_model: 'filename' field required (string, e.g. \"Phi-4-Q4_K_M.gguf\")"
                .to_string()
        })?
        .to_string();
...
pub(super) fn llm_hardware_fit(app: &AppHandle, _msg: &Value) -> Result<Value, String> {
    let result = crate::llm::cmds::get_model_hardware_fit(app.app_state());
