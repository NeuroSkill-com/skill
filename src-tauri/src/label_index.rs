...
use crate::MutexExt;
use crate::commands::{unix_to_ts, NeighborMetrics};
use crate::constants::{LABELS_FILE, SQLITE_FILE};
...
pub fn mean_eeg_for_window(skill_dir: &Path, eeg_start: u64, eeg_end: u64)
    -> Option<Vec<f32>>
{
...
fn mean_metrics_for_window(skill_dir: &Path, eeg_start: u64, eeg_end: u64)
    -> Option<NeighborMetrics>
{
    ...
    // Accumulators
    let mut relax = 0f64; let mut engage = 0f64;
    let mut faa = 0f64;   let mut tar = 0f64;   let mut mood = 0f64;
    let mut meditation = 0f64; let mut cog_load = 0f64; let mut drowsy = 0f64;
    let mut hr = 0f64;    let mut snr = 0f64;
    ...
    for (_date, dir) in date_dirs(skill_dir) {
        ...
        let _ = stmt.query_map(params![ts_start, ts_end], |row| {
            ...
            relax    += g(0);  engage   += g(1);
            faa       += g(2);  tar      += g(3);  mood     += g(4);
            meditation+= g(5);  cog_load += g(6);  drowsy   += g(7);
            hr        += g(8);  snr      += g(9); rel_alpha+= g(10);
            rel_beta  += g(11); rel_theta+= g(12);
            count     += 1;
            Ok(())
    ...
    Some(NeighborMetrics {
        ...
        headache_index: None, migraine_index: None,
        consciousness_lzc: None, consciousness_wakefulness: None,
...
fn read_label_rows(labels_db: &Path) -> Vec<LabelRow> {
    ...
    let mut stmt = match conn.prepare(
        "SELECT id, text, context, eeg_start, eeg_end, created_at,
         ...
         FROM labels"
    ) { Ok(s) => s, Err(e) => { eprintln!("[label_idx] prepare: {e}"); return vec![]; } };
...
pub fn rebuild(skill_dir: &Path, state: &LabelIndexState) -> RebuildStats {
    ...
    if !labels_db.exists() {
        return RebuildStats { text_nodes: 0, eeg_nodes: 0, eeg_skipped: 0 };
    }
    ...
    let context_path = skill_dir.join(CONTEXT_INDEX_FILE);
    let eeg_path     = skill_dir.join(EEG_INDEX_FILE);
    if let Err(e) = text_idx.save(&text_path)       { eprintln!("[label_idx] text save: {e}"); }
    if let Err(e) = context_idx.save(&context_path) { eprintln!("[label_idx] context save: {e}"); }
    if let Err(e) = eeg_idx.save(&eeg_path)         { eprintln!("[label_idx] eeg save: {e}"); }
    ...
    *state.eeg.lock_or_recover()     = Some(eeg_idx);
    ...
    RebuildStats { text_nodes, eeg_nodes, eeg_skipped }
}
...
pub fn insert_label(
    ...
    if !text_embedding.is_empty() {
        ...
        if let Some(ref mut idx) = *guard {
            ...
            if let Err(e) = idx.save(&path) { eprintln!("[label_idx] text save: {e}"); }
        }
    }
    ...
    if !context_embedding.is_empty() {
        ...
        if let Some(ref mut idx) = *guard {
            ...
            if let Err(e) = idx.save(&path) { eprintln!("[label_idx] context save: {e}"); }
        }
    }
    ...
    if let Some(mean_emb) = mean_eeg_for_window(&skill_dir_buf, eeg_start, eeg_end) {
        ...
        if let Some(ref mut idx) = *guard {
            ...
            if let Err(e) = idx.save(&path) { eprintln!("[label_idx] eeg save: {e}"); }
        }
    }
}
