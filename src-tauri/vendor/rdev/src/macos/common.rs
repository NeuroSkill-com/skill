...
pub unsafe fn convert(
    ...
    if let Some(event_type) = option_type {
        // PATCH (macOS 26+): `TSMGetInputSourceProperty` / `TISGetInputSourceProperty`
        // now asserts it is called on the main dispatch queue (via
        // `islGetInputSourceListWithAdditions` → `dispatch_assert_queue`).
        // Calling it from a CGEventTap callback (which runs on the thread that
        // called `CFRunLoopRun`, never the main thread) triggers EXC_BREAKPOINT.
        //
        // We never consume `event.name` — we only use `event.event_type` to
        // update AtomicU64 timestamps — so returning `None` here is correct and
        // eliminates the crash with zero functional loss.
        //
        // Original code:
