...
function onPointerUp(_e: PointerEvent) {
  ...
  const newXMin = Math.min(a.t, b.t),
    newXMax = Math.max(a.t, b.t);
  const newYMin = Math.min(a.v, b.v),
    newYMax = Math.max(a.v, b.v);
...
function drawCanvas2D() {
  ...
  const ml = Math.round(W * 0.06),
    mr = Math.round(W * 0.02);
  const mt = 4,
    mb = Math.max(18, Math.round(H * 0.14));
  const cw = W - ml - mr,
    ch = H - mt - mb;
...
$effect(() => {
  ...
  const _ymin = yMin;
  const _ymax = yMax;
  const _zxmin = zoomXMin;
  const _zxmax = zoomXMax;
  const _zymin = zoomYMin;
  const _zymax = zoomYMax;
