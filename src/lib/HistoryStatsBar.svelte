  ...
  <div class="rounded-2xl border border-border dark:border-white/[0.06]
    ...
    <div class="flex items-center gap-5">
      <div class="flex flex-col items-center">
        <span class="text-[0.85rem] font-bold tabular-nums">{daysCount}</span>
...
{:else}
  ...
  <div class="flex items-center gap-4 mb-1 px-1">
    <div class="flex items-center gap-1">
      <span class="text-[0.62rem] font-bold tabular-nums">{daysCount}</span>
      <span class="text-[0.52rem] text-muted-foreground">{t("history.days")}</span>
    </div>
    {#if historyStats}
      <div class="flex items-center gap-1">
        <span class="text-[0.62rem] font-bold tabular-nums">{totalHours.toFixed(1)}</span>
        <span class="text-[0.52rem] text-muted-foreground">{t("history.hoursTotal")}</span>
      </div>
      <div class="flex items-center gap-1">
        <span class="text-[0.62rem] font-bold tabular-nums">{historyStats.total_sessions}</span>
        <span class="text-[0.52rem] text-muted-foreground">{t("history.sessions")}</span>
      </div>
    {/if}
  </div>
{/if}
