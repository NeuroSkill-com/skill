...
<script lang="ts">
  ...
  import {
    UMAP_COLOR_A, UMAP_COLOR_B, UMAP_SCALE, UMAP_ANIM_MS,
    UMAP_LINK_PALETTE, UMAP_BG, UMAP_BG_LIGHT,
    UMAP_DATE_SAT, UMAP_DATE_LIT,
    UMAP_POINT_SIZE, UMAP_SCALE_MIN, UMAP_SCALE_MAX,
  ...
  const COLOR_A = UMAP_COLOR_A, COLOR_B = UMAP_COLOR_B;
  const SCALE = UMAP_SCALE, ANIM_MS = UMAP_ANIM_MS;
  ...
  function buildTraceTimeTicks(sorted: number[]) {
    if (sorted.length < 2) { traceTimeRange = null; traceTimeTicks = []; return; }
    ...
    if (span <= 0) { traceTimeTicks = []; return; }
  ...
  const uniqueLabels = $derived.by((): LabelEntry[] => {
    ...
    for (const p of curPoints) {
      ...
      if (p.session === 0) e.inA = true; else e.inB = true;
    }
  ...
  function randomPositions(pts: UmapPoint[]): Float32Array {
    ...
    for (let i = 0; i < n; i++) {
      ...
      out[i*3] = cx + gauss()*3; out[i*3+1] = gauss()*3; out[i*3+2] = gauss()*3;
    }
    return out;
  }
  ...
  function applyHighlight() {
    ...
    if (selectedLabel && baseColors) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        for (const j of selIdx) {
          ...
          if (dx*dx + dy*dy + dz*dz <= d2) { proxIdx.add(i); proxSet.add(lbl); break; }
        }
      }
    }
    ...
    if (selectedLabel && scene && THREE && selIdx.size > 0 && proxIdx.size > 0) {
      ...
      const selVecs = [...selIdx].map(i =>
        new THREE.Vector3(curPositions[i*3], curPositions[i*3+1], curPositions[i*3+2]),
      );

      // For each proximate label keep only the single closest (proxPt → selPt) pair.
      interface BestEdge { pVec: ThreeVector3; sVec: ThreeVector3; dist: number; hue: number }
      ...
      for (const ptIdx of proxIdx) {
        ...
        let minD = Infinity, nearSel = selVecs[0];
        for (const sv of selVecs) { const d = pVec.distanceTo(sv); if (d < minD) { minD = d; nearSel = sv; } }
      ...
      for (const { pVec, sVec, dist, hue } of best.values()) {
        ...
        const radius  = dotR * (0.06 + 0.70 * t);    // thin at edge, fat when close
        const opacity = 0.20 + 0.70 * t;             // subtle at edge, strong when close
  ...
  function addLinks(label: string) {
    ...
    group.add(new THREE.Line(lg, new THREE.LineBasicMaterial({
      color: linkColor, transparent: true, opacity: 0.6, linewidth: 2,
    })));
    ...
    for (const idx of matching) { cx2+=curPositions[idx*3]; cy2+=curPositions[idx*3+1]; cz2+=curPositions[idx*3+2]; }
    cx2/=matching.length; cy2/=matching.length; cz2/=matching.length;
    const dm = new THREE.LineDashedMaterial({
      color: linkColor, transparent: true, opacity: 0.25, dashSize: 0.3, gapSize: 0.2, linewidth: 2,
    });
    for (const idx of matching) {
  ...
  function startTrace() {
    ...
    traceTimer = setInterval(() => {
      ...
      const ax = curPositions[iA*3], ay = curPositions[iA*3+1], az = curPositions[iA*3+2];
      const bx = curPositions[iB*3], by = curPositions[iB*3+1], bz = curPositions[iB*3+2];
  ...
  function updateTraceGrow() {
    ...
    for (let i = 0; i < traceGrowing.length; i++) {
      ...
      if (seg.edgeLine) {
        ...
        ep.array[3] = bx; ep.array[4] = by; ep.array[5] = bz;
        ep.needsUpdate = true;
      }
  ...
  function buildCloud(pts: UmapPoint[], positions: Float32Array) {
    const n = pts.length;
    const colA = new THREE.Color(COLOR_A), colB = new THREE.Color(COLOR_B);
    ...
    let gradMinUtc = Infinity, gradMaxUtc = -Infinity;
    if (gradSess) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (pts[i].label) { lPos.push(positions[i*3],positions[i*3+1],positions[i*3+2]); lCol.push(c.r,c.g,c.b); lIdx.push(i); }
    }
    ...
    mainCloud = new THREE.Points(g, new THREE.PointsMaterial({ size: ps, vertexColors: true, transparent: true, opacity: 0.5, sizeAttenuation: true, depthWrite: false }));
    scene.add(mainCloud);
    ...
    if (lIdx.length) {
      ...
      for (let li = 0; li < lIdx.length; li++) {
        ...
        const d = pts[i].dist;
        const sizeMult = d != null
          ? SCALE_NEAR - (d - distMin) / distSpan * (SCALE_NEAR - SCALE_FAR)
          : 1.0;

        // Dot: sphere for session A, octahedron for session B.
        const dotGeo = sess === 0
          ? new THREE.SphereGeometry(DOT_R, 8, 8)
          : new THREE.OctahedronGeometry(DOT_R);
        ...
        const ringGeo = sess === 0
          ? new THREE.RingGeometry(RING_I, RING_O, 24)
          : new THREE.RingGeometry(RING_I, RING_O, 4);
        const ring = new THREE.Mesh(ringGeo, new THREE.MeshBasicMaterial({
          color: ringCol, transparent: true, opacity: 0.85, side: THREE.DoubleSide,
        }));
      ...
      labelCloud.__updatePositions = (pos: Float32Array) => {
        ...
        for (let li = 0; li < lIdx.length; li++) {
          ...
          // dot
          labelGrp.children[ci].position.set(px, py, pz); ci++;
          // ring
          labelGrp.children[ci].position.set(px, py, pz); ci++;
        }
      };
    }
    curPoints = pts; labeledIdx = lIdx; curPositions = positions.slice();
  ...
  function onDataChange(d: UmapResult) {
    ...
    clearHighlightEdges();
    animating       = true;
    console.log(`[UmapViewer3D] data: ${d.points.length} pts, dim=${d.dim}`);
    const target = normalise(d.points);
    const start = randomPositions(d.points);
    buildCloud(d.points, start);
    fromPos = start; toPos = target; animStart = performance.now();
  ...
  $effect(() => {
    const s = pointScale;
    if (prevPointScale === null) { prevPointScale = s; return; }
  ...
  $effect(() => {
    ...
    if (prevColorByDate === null) { prevColorByDate = cbd; return; }
  ...
  let prevTimeGradient: "A" | "B" | null | undefined = undefined;
  $effect(() => {
    const tg = timeGradient;
    if (prevTimeGradient === undefined) { prevTimeGradient = tg; return; }
  ...
  onMount(async () => {
    ...
    try {
      ...
      console.log(`[UmapViewer3D] init ${w}×${h}`);
      ...
      canvas.addEventListener("pointermove", (e: PointerEvent) => {
        if (!curPoints.length) { tooltip = null; return; }
        ...
        if (hitIdx < 0 && mainCloud) { const h = raycaster.intersectObject(mainCloud); if (h.length) hitIdx = h[0].index!; }
        if (hitIdx >= 0 && hitIdx < curPoints.length) {
          const p = curPoints[hitIdx]; const dt = new Date(p.utc * 1000);
          ...
          const lb = p.label ? `\n🏷 ${p.label}` : "";
          const ch = p.label ? `\n${t("umap.clickConnect")}` : "";
          tooltip = { x: e.clientX - r.left, y: e.clientY - r.top, text: `${dateStr}${s} · ${dt.toLocaleTimeString()}${distStr}${lb}${ch}` };
        } else { tooltip = null; }
      });
      canvas.addEventListener("pointerleave", () => { tooltip = null; });

      // ── Click ────────────────────────────────────────────────────────
      canvas.addEventListener("pointerdown", (e: PointerEvent) => { downPos = { x: e.clientX, y: e.clientY }; });
      canvas.addEventListener("pointerup", (e: PointerEvent) => {
        ...
        if (!label && mainCloud) { const h = raycaster.intersectObject(mainCloud); if (h.length) label = curPoints[h[0].index!]?.label; }
        if (label) { toggleLinks(label); }
      });
      ...
      function loop() {
        ...
        if (fromPos && toPos && mainCloud) {
          ...
          if (t >= 1) {
            ...
            animating = false;
            if (pendingAutoConnect) { pendingAutoConnect = false; runAutoConnect(); }
          }
        }
      ...
      loaded = true;
      console.log("[UmapViewer3D] ready, scene children:", scene.children.length);
