...
<div class="electrode-placement flex flex-col items-center gap-2 {compact ? '' : 'py-2'}"
  ...
  {#if !compact}
    <h3 class="text-ui-md font-bold tracking-tight">{t("electrode.title")}</h3>
    <span class="text-ui-sm text-muted-foreground">{deviceLabel} — {ELECTRODES.length} electrodes</span>
  {/if}
  ...
  {#if !compact}
    <div class="grid {isMw75 ? 'grid-cols-3' : 'grid-cols-2'} gap-x-3 gap-y-1 text-ui-sm">
      {#each ELECTRODES as el, i}
