...
if (typeof window !== "undefined") {
  import("$lib/charts/ElectrodeHead3D.svelte").then((m) => {
    Head3D = m.default;
  });
}
...
<div class="flex flex-col items-center gap-3">
  ...
  <div class="flex flex-wrap items-center gap-x-2 gap-y-1 w-full max-w-[480px] rounded-lg border border-border dark:border-white/[0.06]
              bg-muted/20 px-3 py-1.5">
    <span class="text-ui-xs font-semibold text-muted-foreground/60 uppercase tracking-wider shrink-0">Signal</span>
    {#each stripNames as chName, idx}
      ...
      <div class="flex items-center gap-1">
        ...
        <span class="text-ui-xs font-mono font-bold" style="color:{qualityColor(q)}">{chName}</span>
      </div>
    {/each}
  </div>
  {/if}
  ...
  <div
    class="relative rounded-xl border border-border dark:border-white/[0.06]
           bg-surface-1 overflow-hidden cursor-grab active:cursor-grabbing
           w-full max-w-[480px]"
  ...
  >
    ...
    {:else}
      <div class="absolute inset-0 flex items-center justify-center text-muted-foreground text-ui-md">
        Loading 3D view…
      </div>
    {/if}
      ...
      <div class="flex items-center gap-1.5 mb-2 flex-wrap">
        ...
        {#if el.muse && effectiveQuality}
          ...
          {#if chIdx >= 0}
            ...
            <span class="text-ui-xs font-bold tabular-nums ml-1" style="color:{qualityColor(q)}">
              Signal: {labelToText(effectiveLabels[chIdx])}
            </span>
          {/if}
        {/if}
        <button
          class="ml-auto text-ui-xs text-muted-foreground hover:text-foreground cursor-pointer"
          onclick={() => selectedElectrode = null}
        >✕</button>
      </div>
      <div class="space-y-1 text-ui-sm leading-relaxed">
        <div>
          <span class="text-muted-foreground/50 font-semibold text-ui-2xs uppercase tracking-wider">Position</span>
          <p class="text-foreground/80">{el.lobe} — {el.function}</p>
        </div>
        <div>
          <span class="text-muted-foreground/50 font-semibold text-ui-2xs uppercase tracking-wider">Signals</span>
          <p class="text-foreground/80">{el.signals}</p>
        </div>
        {#if el.museRole}
          <div>
            <span class="text-muted-foreground/50 font-semibold text-ui-2xs uppercase tracking-wider">Muse Role</span>
            <p class="text-indigo-400 text-ui-sm">{el.museRole}</p>
          </div>
        {/if}
      </div>
    </div>
