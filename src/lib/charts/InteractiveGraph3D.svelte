...
<div class="relative w-full h-full" bind:this={container}
  ...
  {#if tooltip}
    <div class="pointer-events-none fixed z-50 px-2.5 py-1.5 rounded-lg
                ...
                text-ui-base max-w-[260px] backdrop-blur-sm"
         style="left:{tooltip.x + 12}px; top:{tooltip.y}px;">
      {#each tooltip.lines as line, li}
        <div class="{li === 0 ? 'font-bold text-foreground' : li === tooltip.lines.length-1 && selectedNodeId === null && hoveredNodeId ? 'text-muted-foreground/40 italic text-ui-xs' : 'text-muted-foreground/80'} leading-snug">
          {line}
        </div>
      {/each}
    </div>
  ...
  <button onclick={resetCamera}
          class="absolute top-2 left-2 px-2 py-1 rounded-md text-ui-2xs
  ...
  {#if selectedNodeId !== null}
    <div class="pointer-events-none absolute bottom-2 left-1/2 -translate-x-1/2
                ...
                text-ui-2xs text-muted-foreground/55 backdrop-blur-sm select-none">
