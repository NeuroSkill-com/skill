...
<div class="flex flex-col gap-1.5">
  ...
  <div class="flex items-center gap-4 flex-wrap">
    <div class="flex items-center gap-1.5">
      <span class="text-ui-2xs font-semibold text-muted-foreground/70">{t("dashboard.accel")}</span>
      {#each AXIS_LABELS as axis, i}
        <span class="font-mono text-ui-xs tabular-nums" style="color:{AXIS_COLORS[i]}">
          {axis}{latestAccel[i] >= 0 ? "+" : ""}{latestAccel[i].toFixed(3)}
        </span>
      {/each}
    </div>
    <div class="flex items-center gap-1.5">
      <span class="text-ui-2xs font-semibold text-muted-foreground/70">{t("dashboard.gyro")}</span>
      {#each AXIS_LABELS as axis, i}
        <span class="font-mono text-ui-xs tabular-nums" style="color:{AXIS_COLORS[i]}">
  ...
  <canvas
    ...
    class="w-full rounded-lg bg-black/[0.03] dark:bg-white/[0.02]"
