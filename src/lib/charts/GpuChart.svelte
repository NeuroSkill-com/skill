...
async function pollGpu() {
  try {
    ...
    if (gpu !== null) {
      ...
      history = [...history.filter((p) => p.ts >= ts - WIN_MS - 2_000), { overall: gpu.overall, tiler: gpu.tiler, ts }];
    }
  } catch (err) {
    gpuErrCount++;
...
{#if visible}
  <div class="w-full rounded-xl border border-border dark:border-white/[0.06] overflow-hidden mb-2">
    ...
    <div class="flex items-center gap-2 px-3.5 pt-2 pb-1.5 bg-surface-1">
      <SectionHeader class="shrink-0">{t("gpu.title")}</SectionHeader>
      <span class="gpu-live w-1.5 h-1.5 rounded-full shrink-0" style="background:{col}"></span>
      <span class="ml-auto text-ui-xs text-muted-foreground/50 tabular-nums shrink-0">
        {t("gpu.render")}&nbsp;{(gpuStats!.render * 100).toFixed(0)}%&nbsp;·&nbsp;{t("gpu.tiler")}&nbsp;{(gpuStats!.tiler * 100).toFixed(0)}%
      </span>
      <span class="text-ui-md font-bold tabular-nums leading-none shrink-0" style="color:{col}">
