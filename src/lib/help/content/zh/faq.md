...
## ZUNA 编码器做什么？
ZUNA 是 {app} 中多个 EEG 嵌入后端之一。它是一个 GPU 加速的 transformer 编码器，将 5 秒 EEG 时段转换为紧凑的嵌入向量。这些向量捕捉每个时刻的神经特征，驱动相似性搜索功能。其他后端包括 LUNA 和 NeuroRVQ。
...
## 为什么我的设备自动断开连接了？
如果在收到至少一帧 EEG 数据后 30 秒内没有新数据到达，{app} 会将设备视为静默断开（例如移出 BLE 范围或在未正常断开的情况下关机）。托盘图标恢复为灰色，扫描自动恢复。

## 如何授予蓝牙权限？
macOS 会在 {app} 首次尝试连接时显示权限对话框。如果您之前关闭了它，请进入系统设置 → 隐私与安全性 → 蓝牙并启用 {app}。

## 数据库中存储了哪些指标？
每 2.5 秒时段存储：EEG 嵌入向量、各通道平均的相对频段功率（delta、theta、alpha、beta、gamma、high-gamma）、以 JSON 格式存储的每通道频段功率、派生评分（放松度、参与度）、FAA、交叉频段比（TAR、BAR、DTR）、频谱形状（PSE、APF、BPS、SNR）、相干性、Mu 抑制、情绪指数，以及可用时的 PPG 平均值。
...
## 什么是标签，如何使用？
标签是用户自定义的标记（如"冥想"、"阅读"），附加到录制过程中的某个时刻。它们与 EEG 嵌入一起存储。在 UMAP 查看器中，带标签的点显示得更大并带有彩色圆环——点击一个可以追踪该标签在两个会话中的时间变化。
...
## 什么是 WebSocket API？
{app} 在本地网络上提供 JSON WebSocket API（mDNS：_skill._tcp）。命令包括：status、label、search、compare（指标 + 睡眠 + UMAP 票据）、sessions、sleep、umap（排队 3D 投影）、umap_poll（获取结果）。运行 'node test.js' 进行冒烟测试。
...
## 如何管理或编辑我的标注？
通过命令面板打开标签窗口（⌘K → "所有标签"）。它显示所有标注，支持内联文本编辑（点击标签，按 ⌘↵ 保存或 Esc 取消）、删除（需确认）以及显示 EEG 时间范围的元数据。使用搜索框按文本筛选。大量存档时标签每页显示 50 个。

## 如何并排比较两个特定会话？
在历史页面中，点击"快速比较"进入比较模式。每个会话行上会出现复选框——选择两个，然后点击"比较所选"以打开预加载两个会话的比较窗口。也可以从托盘或命令面板打开比较窗口，手动使用会话下拉菜单选择。

## 文本嵌入搜索如何工作？
您的查询由用于索引标签的同一 sentence-transformer 模型转换为向量。然后在 HNSW 标签索引中使用近似最近邻查找搜索该向量。结果是按语义相似度排名的您自己的标注——因此搜索"平静且专注"会找到"深度阅读"或"冥想"等标签，即使这些确切的词从未出现在您的查询中。需要先下载嵌入模型并构建标签索引（设置 → 嵌入）。

## 交互式跨模态搜索如何工作？
交互式搜索在单次查询中桥接文本、EEG 和时间。步骤 1：将文本查询嵌入为向量。步骤 2：找到语义最相似的 text-k 个标签。步骤 3：对于每个标签，{app} 计算其录制窗口内的平均 EEG 嵌入，并从所有每日索引中检索最近的 eeg-k 个 EEG 时段——从语言空间跨越到脑状态空间。步骤 4：对于找到的每个 EEG 时刻，收集 ±reach 分钟范围内的标注作为"发现的标签"。四个节点层（查询 → 文本匹配 → EEG 邻居 → 发现的标签）渲染为 4 层有向图。导出为 SVG 获取静态图像，或导出为 DOT 源文件在 Graphviz 中进一步处理。
...
## 我可以同时使用两个 BCI 设备吗？
可以——NeuroSkill™ 可以同时从两个设备流式传输。先连接的设备驱动实时仪表盘、频段功率显示和 EEG 嵌入管线。第二个设备的数据记录到 CSV 供离线分析。实时管线中的多设备同时分析计划在未来版本中实现。

## 为什么 Cyton 的 8 个通道中只有 4 个用于实时分析？
实时分析管线（滤波器、频段功率、EEG 嵌入、信号质量指示点）目前设计为 4 通道输入，以匹配 Muse 头戴设备格式。对于 Cyton（8 通道）和 Cyton+Daisy（16 通道），通道 1–4 馈入实时管线；所有通道均写入 CSV 供离线使用。完整的多通道管线支持在开发路线图中。
...
## {app} 是否支持 AWEAR 头带？
是的。AWEAR 是一款单通道 BLE EEG 设备，采样率为 256 Hz。连接方式与其他 BLE 设备相同——打开头带电源，如果提示则授予蓝牙权限，{app} 将自动发现并连接。单个 EEG 通道驱动实时分析管线。
...
键盘和鼠标追踪使用 CGEventTap——一个在输入事件到达各应用之前拦截系统级输入事件的 macOS API。Apple 要求任何读取全局输入的应用都具有辅助功能权限，无论该应用如何使用这些数据。没有辅助功能权限时，tap 会静默失败：NeuroSkill 继续正常工作，但上次键盘和上次鼠标时间戳保持为零。授权方法：系统设置 → 隐私与安全性 → 辅助功能 → 找到 NeuroSkill → 开启。如果您不想授权，请在设置中禁用"追踪键盘和鼠标活动"开关——这将从根本上阻止安装钩子。活动窗口追踪（应用名称和路径）使用 AppleScript/osascript，不需要辅助功能权限。
...
## 如何在 macOS 上授予辅助功能权限？
打开系统设置 → 隐私与安全性 → 辅助功能。在列表中找到 {app} 并开启。您也可以在应用内的权限选项卡中点击"打开辅助功能设置"。
