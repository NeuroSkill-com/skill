...
<div class="flex flex-col gap-6 pb-6">
    ...
    <div class="rounded-xl border border-border dark:border-white/[0.06]
      ...
      {#each commandSection.items as item}
        ...
        <div class="px-4 py-3 flex flex-col gap-1">
          ...
          <span class="text-ui-md text-muted-foreground/70">{parts[0] ?? ""}</span>
  ...
  <HelpSection title={t("helpApi.discoveryWireFormat")}>
    <div class="rounded-xl border border-border dark:border-white/[0.06]
                bg-muted/50 dark:bg-surface-3 px-4 py-3 flex flex-col gap-2">
      <p class="text-ui-md font-semibold text-muted-foreground uppercase tracking-widest">
        {t("helpApi.discoverService")}
      </p>
      <pre class="text-ui-md font-mono text-foreground/80 whitespace-pre-wrap"># macOS
dns-sd -B _skill._tcp

# Linux
avahi-browse _skill._tcp</pre>
      <p class="text-ui-md font-semibold text-muted-foreground uppercase tracking-widest mt-1">
        {t("helpApi.outboundEvents")}
      </p>
      <pre class="text-ui-md font-mono text-foreground/80 whitespace-pre-wrap">{`{ "event": "eeg-bands" | "status" | "label-created", "payload": { … } }`}</pre>
      <p class="text-ui-md font-semibold text-muted-foreground uppercase tracking-widest mt-1">
        {t("helpApi.inboundCommands")}
      </p>
      <pre class="text-ui-md font-mono text-foreground/80 whitespace-pre-wrap">{`{ "command": "status" }
...
{ "command": "compare", "a_start_utc": …, "a_end_utc": …, "b_start_utc": …, "b_end_utc": … }`}</pre>
      ...
      <pre class="text-ui-md font-mono text-foreground/80 whitespace-pre-wrap">{`{ "command": "status", "ok": true, "device": { … }, … }
