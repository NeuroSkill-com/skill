...
export function buildTextKnnGraph(results: LabelNeighbor[], query: string): UmapResult {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const radius = (0.18 + t * 0.7) * GRAPH_SCALE;
...
export function computeIxTimeHeatmap(nodes: GraphNode[]) {
  ...
  const dayLabels = days.map((d) =>
    new Date(Date.UTC(+d.slice(0, 4), +d.slice(4, 6) - 1, +d.slice(6, 8), 12)).toLocaleDateString(undefined, {
      month: "short",
      day: "numeric",
    }),
  );
 ...
 */
export function dedupeFoundLabels(nodes: GraphNode[], edges: GraphEdge[]): { nodes: GraphNode[]; edges: GraphEdge[] } {
  const canonMap = new Map<string, string>();
  const keepNodes: GraphNode[] = [];

  for (const n of nodes) {
    if (n.kind !== "found_label") {
      keepNodes.push(n);
      continue;
    }
    const key = (n.text ?? "").trim().toLowerCase();
    if (!canonMap.has(key)) {
      canonMap.set(key, n.id);
      keepNodes.push(n);
    }
  }
