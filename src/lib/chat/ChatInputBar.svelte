...
import type { Attachment, ServerStatus } from "$lib/chat/chat-types";
import PromptLibrary from "$lib/chat/PromptLibrary.svelte";
...
<footer class="relative shrink-0 border-t border-border dark:border-white/[0.06]
  ...
  {#if eegBlocked}
    <div class="flex items-center gap-2 mb-2 px-2 py-1.5 rounded-lg
      ...
      <span class="text-ui-base font-medium leading-snug">
  ...
  {#if nCtx > 0 && liveUsedTokens > 0}
    ...
    {#if warnPct >= 85}
      <div class="flex items-center justify-center gap-1.5 mb-1.5 px-2 py-1 rounded-md
        ...
        <span class="text-ui-xs {warnPct >= 95 ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400'} leading-tight">
  ...
  <div class="flex items-center justify-center gap-1.5 mb-1.5 px-2 py-1 rounded-md
    ...
    <span class="text-ui-xs text-amber-600/70 dark:text-amber-400/70 leading-tight select-none">
  ...
  <div class="flex gap-2 rounded-xl border border-border dark:border-white/[0.08]
    ...
    <button
      ...
      class="shrink-0 p-1 rounded-md transition-colors cursor-pointer self-center
             ...
             {promptLibRef?.isOpen()
               ...
               : 'text-muted-foreground/50 hover:text-violet-600 dark:hover:text-violet-600 hover:bg-violet-500/10'}">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"
           stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4">
    ...
    <textarea
      ...
      disabled={status !== "running" || generating || eegBlocked}
      rows="1"
      class="flex-1 bg-transparent text-ui-lg text-foreground resize-none overflow-y-auto
  ...
  <p class="text-ui-xs text-muted-foreground/30 text-center mt-1.5">
    {#if eegBlocked}
      {t("chat.hint.noEeg")}
