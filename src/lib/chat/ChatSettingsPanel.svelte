  ...
  <div class="flex flex-col gap-1.5">
    <div class="flex items-baseline justify-between gap-2">
      <label for="chat-system-prompt"
             class="text-ui-sm font-semibold uppercase tracking-widest text-muted-foreground">
        {t("chat.systemPrompt")}
      </label>
      <div class="flex items-center gap-2">
        ...
        {#if !isDefaultPrompt}
          <button
            ...
            class="flex items-center gap-0.5 text-ui-sm text-muted-foreground/50
                   hover:text-violet-600 dark:hover:text-violet-600 transition-colors
    ...
    <textarea
      ...
      rows="4"
      spellcheck="true"
      class="w-full rounded-lg border border-border bg-background text-ui-md
    ...
    <div class="flex flex-col gap-1">
      <span class="text-ui-xs font-semibold uppercase tracking-widest
      ...
      <div class="flex flex-wrap gap-1">
        {#each SYSTEM_PROMPT_PRESETS as preset}
          ...
          <button
            ...
            title={preset.prompt}
            class="flex items-center gap-1 px-2 py-0.5 rounded-md text-ui-sm
                   border transition-all cursor-pointer select-none
                   {isActive
                     ? 'border-violet-500/50 bg-violet-500/10 text-violet-600 dark:text-violet-400'
                     : 'border-border bg-background text-muted-foreground/70 hover:border-violet-500/40 hover:bg-violet-500/8 hover:text-foreground'}">
    ...
    <div class="grid grid-cols-4 gap-1.5 rounded-lg border border-cyan-500/20
      ...
      {#each [
        ...
        <div class="flex flex-col items-center gap-0">
          <span class="text-ui-2xs text-cyan-600/60 dark:text-cyan-400/60 font-medium uppercase">
            {s.label}
          </span>
          <span class="text-ui-sm font-semibold tabular-nums text-cyan-700 dark:text-cyan-300">
            {s.val}
          </span>
        </div>
      {/each}
    ...
    {#each [
      ...
      <div class="flex flex-col gap-0.5">
        <div class="flex items-baseline justify-between">
          <span class="text-ui-sm text-muted-foreground">{t(s.labelKey)}</span>
          <span class="text-ui-sm font-mono text-foreground tabular-nums">{s.value}</span>
        </div>
