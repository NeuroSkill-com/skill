...
import type { BandPowers, BandSnapshot } from "$lib/chat/chat-types";
...
export function buildEegBlock(b: BandSnapshot): string {
  ...
  const avg = (key: keyof BandPowers) => b.channels.reduce((s, ch) => s + (ch[key] as number), 0) / n;
  ...
  if (b.hr != null)
    lines.push(
      `Heart rate: ${b.hr.toFixed(0)} bpm${b.rmssd != null ? ` | HRV (RMSSD): ${b.rmssd.toFixed(1)} ms` : ""}`,
    );
