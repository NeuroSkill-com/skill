...
<script lang="ts">
import type { CompressionLevel, ToolConfig, ToolExecutionMode } from "$lib/chat/chat-types";
import { TOOL_THINKING_LEVELS, type ToolThinkingLevel } from "$lib/chat/chat-types";
...
<div class="flex-1 min-h-0 overflow-y-auto
            bg-surface-3 px-4 py-3 flex flex-col gap-3
            scrollbar-thin scrollbar-track-transparent scrollbar-thumb-border">

  <div class="flex flex-col gap-1.5">
    <div class="flex items-center justify-between gap-2">
      <span class="text-ui-sm font-semibold uppercase tracking-widest text-muted-foreground">
        {t("chat.tools.label")}
      </span>
      <span class="text-ui-xs tabular-nums text-muted-foreground/40 select-none">
        {enabledToolCount}/9
      </span>
    </div>
      ...
      {#each [
        ...
        <button
          ...
          <div class="flex flex-col gap-0 min-w-0">
            <span class="text-ui-sm font-medium truncate">
              {t(`chat.tools.${tool.key}`)}
            </span>
            <span class="text-ui-2xs text-muted-foreground/50 truncate leading-tight">
    ...
    <div class="mt-1.5">
      <div class="flex items-center justify-between gap-2 mb-1">
        <span class="text-ui-xs text-muted-foreground/60">{t("chat.tools.executionMode")}</span>
      </div>
      <div class="flex rounded-md overflow-hidden border border-border text-ui-sm font-medium">
        {#each [
    ...
    <div class="flex items-center justify-between gap-3 mt-1.5">
      <div class="flex flex-col gap-0">
        <span class="text-ui-sm font-semibold text-foreground">{t("llm.tools.maxRounds")}</span>
        <span class="text-ui-2xs text-muted-foreground/60 leading-snug">{t("llm.tools.maxRoundsDesc")}</span>
      </div>
      <div class="flex items-center gap-0.5 shrink-0">
        {#each [1, 3, 5, 10] as val}
          <button
            onclick={() => onUpdate({ max_rounds: val })}
            class="rounded-md border px-1.5 py-0.5 text-ui-sm font-semibold transition-all cursor-pointer
    ...
    <div class="flex items-center justify-between gap-3 mt-0.5">
      <div class="flex flex-col gap-0">
        <span class="text-ui-sm font-semibold text-foreground">{t("llm.tools.maxCallsPerRound")}</span>
        <span class="text-ui-2xs text-muted-foreground/60 leading-snug">{t("llm.tools.maxCallsPerRoundDesc")}</span>
      </div>
      <div class="flex items-center gap-0.5 shrink-0">
        {#each [1, 2, 4, 8] as val}
          <button
    ...
    <div class="flex items-center justify-between gap-3 mt-0.5">
      <div class="flex flex-col gap-0">
        <span class="text-ui-sm font-semibold text-foreground">{t("llm.tools.contextCompression")}</span>
        <span class="text-ui-2xs text-muted-foreground/60 leading-snug">{t("llm.tools.contextCompressionDesc")}</span>
      </div>
      <div class="flex rounded-md overflow-hidden border border-border text-ui-xs font-medium shrink-0">
        {#each [
    ...
    <div class="flex items-center justify-between gap-3 mt-0.5">
      <div class="flex flex-col gap-0">
        <span class="text-ui-sm font-semibold text-foreground">{t("chat.tools.thinkingBudget")}</span>
        <span class="text-ui-2xs text-muted-foreground/60 leading-snug">{t("chat.tools.thinkingBudgetDesc")}</span>
      </div>
      <div class="flex rounded-md overflow-hidden border border-border text-ui-xs font-medium shrink-0">
