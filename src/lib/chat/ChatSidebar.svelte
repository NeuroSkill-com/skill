...
<div class="flex flex-col h-full select-none">
  ...
  <div class="flex items-center justify-between gap-1
    ...
    <span class="text-ui-xs font-semibold uppercase tracking-widest text-muted-foreground">
  ...
  <div class="flex-1 overflow-y-auto
    ...
    {#if sessions.length === 0 && !showArchive}
      <p class="text-center text-ui-base text-muted-foreground/40 px-3 py-6 leading-snug">
              ...
              <div class="flex-1 min-w-0 pr-6 pl-1.5">
                {#if isEditing}
                  <input
                    ...
                    class="w-full text-ui-md font-medium bg-background border border-violet-500/40
                           rounded px-1.5 py-0.5 text-foreground focus:outline-none
                           focus:ring-1 focus:ring-primary/50"
                  />
                {:else}
                  <p class="text-ui-md font-medium text-foreground truncate leading-tight">
                    {shortLabel(s)}
                  </p>
                {/if}

                <div class="flex items-center gap-1.5 mt-0.5">
                  <span class="text-ui-sm text-muted-foreground/50 shrink-0">
                    {relTime(s.created_at)}
                  </span>
                  {#if s.message_count > 0}
                    <span class="text-ui-xs text-muted-foreground/30 tabular-nums">
    ...
    <div class="border-t border-border dark:border-white/[0.06] mt-1">
      <button
        ...
        {#if archived.length > 0}
          <span class="text-ui-2xs tabular-nums opacity-60">{archived.length}</span>
        {/if}
      </button>

      {#if showArchive}
        {#if archived.length === 0}
          <p class="text-center text-ui-sm text-muted-foreground/30 px-3 py-3">
              ...
              <li>
                <div
                  ...
                  <div class="flex-1 min-w-0 pr-12 pl-1.5">
                    <p class="text-ui-base font-medium text-foreground truncate leading-tight">
                      {shortLabel(s)}
                    </p>
                    <div class="flex items-center gap-1.5 mt-0.5">
                      <span class="text-ui-xs text-muted-foreground/50 shrink-0">
                        {relTime(s.created_at)}
                      </span>
                      {#if s.message_count > 0}
                  ...
                  <div class="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-0.5
                    ...
                    <button
                      ...
                      class="p-1 rounded-md transition-colors cursor-pointer
                             text-muted-foreground/40 hover:text-violet-600 hover:bg-violet-500/10">
                      <!-- Undo arrow icon -->
                      <svg viewBox="0 0 16 16" fill="none" stroke="currentColor"
                           stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"
                           class="w-3 h-3">
