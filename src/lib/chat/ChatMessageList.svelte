...
interface Props {
  ...
  onUpdateToolUse: (msgId: number, tuIdx: number, patch: Partial<import("$lib/chat/chat-types").ToolUseEvent>) => void;
    ...
    <div class="flex flex-col items-center justify-center flex-1 gap-4 text-center py-12">
      ...
      {#if status === "stopped"}
        <div class="flex flex-col items-center gap-2">
          <p class="text-ui-lg font-semibold text-foreground">{t("chat.empty.stopped")}</p>
          ...
          <div class="flex gap-2 mt-1">
            <button
              ...
              class="px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-700
                     text-white text-ui-md font-semibold transition-colors cursor-pointer">
      ...
      {:else if status === "loading"}
        <div class="flex flex-col items-center gap-3 w-full max-w-xs">
          <p class="text-ui-lg font-semibold text-foreground">{t("chat.status.loading")}</p>

          <!-- Step indicators -->
          <div class="flex flex-col gap-1.5 w-full">
            {#each loadingSteps as step, i}
              <div class="flex items-center gap-2 text-ui-md {loadingActiveIdx > i
      ...
      {:else}
        <p class="text-ui-lg text-muted-foreground">{t("chat.empty.ready")}</p>
      {/if}
    </div>
            ...
            {#if msg.content}
              <div class="group/user-bubble">
                <div class="rounded-2xl rounded-tr-sm bg-violet-600 text-white
                            px-3.5 py-2.5 text-ui-lg leading-relaxed whitespace-pre-wrap break-words">
                  {msg.content}
                </div>
                  ...
                  <div class="flex justify-end opacity-0 group-hover/user-bubble:opacity-100
                    ...
                    <button
                      ...
                      class="flex items-center gap-1 px-1.5 py-0.5 rounded-md
                             text-muted-foreground/50 hover:text-violet-600 dark:hover:text-violet-600
                             hover:bg-violet-500/10 transition-colors cursor-pointer text-ui-sm">
        ...
        <div class="flex justify-start gap-2.5">
          {#if msg.pending}
            <div class="w-6 h-6 shrink-0 mt-0.5 flex items-center justify-center">
              <svg class="w-5 h-5 animate-spin text-violet-600 dark:text-violet-400" viewBox="0 0 24 24" fill="none">
          ...
          {:else}
            <div class="w-6 h-6 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600
                        flex items-center justify-center shrink-0 mt-0.5 text-white text-ui-xs font-bold">
              AI
            </div>
          {/if}
          ...
          <div class="flex flex-col gap-1 max-w-[82%]">
              ...
              <div class="rounded-xl border border-violet-500/20 bg-violet-500/5
                ...
                <button
                  ...
                  {#if msg.pending && !msg.thinking?.trim()}
                    <span class="flex gap-0.5">
                      {#each [0,1,2] as i}
                        <span class="w-1 h-1 rounded-full bg-violet-500/60 animate-bounce"
                  ...
                  {:else}
                    ...
                    {#if !msg.pending && msg.thinking}
                      <span class="ml-auto text-ui-sm text-muted-foreground/50">
                ...
                {#if msg.thinkOpen && msg.thinking}
                  <div class="px-3 pb-2 pt-0 text-muted-foreground/70 leading-relaxed
                              border-t border-primary/10 text-ui-base break-words overflow-hidden">
            ...
            {#if cleanLeadInForDisplay(msg.leadIn ?? "", !!msg.toolUses?.length)}
              <div class="rounded-2xl rounded-tl-sm border border-border/70 bg-background/80
                          px-3 py-2 text-ui-md leading-relaxed text-muted-foreground
            ...
            {#if msg.content.trim()}
              <div class="group/bubble flex flex-col gap-0.5">
                <div class="rounded-2xl rounded-tl-sm bg-muted dark:bg-surface-2
                            px-3.5 py-2.5 text-ui-lg leading-relaxed text-foreground
                  ...
                  <div class="flex gap-1 opacity-0 group-hover/bubble:opacity-100 transition-opacity duration-150">
                    <button
                      ...
                      class="flex items-center gap-1 px-1.5 py-0.5 rounded-md
                             ...
                             hover:bg-muted transition-colors cursor-pointer text-ui-sm">
            ...
            {#if msg.pending && generating && streamTokens > 0}
              ...
              <span class="text-ui-xs text-violet-500/70 tabular-nums px-1 animate-pulse">
              ...
              <div class="flex flex-col gap-1 px-1">
                ...
                {#if msg.usage && msg.usage.n_ctx > 0}
                  ...
                  <div class="flex items-center gap-1.5" title="{t('chat.ctxUsage')}: {msg.usage.total_tokens} / {msg.usage.n_ctx} {t('chat.tok')} ({usedPct}%)">
                    ...
                    <span class="text-ui-2xs tabular-nums text-muted-foreground/40 shrink-0">
...
{#if !pinned}
  <button
    ...
    class="absolute bottom-3 left-1/2 -translate-x-1/2
           ...
           text-ui-base font-medium text-muted-foreground
