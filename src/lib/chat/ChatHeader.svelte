...
import type { BandSnapshot, ServerStatus } from "$lib/chat/chat-types";
...
<header class="relative flex flex-nowrap items-center gap-2 px-3 py-2 border-b border-border dark:border-white/[0.06]
                bg-surface-3 shrink-0 overflow-hidden min-h-0"
  ...
  {#if supportsTools}
    <button
      ...
      class="flex items-center gap-1 px-1.5 py-0.5 rounded-md transition-colors cursor-pointer
             shrink-0 text-ui-sm font-semibold
             {showTools
               ? 'bg-violet-500/20 text-violet-600 dark:text-violet-400 ring-1 ring-primary/30'
               : enabledToolCount > 0
  ...
  {#if status === "loading" && loadingDetail}
    <div class="flex items-center gap-1.5 shrink-0 text-ui-sm text-violet-600 dark:text-violet-400 font-medium">
  ...
  {#if canStart}
    <button
      ...
      class="flex items-center gap-1 text-ui-base font-semibold px-2.5 py-1
