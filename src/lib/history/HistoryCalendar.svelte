...
<div class="flex flex-col gap-2" transition:fade={{ duration: 120 }}>
  {#if viewMode === "year"}
    ...
    <div class="rounded-xl border border-border dark:border-white/[0.06]
                bg-surface-1 p-4 overflow-x-auto">
  ...
  {:else if viewMode === "month"}
    ...
    <div class="rounded-xl border border-border dark:border-white/[0.06]
                bg-surface-1 p-4">
      <div class="text-center text-ui-base font-semibold text-foreground mb-2">
        {calendarMonth}
      </div>
      ...
      <div class="grid grid-cols-7 gap-1">
        {#each calendarCells as cell}
          ...
          <div
            ...
            onclick={() => { if (cell.count > 0) navigateToDay(cell.dayKey); }}
          >
            <span class="text-ui-2xs tabular-nums leading-none
  ...
  {:else if viewMode === "week"}
    <!-- Week timeline grid -->
    <div class="rounded-xl border border-border dark:border-white/[0.06]
                bg-surface-1 p-3 overflow-x-auto">
      <!-- Hour labels header -->
      ...
      {#each weekGridDays as wgd (wgd.dayKey)}
        <div class="grid gap-px items-center" style="grid-template-columns: 72px repeat(48, minmax(0,1fr))">
          <!-- svelte-ignore a11y_click_events_have_key_events -->
          <!-- svelte-ignore a11y_no_static_element_interactions -->
          <div class="text-ui-2xs font-semibold tabular-nums text-muted-foreground/60 pr-2 text-right truncate
          ...
          <div class="col-span-48 h-5 relative">
            {#each wgd.sessions as sess}
              ...
              <button type="button"
                   class="absolute top-0.5 bottom-0.5 rounded-sm bg-violet-500/60 dark:bg-violet-500/60/50
                          hover:bg-violet-500 dark:hover:bg-violet-500/40 transition-colors cursor-pointer border-0 p-0"
  ...
  <div class="flex items-center gap-3 text-ui-2xs text-muted-foreground/50 px-1 flex-wrap">
