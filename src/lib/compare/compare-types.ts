...
export const insightMetrics: { key: keyof SessionMetrics; label: string }[] = [
  ...
  { key: "stress_index", label: "Stress" },
  { key: "coherence", label: "Coherence" },
  { key: "snr", label: "SNR" },
  { key: "mu_suppression", label: "μ Suppression" },
...
export function bv(m: SessionMetrics | null, k: BK): number {
  return m ? (m[k] ?? 0) : 0;
}
...
import { gauss } from "$lib/umap/umap-helpers";
