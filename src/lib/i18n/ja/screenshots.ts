...
const screenshots: Record<string, string> = {
  ...
  "screenshots.backendRlx": "RLX（ローカル、safetensors）",
  ...
  "screenshots.ocrEngineOcrs": "rlx-ocr — ローカル CPU / GPU（クロスプラットフォーム）",
  "screenshots.ocrAppleVisionHint": "⚡ Apple Vision は GPU/ANE で動作し、macOS では rlx-ocr より高速です",
  ...
  "screenshots.ocrDesc":
    "テキストは rlx-ocr エンジンを使用してダウンサイズ前のフル解像度で各スクリーンショットから抽出されます。抽出されたテキストは nomic-embed-text-v1.5 で埋め込まれ、セマンティックテキスト検索用の別の HNSW インデックスにインデックス化されます。OCR モデルはバンドルされており、オフラインで動作します — ダウンロードは不要です。",
