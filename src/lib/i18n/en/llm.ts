...
const llm: Record<string, string> = {
  ...
  "model.reembedDesc":            "Re-encode all raw EXG session recordings with the selected model backend and rebuild HNSW indices.",
  ...
  "llm.tools.dateHint":                 "Gives the model awareness of the current date, time, Unix timestamp, and timezone. Useful for time-sensitive questions, scheduling, and relative date calculations. No data leaves your machine.",
  "llm.tools.locationHint":             "Performs a single IP-geolocation lookup to determine your approximate country, region, city, and timezone. Uses a public API — your IP address is sent to the geolocation service. No precise GPS coordinates are shared.",
  "llm.tools.webSearchHint":            "Lets the model search the web for up-to-date information using DuckDuckGo, Brave, or your self-hosted SearXNG. Search queries are sent to the selected provider. Results are truncated and compressed before being added to context.",
  "llm.tools.webFetchHint":             "Fetches the text content of a URL the model specifies. Useful for reading documentation, articles, or API responses. The request is made from your machine — the target server will see your IP address.",
  "llm.tools.bashHint":                 "Lets the model run shell commands directly on your machine. Output is captured and returned to the conversation. A safety check blocks obviously destructive commands (rm -rf /, etc.), but this is inherently risky — the model has the same permissions as your user account.",
  "llm.tools.readFileHint":             "Allows the model to read any text file on disk that your user account can access. Large files are paginated. Useful for analyzing logs, configs, code, and data files without copy-pasting.",
  "llm.tools.writeFileHint":            "Allows the model to create new files or completely overwrite existing ones. Parent directories are created automatically. The model can write code, configs, scripts, or any text content to disk.",
  "llm.tools.editFileHint":             "Allows the model to make precise find-and-replace edits to existing files without rewriting the entire file. Useful for targeted code changes, config tweaks, and patch-like modifications.",
  "llm.tools.advancedHint":             "Advanced tools are highlighted in amber because they grant the model direct access to your filesystem or shell. While safety checks are in place, these tools carry inherent risk — the model operates with your user permissions. Enable only if you trust the model and understand the implications.",
  ...
  "chat.tools.thinkingBudgetDesc":      "How much the model may think during tool-calling rounds. Lower = faster responses after tool results.",
  ...
  "llm.tools.retrySectionDesc":         "Retry web tools (search, fetch, location) on transient errors with exponential backoff.",
