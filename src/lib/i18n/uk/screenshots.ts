// SPDX-License-Identifier: GPL-3.0-only
// Copyright (C) 2026 NeuroSkill.com
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, version 3 only.
/** UK "screenshots" namespace translations. */
const screenshots: Record<string, string> = {
  "screenshots.title": "Screenshots",
  "screenshots.enableToggle": "Screenshots",
  "screenshots.enableDesc": "Screenshots",
  "screenshots.sessionOnlyToggle": "Screenshots",
  "screenshots.sessionOnlyDesc": "Screenshots",
  "screenshots.interval": "Інтервал захоплення",
  "screenshots.intervalDesc":
    "Вирівняно з епохами вбудовування ЕЕГ (по 5 с). 1× = кожна епоха, 2× = кожна друга, до 12× (60 с).",
  "screenshots.intervalUnit": "с",
  "screenshots.intervalEpoch": "епоха",
  "screenshots.imageSize": "Screenshots",
  "screenshots.imageSizeDesc": "Screenshots",
  "screenshots.imageSizeUnit": "px",
  "screenshots.imageSizeRecommended": "Screenshots",
  "screenshots.quality": "Screenshots",
  "screenshots.qualityDesc": "Screenshots",
  "screenshots.embeddingModel": "Screenshots",
  "screenshots.embeddingModelDesc": "Screenshots",
  "screenshots.backendRlx": "RLX (локально, safetensors)",
  "screenshots.backendMmproj": "mmproj",
  "screenshots.backendLlmVlm": "LLM VLM (embed + OCR via vision model)",
  "screenshots.modelNomic": "Nomic Embed Vision v1.5 — 768d",
  "screenshots.reembed": "Screenshots",
  "screenshots.reembedDesc": "Screenshots",
  "screenshots.reembedBtn": "Screenshots",
  "screenshots.reembedNowBtn": "Screenshots",
  "screenshots.reembedding": "Screenshots",
  "screenshots.stale": "stale",
  "screenshots.unembedded": "не вбудовано",
  "screenshots.estimate": "Screenshots",
  "screenshots.modelChanged": "Screenshots",
  "screenshots.modelChangedDesc": "Screenshots",
  "screenshots.privacyNote": "Screenshots",
  "screenshots.storagePath": "~/.skill/screenshots/",
  "screenshots.stats": "Screenshots",
  "screenshots.totalCount": "Screenshots",
  "screenshots.embeddedCount": "Screenshots",
  "screenshots.unembeddedCount": "Screenshots",
  "screenshots.staleCount": "Screenshots",
  "screenshots.permissionRequired": "Потрібен дозвіл на запис екрана",
  "screenshots.permissionDesc":
    "macOS потребує дозволу на запис екрана та системного аудіо для захоплення вікон інших програм. Без нього знімки можуть бути порожніми або показувати лише вашу програму.",
  "screenshots.permissionGranted": "Дозвіл на запис екрана надано.",
  "screenshots.openPermissionSettings": "Відкрити налаштування запису екрана",
  "screenshots.ocrToggle": "Вилучення тексту OCR",
  "screenshots.ocrToggleDesc":
    "Вилучати текст зі знімків екрана для текстового пошуку. Працює на зображенні повної роздільності перед зменшенням.",
  "screenshots.gpuToggle": "Прискорення GPU",
  "screenshots.gpuToggleDesc":
    "Використовувати GPU для вбудовування зображень та OCR. Вимкніть для примусового використання CPU (звільняє GPU для LLM/EEG).",
  "screenshots.ocrEngineSelect": "Двигун OCR",
  "screenshots.ocrEngineAppleVision": "Apple Vision — GPU / Neural Engine (рекомендовано на macOS)",
  "screenshots.ocrEngineOcrs": "rlx-ocr — локальний CPU / GPU (кросплатформний)",
  "screenshots.ocrAppleVisionHint": "⚡ Apple Vision працює на GPU/ANE і швидший за rlx-ocr на macOS",
  "screenshots.ocrActiveModels": "Активні моделі",
  "screenshots.ocrInference": "Інференс",
  "screenshots.ocrTitle": "Вилучення тексту OCR",
  "screenshots.ocrEngine": "OCR на пристрої",
  "screenshots.ocrDesc":
    "Текст вилучається з кожного знімка екрана у повній роздільності перед зменшенням за допомогою двигуна rlx-ocr. Вилучений текст вбудовується через nomic-embed-text-v1.5 та індексується в окремому індексі HNSW для семантичного текстового пошуку. Моделі OCR постачаються в комплекті та працюють офлайн — завантаження не потрібне.",
  "screenshots.ocrDetModel": "Модель виявлення",
  "screenshots.ocrRecModel": "Модель розпізнавання",
  "screenshots.ocrTextEmbed": "Текстове вбудовування",
  "screenshots.ocrIndex": "Текстовий індекс",
  "screenshots.ocrSearchHint": "Використовуйте вікно Пошуку → вкладка Зображення для пошуку тексту на знімках.",
  "screenshots.ocrSearchTitle": "Пошук за текстом екрана",
  "screenshots.ocrSearchPlaceholder": "Шукати видимий текст на знімках…",
  "screenshots.ocrSearchBtn": "Шукати",
  "screenshots.ocrModeSubstring": "Текстовий збіг",
  "screenshots.ocrModeSemantic": "Семантичний",
  "screenshots.ocrNoResults": "Відповідних знімків екрана не знайдено.",
  "screenshots.perfTitle": "Продуктивність конвеєра",
  "screenshots.perfCapture": "Потік захоплення",
  "screenshots.perfEmbed": "Потік вбудовування",
  "screenshots.perfTotal": "total",
  "screenshots.perfWindowCapture": "Захоплення вікна",
  "screenshots.perfOcr": "Вилучення OCR",
  "screenshots.perfResize": "Зміна розміру + доповнення",
  "screenshots.perfSave": "Збереження + SQLite",
  "screenshots.perfIterTotal": "Загалом ітерація",
  "screenshots.perfVisionEmbed": "Візуальне вбудовування",
  "screenshots.perfTextEmbed": "Текстове вбудовування",
  "screenshots.perfQueue": "Глибина черги",
  "screenshots.perfDrops": "Пропущено",
  "screenshots.perfBackoff": "Затримка",
  "screenshots.perfDropsHint":
    "потік вбудовування надто повільний — інтервал автоматично збільшено, відновиться коли черга спорожніє",
  "screenshots.perfErrors": "помилки",

  "screenshots.userScreenshotToggle": "Відстежувати знімки екрана користувача",
  "screenshots.userScreenshotDesc":
    "Визначає коли ви робите знімок екрана (Cmd+Shift+3/4 на macOS, Print Screen на Windows) та імпортує його для OCR, пошуку та кореляції з EEG. Оригінальні файли ніколи не переміщуються та не видаляються.",
  "screenshots.clipboardImageToggle": "Відстежувати зображення з буфера обміну",
  "screenshots.clipboardImageDesc":
    "Коли зображення копіюється до буфера обміну, воно захоплюється та індексується разом з іншими знімками екрана для пошуку та аналізу.",
};

export default screenshots;
