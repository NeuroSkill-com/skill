...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>{t("llm.section.models")}</SectionHeader>
  ...
  {#if families.length === 0}
    <Card class="border-border dark:border-white/[0.06] bg-surface-1">
      <CardContent class="flex flex-col items-center gap-2 py-8">
        ...
        <p class="text-ui-md text-muted-foreground">{t("llm.noFeature")}</p>
      </CardContent>
    </Card>
  {:else}
      ...
      <div class="flex flex-col gap-1.5">
        {#if activeEntry}
          ...
          <button onclick={() => navigateToEntry(activeEntry)}
            ...
            <span class="text-ui-xs font-semibold text-emerald-600 dark:text-emerald-400 shrink-0">✓ ACTIVE</span>
            <span class="text-ui-base font-semibold text-foreground truncate">{activeEntry.family_name}</span>
            <span class="text-ui-base font-mono text-muted-foreground shrink-0">{activeEntry.quant}</span>
            <span class="text-ui-sm text-muted-foreground/60 shrink-0">{fmtSize(activeEntry.size_gb)}</span>
            {#if !isInCurrentFamily}
              <span class="ml-auto text-ui-2xs text-muted-foreground/40 shrink-0">→</span>
            {/if}
          </button>
        {/if}
          ...
          <button onclick={() => navigateToEntry(dl)}
            class="flex flex-col gap-1 w-full rounded-lg border px-3 py-2 text-left transition-all cursor-pointer
                   {isInCurrentFamily
                     ...
                     : 'border-border/60 dark:border-white/[0.06] bg-surface-1 hover:border-blue-500/30'}">
            <div class="flex items-center gap-2 w-full">
              <span class="text-ui-xs font-semibold text-blue-500 shrink-0 animate-pulse">⬇ DOWNLOADING</span>
              <span class="text-ui-base font-semibold text-foreground truncate">{dl.family_name}</span>
              <span class="text-ui-base font-mono text-muted-foreground shrink-0">{dl.quant}</span>
              <span class="text-ui-sm text-muted-foreground/60 shrink-0">{fmtSize(dl.size_gb)}</span>
              {#if !isInCurrentFamily}
                <span class="ml-auto text-ui-2xs text-muted-foreground/40 shrink-0">→</span>
              {/if}
            </div>
            ...
            {#if dl.status_msg}
              <span class="text-ui-xs text-blue-500/80 truncate">{dl.status_msg}</span>
            {:else if dl.progress > 0}
              <span class="text-ui-xs text-blue-500/80">{(dl.progress * 100).toFixed(1)}%</span>
            {/if}
          </button>
        {/each}
      </div>
    ...
    {#if hardwareFits.size > 0}
      ...
      {#if anyFit}
        <div class="flex items-center gap-2 text-ui-xs text-muted-foreground/60 px-0.5 -mt-0.5 mb-0.5">
      ...
      <Card class="border-border dark:border-white/[0.06] bg-surface-1 gap-0 py-0 overflow-hidden">
        <CardContent class="py-0 px-0 flex flex-col">
          <div class="px-4 pt-3.5 pb-3 flex flex-col gap-1.5">
            <p class="text-ui-base text-muted-foreground leading-snug">{selectedFamily.desc}</p>
            <div class="flex items-center gap-1 flex-wrap">
              {#each selectedFamily.tags.filter((t: string) => !["tiny", "small", "medium", "large"].includes(t)) as tag}
                <Badge variant="outline" class="text-ui-2xs py-0 px-1.5 {tagColor(tag)}">{tagLabel(tag)}</Badge>
              {/each}
              <div class="ml-auto flex items-center gap-1 flex-wrap justify-end">
                {#each selectedFamily.vendors as vendor}
                  <Badge variant="outline" class="text-ui-2xs py-0 px-1.5 border-slate-500/20 bg-slate-500/10 text-slate-600 dark:text-slate-300">{vendor}</Badge>
                {/each}
              </div>
            </div>
            <div class="flex items-center gap-2 flex-wrap text-ui-sm text-muted-foreground/70">
          ...
          <div class="grid grid-cols-[4rem_4rem_1fr_auto] gap-x-2 items-center px-4 py-1.5 border-t border-b border-border/40 dark:border-white/[0.04] bg-surface-3">
            <span class="text-ui-xs font-semibold uppercase tracking-widest text-muted-foreground/60">Quant</span>
            <span class="text-ui-xs font-semibold uppercase tracking-widest text-muted-foreground/60">Size</span>
            <span class="text-ui-xs font-semibold uppercase tracking-widest text-muted-foreground/60">Notes</span>
            <span></span>
          </div>
          ...
          <div class="flex flex-col divide-y divide-border/40 dark:divide-white/[0.05]">
            {#each [...selectedEntryGroups.primary, ...(showAllQuants ? selectedEntryGroups.extra : [])] as entry (entry.filename)}
              ...
              <div class="flex flex-col gap-1 px-4 py-2.5 {isActive ? 'bg-violet-500/10 dark:bg-violet-500/20' : ''} {notDownloaded ? 'opacity-50' : ''}">
                <div class="grid grid-cols-[4rem_4rem_1fr_auto] gap-x-2 items-center min-w-0">
                  <span class="text-ui-md font-bold font-mono text-foreground truncate">
                    {entry.quant}
                    {#if entry.recommended}<span class="text-ui-xs text-violet-600 dark:text-violet-400 font-sans not-italic ml-0.5">★</span>{/if}
                  </span>
                  <span class="text-ui-md tabular-nums font-semibold {downloaded ? 'text-foreground/80' : 'text-muted-foreground'}">{fmtSize(entry.size_gb)}</span>
                  <div class="flex items-center gap-1.5 min-w-0">
                    {#if selectedFamilyHasMultipleVendors}
                      <span class="shrink-0 rounded-full border border-slate-500/20 bg-slate-500/10 px-1.5 py-0.5 text-ui-2xs font-semibold text-slate-600 dark:text-slate-300">{vendorLabel(entry.repo)}</span>
                    {/if}
                    {#if fit}
                      <span class="shrink-0 rounded-full border px-1.5 py-0.5 text-ui-2xs font-semibold {fitBadgeClass(fit.fitLevel)}"
                    ...
                    {/if}
                    <span class="text-ui-sm text-muted-foreground/70 truncate">{entry.description}</span>
                    {#if isActive}
                      <span class="shrink-0 text-ui-xs font-semibold text-emerald-600 dark:text-emerald-400">✓ active</span>
                    {:else if downloaded}
                      <span class="shrink-0 text-ui-xs font-semibold text-sky-600 dark:text-sky-400">downloaded</span>
                    {/if}
                    {#if downloading}<span class="shrink-0 text-ui-xs text-blue-500 animate-pulse">downloading…</span>{/if}
                    {#if failed}<span class="shrink-0 text-ui-xs text-red-500">failed</span>{/if}
                  </div>

                  <div class="flex items-center gap-1 shrink-0 justify-end">
                    {#if downloading}
                      <Button size="sm" variant="outline" class="h-6 text-ui-sm px-2 text-destructive border-destructive/30 hover:bg-destructive/10" onclick={() => onCancelDownload(entry.filename)}>Cancel</Button>
                    {:else if downloaded}
                      <Button size="sm" variant="ghost" class="h-6 text-ui-sm px-2 text-muted-foreground/60 hover:text-red-500" onclick={() => onDeleteModel(entry.filename)}>Delete</Button>
                      <Button size="sm" class="h-6 text-ui-sm px-2.5 {isActive ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/20' : 'bg-violet-600 hover:bg-violet-700 text-white'}" onclick={() => onSelectModel(entry.filename)}>{isActive ? "Active" : "Use"}</Button>
                    {:else}
                      <Button size="sm" class="h-6 text-ui-sm px-2.5 bg-violet-600 hover:bg-violet-700 text-white" onclick={() => onDownload(entry.filename)}>
                        {failed ? "Retry" : `Download ${fmtSize(entry.size_gb)}`}{entry.shard_files?.length > 1 ? ` (${entry.shard_files.length} parts)` : ""}
                      </Button>
                      <Button size="sm" variant="outline" class="h-6 text-ui-sm px-2 text-muted-foreground border-border/50 hover:text-foreground" onclick={() => onSelectModel(entry.filename)}>Use</Button>
                    {/if}
                  </div>
                </div>
                ...
                {#if downloading}
                  ...
                  {#if entry.status_msg}<p class="text-ui-sm text-blue-500 truncate">{entry.status_msg}</p>{/if}
                {/if}

                <p class="text-ui-2xs text-muted-foreground/70 font-mono break-all">🤗 hf download {entry.repo} {entry.filename}</p>

                {#if failed && entry.status_msg}
                  <p class="text-ui-sm text-destructive/80 font-mono break-all leading-relaxed rounded bg-destructive/5 border border-destructive/10 px-2 py-1">{entry.status_msg}</p>
                {/if}

                {#if downloaded && entry.local_path}
                  <p class="text-ui-xs font-mono text-muted-foreground/40 break-all leading-tight">{entry.local_path}</p>
                {/if}

                {#if fit}
                  <div class="flex items-center gap-2 flex-wrap text-ui-xs text-muted-foreground/60 mt-0.5">
          ...
          {#if selectedFamily.mmproj.length > 0}
            <div class="border-t border-border dark:border-white/[0.06] px-4 py-3 bg-amber-50/30 dark:bg-amber-950/10">
              <p class="text-ui-sm font-semibold text-amber-700 dark:text-amber-400 mb-2">Vision projector (required for image input)</p>
              <p class="text-ui-sm text-amber-700/80 dark:text-amber-300/80 mb-2 leading-snug">Multimodal projectors extend the active LLM. They are loaded with a compatible text model, not used as standalone models.</p>
              <div class="flex flex-col gap-1.5">
                  ...
                  <div class="flex flex-col gap-1">
                    <div class="flex items-center gap-2">
                      <div class="flex-1 min-w-0 flex items-center gap-1.5">
                        {#if selectedFamilyHasMultipleVendors}
                          <span class="shrink-0 rounded-full border border-slate-500/20 bg-slate-500/10 px-1.5 py-0.5 text-ui-2xs font-semibold text-slate-600 dark:text-slate-300">{vendorLabel(mp.repo)}</span>
                        {/if}
                        <span class="text-ui-base font-mono text-foreground truncate">{mp.filename}</span>
                        <span class="text-ui-sm text-muted-foreground shrink-0">{fmtSize(mp.size_gb)}</span>
                        {#if mp.recommended}<span class="text-ui-xs text-violet-600 dark:text-violet-400">★</span>{/if}
                        {#if isActiveMm}<span class="text-ui-xs font-semibold text-amber-600 dark:text-amber-400 shrink-0">✓ active</span>{/if}
                      </div>
                      <div class="flex items-center gap-1 shrink-0">
                        {#if mpDl}
                          <Button size="sm" variant="outline" class="h-5 text-ui-sm px-1.5 text-destructive border-destructive/30" onclick={() => onCancelDownload(mp.filename)}>Cancel</Button>
                        {:else if mpDownloaded}
                          <Button size="sm" variant="ghost" class="h-5 text-ui-sm px-1.5 text-muted-foreground/60 hover:text-red-500" onclick={() => onDeleteModel(mp.filename)}>Delete</Button>
                          <Button size="sm" class="h-5 text-ui-sm px-2 {isActiveMm ? 'bg-amber-500/15 text-amber-700 dark:text-amber-400 border border-amber-500/30' : 'bg-amber-600 hover:bg-amber-700 text-white'}" onclick={() => onSelectMmproj(mp.filename)}>{isActiveMm ? "Active" : "Use"}</Button>
                        {:else}
                          <Button size="sm" class="h-5 text-ui-sm px-2 bg-amber-600 hover:bg-amber-700 text-white" onclick={() => onDownload(mp.filename)}>Download {fmtSize(mp.size_gb)}</Button>
                        {/if}
                      </div>
                    </div>
                    ...
                    {#if mpDl}
                      ...
                      {#if mp.status_msg}<p class="text-ui-xs text-amber-600 truncate">{mp.status_msg}</p>{/if}
                    {/if}
                    <p class="text-ui-2xs text-muted-foreground/70 font-mono break-all">🤗 hf download {mp.repo} {mp.filename}</p>
                  </div>
                {/each}
              </div>
            </div>
