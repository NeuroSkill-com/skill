...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <span class="text-[0.56rem] font-semibold tracking-widest uppercase text-muted-foreground">{t("llm.tools.section")}</span>
    <span class="text-[0.52rem] text-muted-foreground/50">{config.tools.enabled ? toolRows.filter((r) => config.tools[r.key]).length + "/" + toolRows.length : "off"}</span>
  ...
  <Card class="border-border dark:border-white/[0.06] bg-white dark:bg-[#14141e] gap-0 py-0 overflow-hidden">
    <CardContent class="flex flex-col py-0 px-0">
      <div class="flex items-center justify-between gap-4 px-4 pt-3.5 pb-2">
        <p class="text-[0.65rem] text-muted-foreground leading-relaxed">{t("llm.tools.sectionDesc")}</p>
        <button role="switch" aria-checked={config.tools.enabled} aria-label={t("llm.tools.enableAll")}
          ...
          class="relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 {config.tools.enabled ? 'bg-blue-500' : 'bg-muted dark:bg-white/10'}">
      ...
      <div class="flex flex-col gap-2 px-4 pb-3 {config.tools.enabled ? '' : 'opacity-40 pointer-events-none'}">
        {#each toolRows as tool}
          ...
          <div class="relative rounded-xl border {tool.warn && config.tools[tool.key] ? 'border-amber-500/40 bg-amber-50/40 dark:bg-amber-950/15' : 'border-border/60 dark:border-white/[0.06] bg-slate-50/60 dark:bg-[#111118]'}"
            ...
            <div class="flex items-center justify-between gap-4 px-3 py-2.5">
              <div class="flex flex-col gap-0.5">
                <div class="flex items-center gap-1.5">
                  ...
                  {#if tool.warn}
                    <span class="text-[0.5rem] font-semibold rounded-full border px-1.5 py-0 border-amber-500/30 bg-amber-500/10 text-amber-600 dark:text-amber-400">{t("llm.tools.advanced")}</span>
                  {/if}
                </div>
              ...
              <button role="switch" aria-checked={config.tools[tool.key]} aria-label={tool.label}
                ...
                class="relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 {config.tools[tool.key] ? (tool.warn ? 'bg-amber-500' : 'bg-blue-500') : 'bg-muted dark:bg-white/10'}">
            ...
            {#if hoveredTool === tool.key}
              <div class="px-3 pb-2.5 flex flex-col gap-1.5 animate-in fade-in duration-150">
                ...
                {#if tool.warn}<p class="text-[0.54rem] leading-relaxed text-amber-600/80 dark:text-amber-400/70 italic">{t("llm.tools.advancedHint")}</p>{/if}
              </div>
            {/if}
          </div>
        {/each}
        ...
        {#if config.tools.web_search}
          <div class="flex flex-col gap-2.5 rounded-xl border border-border/60 dark:border-white/[0.06] bg-slate-50/60 dark:bg-[#111118] px-3 py-2.5">
            ...
            <div class="flex rounded-lg overflow-hidden border border-border text-[0.66rem] font-medium">
              {#each [
                ...
                <button onclick={async () => await onConfigChange({ ...config, tools: { ...config.tools, web_search_provider: { ...config.tools.web_search_provider, backend: opt.key } } })}
                  class="flex-1 py-1.5 transition-colors cursor-pointer {config.tools.web_search_provider.backend === opt.key ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:bg-muted'}">{opt.label}</button>
              {/each}
            </div>

            {#if config.tools.web_search_provider.backend === "brave"}
              <div class="flex flex-col gap-1">
                <label for="brave-api-key" class="text-[0.64rem] font-semibold text-foreground">{t("llm.tools.braveApiKey")}</label>
                ...
                <input id="brave-api-key" type="password" autocomplete="off" placeholder="BSA..."
                  ...
                  class="mt-0.5 w-full rounded-lg border border-border/60 dark:border-white/[0.08] bg-white dark:bg-[#0c0c14] px-2.5 py-1.5 text-[0.7rem] text-foreground placeholder:text-muted-foreground/50 outline-none focus:ring-1 focus:ring-blue-500/50" />
              </div>
            {/if}

            {#if config.tools.web_search_provider.backend === "searxng"}
              <div class="flex flex-col gap-1">
                <label for="searxng-url" class="text-[0.64rem] font-semibold text-foreground">{t("llm.tools.searxngUrl")}</label>
          ...
          <div class="flex flex-col gap-2.5 rounded-xl border border-border/60 dark:border-white/[0.06] bg-slate-50/60 dark:bg-[#111118] px-3 py-2.5">
            <div class="flex items-center justify-between gap-2">
              ...
              <button role="switch" aria-checked={config.tools.web_cache?.enabled ?? true} aria-label={t("llm.tools.webCache")}
                ...
                class="relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 {(config.tools.web_cache?.enabled ?? true) ? 'bg-blue-500' : 'bg-muted dark:bg-white/10'}">
            ...
            {#if config.tools.web_cache?.enabled ?? true}
              ...
              <div class="flex flex-col gap-2 pt-1">
                ...
                {#if cacheEntries.length > 0}
                  <div class="flex flex-col gap-1 max-h-44 overflow-y-auto rounded-lg border border-border/40 dark:border-white/[0.04] bg-white dark:bg-[#0c0c14] p-1.5">
                    {#each cacheEntries as entry}
                      <div class="flex items-center justify-between gap-2 rounded-md px-2 py-1 hover:bg-muted/50 dark:hover:bg-white/[0.03] group">
                        <div class="flex flex-col gap-0 min-w-0">
                          <div class="flex items-center gap-1.5">
                            <span class="text-[0.48rem] font-semibold rounded-full border px-1 py-0 {entry.kind === 'search' ? 'border-blue-500/30 bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400'}">
                        ...
                        <div class="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                          <button onclick={() => onRemoveDomain(entry.domain)} title={t("llm.tools.webCacheRemoveDomain")} class="rounded border border-border px-1 py-0.5 text-[0.48rem] text-muted-foreground hover:text-foreground hover:bg-muted transition-colors cursor-pointer bg-background">{entry.domain} ✕</button>
                          <button onclick={() => onRemoveEntry(entry.key)} title={t("llm.tools.webCacheRemoveEntry")} class="rounded border border-red-500/30 px-1 py-0.5 text-[0.48rem] text-red-500 hover:bg-red-500/10 transition-colors cursor-pointer bg-background">✕</button>
                        </div>
                      </div>
                    {/each}
                  </div>
      ...
      <div class="flex flex-col gap-3 px-4 py-3 border-t border-border/40 dark:border-white/[0.04] bg-slate-50 dark:bg-[#111118] {config.tools.enabled ? '' : 'opacity-40 pointer-events-none'}">
          ...
          <div class="flex rounded-lg overflow-hidden border border-border text-[0.68rem] font-medium">
            {#each [{ key: "parallel" as ToolExecutionMode, label: t("llm.tools.parallel") }, { key: "sequential" as ToolExecutionMode, label: t("llm.tools.sequential") }] as mode}
              <button onclick={async () => await onConfigChange({ ...config, tools: { ...config.tools, execution_mode: mode.key } })}
                class="flex-1 py-1.5 transition-colors cursor-pointer {config.tools.execution_mode === mode.key ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:bg-muted'}">{mode.label}</button>
            {/each}
          </div>
        </div>
          ...
          <div class="flex items-center gap-1">
            {#each [1, 3, 5, 10, 15] as val}
              <button onclick={async () => await onConfigChange({ ...config, tools: { ...config.tools, max_rounds: val } })}
                class="rounded-lg border px-2 py-1 text-[0.64rem] font-semibold transition-all cursor-pointer {config.tools.max_rounds === val ? 'border-violet-500/50 bg-violet-500/10 text-violet-600 dark:text-violet-400' : 'border-border bg-background text-muted-foreground hover:text-foreground'}">{val}</button>
            {/each}
          </div>
        </div>
          ...
          <div class="flex items-center gap-1">
            {#each [1, 2, 4, 8] as val}
              <button onclick={async () => await onConfigChange({ ...config, tools: { ...config.tools, max_calls_per_round: val } })}
                class="rounded-lg border px-2 py-1 text-[0.64rem] font-semibold transition-all cursor-pointer {config.tools.max_calls_per_round === val ? 'border-violet-500/50 bg-violet-500/10 text-violet-600 dark:text-violet-400' : 'border-border bg-background text-muted-foreground hover:text-foreground'}">{val}</button>
            {/each}
          </div>
        </div>
        ...
        <div class="flex flex-col gap-2.5 pt-1">
          ...
          <div class="flex rounded-lg overflow-hidden border border-border text-[0.66rem] font-medium">
            {#each [
              ...
              <button onclick={async () => await onConfigChange({ ...config, tools: { ...config.tools, context_compression: { ...config.tools.context_compression, level: opt.key } } })}
                class="flex-1 py-1.5 transition-colors cursor-pointer {config.tools.context_compression.level === opt.key ? 'bg-primary text-primary-foreground' : 'bg-background text-muted-foreground hover:bg-muted'}">{opt.label}</button>
            {/each}
          </div>
          ...
          {#if config.tools.context_compression.level !== "off"}
            <div class="flex gap-3">
              <div class="flex-1 flex flex-col gap-1">
                <label for="comp-max-results" class="text-[0.62rem] text-muted-foreground">{t("llm.tools.maxSearchResults")}</label>
                <input id="comp-max-results" type="number" min="0" max="20" step="1"
                  ...
                  class="w-full rounded-lg border border-border/60 dark:border-white/[0.08] bg-white dark:bg-[#0c0c14] px-2.5 py-1.5 text-[0.7rem] text-foreground placeholder:text-muted-foreground/50 outline-none focus:ring-1 focus:ring-blue-500/50" />
                <span class="text-[0.54rem] text-muted-foreground/60">{t("llm.tools.zeroAutoLabel")}</span>
              </div>
              <div class="flex-1 flex flex-col gap-1">
                <label for="comp-max-chars" class="text-[0.62rem] text-muted-foreground">{t("llm.tools.maxResultChars")}</label>
        ...
        <div class="flex flex-col gap-3 pt-2">
          ...
          <div class="flex gap-3">
            <div class="flex-1 flex flex-col gap-1">
              <label for="retry-max" class="text-[0.62rem] text-muted-foreground">{t("llm.tools.retryMaxRetries")}</label>
              <div class="flex items-center gap-1">
                {#each [0, 1, 2, 3] as val}
                  <button onclick={async () => await onConfigChange({ ...config, tools: { ...config.tools, retry: { ...config.tools.retry, max_retries: val } } })}
                    class="rounded-lg border px-2 py-1 text-[0.64rem] font-semibold transition-all cursor-pointer {config.tools.retry.max_retries === val ? 'border-violet-500/50 bg-violet-500/10 text-violet-600 dark:text-violet-400' : 'border-border bg-background text-muted-foreground hover:text-foreground'}">{val}</button>
                {/each}
              </div>
              <span class="text-[0.54rem] text-muted-foreground/60">{t("llm.tools.retryMaxRetriesDesc")}</span>
            </div>
            <div class="flex-1 flex flex-col gap-1">
              <label for="retry-delay" class="text-[0.62rem] text-muted-foreground">{t("llm.tools.retryBaseDelay")}</label>
              <div class="flex items-center gap-1">
