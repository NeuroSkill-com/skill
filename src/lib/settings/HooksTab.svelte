...
<SettingsCard>
  <CardContent class="space-y-4 p-4">
    ...
    <div class="rounded-md border border-border/60 bg-card/40 p-2 space-y-1.5">
      <div class="text-ui-base font-medium text-muted-foreground">{t("hooks.examplesTitle")}</div>
      <div class="flex flex-wrap gap-1.5">
        {#each HOOK_EXAMPLES as ex}
          <button
            class="inline-flex items-center gap-1 rounded-md border border-border/60 px-2 py-1 text-ui-base text-foreground hover:bg-muted"
        ...
        {#each hooks as hook, i}
          <div class="rounded-lg border border-border/60 bg-card/50 p-3 space-y-3">
            <div class="grid grid-cols-12 gap-2 items-center">
              ...
              <Button class="col-span-2 min-w-0 w-full h-auto whitespace-normal break-words py-1.5 text-ui-base leading-tight" variant="outline" size="sm" onclick={() => removeHook(i)}>
                {t("hooks.remove")}
              </Button>
            </div>
            ...
            {#if statuses[hook.name]}
              <div class="grid grid-cols-12 gap-2 items-center rounded-md border border-border/50 bg-muted/30 p-2">
                <div class="col-span-9 text-ui-base text-muted-foreground">
            ...
            <div class="grid grid-cols-12 gap-2 items-center">
              ...
              />
              <Button class="col-span-3 min-w-0 w-full h-auto whitespace-normal break-words py-1.5 text-ui-base leading-tight" variant="outline" size="sm" onclick={() => addKeyword(i)}>
                {t("hooks.addKeyword")}
              </Button>
            </div>
            ...
            {#if (keywordDrafts[i] ?? "").trim().length >= 2}
              <div class="rounded-md border border-border/60 bg-card/60 p-2 space-y-1">
                <div class="text-ui-sm font-medium text-muted-foreground">{t("hooks.keywordSuggestions")}</div>
                <div class="text-ui-sm text-muted-foreground">{t("hooks.keywordSuggestionKeyboardHint")}</div>
                {#if keywordSuggesting[i]}
                  <div class="text-ui-base text-muted-foreground">{t("hooks.keywordSuggestionsLoading")}</div>
                {:else if (keywordSuggestions[i]?.length ?? 0) === 0}
                  <div class="text-ui-base text-muted-foreground">{t("hooks.keywordSuggestionEmpty")}</div>
                {:else}
                  <div class="flex flex-wrap gap-1.5">
                    {#each keywordSuggestions[i] ?? [] as s, sIdx}
                      <button
                        class="inline-flex items-center gap-1 rounded-md border px-2 py-1 text-ui-base text-foreground hover:bg-muted {sIdx === (keywordSuggestionFocus[i] ?? -1) ? 'border-primary/70 bg-violet-500/10' : 'border-border/60'}"
            ...
            {#if suggestions[i]}
              ...
              <div class="rounded-md border border-violet-500/30 bg-violet-500/5 p-3 space-y-2 text-ui-base">
                <div class="flex items-center justify-between gap-2">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <Button size="sm" class="h-6 px-2 text-ui-base" onclick={() => applySuggestion(i)}>
                ...
                {#if sug.eeg_max > 0}
                  <div class="space-y-1">
                    ...
                    <div class="flex justify-between text-ui-sm text-muted-foreground">
    ...
    <div class="border-t border-border/40 pt-3">
      ...
      >
        ...
        {t("hooks.history")}
        {#if logTotal > 0}
          <span class="ml-1 rounded-full bg-muted px-1.5 py-0.5 text-ui-sm">{logTotal}</span>
        {/if}
      </button>
            ...
            <div class="space-y-1.5">
              {#each logRows as row}
                ...
                <details class="group rounded-md border border-border/50 bg-card/40">
                  <summary class="flex cursor-pointer items-center gap-2 p-2 text-ui-base">
                  ...
                  <div class="border-t border-border/40 p-2 space-y-1 text-ui-base text-muted-foreground">
