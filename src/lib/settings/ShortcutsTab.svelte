...
<section class="flex flex-col gap-2" onkeydown={onRecordKeydown}>
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>{t("settings.globalShortcuts")}</SectionHeader>
    <span class="ml-auto text-ui-xs text-muted-foreground/60">{t("settings.hotkeys")}</span>
  </div>
    ...
    <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
      ...
      {#each ([
        ...
        <div class="flex items-center gap-3 px-4 py-2.5">
          <span class="text-ui-md font-semibold text-foreground w-[120px] shrink-0">{label}</span>

          <div class="flex-1 flex items-center gap-1.5 min-w-0">
            {#if recording && recordingTarget === target}
              <span class="animate-pulse text-ui-base text-muted-foreground italic">
            ...
            {:else if value}
              {#each shortcutTokens(value) as token}
                <span class="inline-flex items-center justify-center px-1.5 py-0.5 rounded
                             text-ui-base font-semibold leading-none
                             bg-muted dark:bg-white/[0.08]
                             border border-border dark:border-white/[0.10]
                             text-foreground shadow-[inset_0_-1px_0_0] shadow-border">
                  {token}
                </span>
              {/each}
            {:else}
              <span class="text-ui-base text-muted-foreground/50 italic">{t("settings.notSet")}</span>
            {/if}
          </div>

          {#if shortcutError && recordingTarget === target}
            <span class="text-ui-sm text-destructive truncate max-w-[100px]" title={shortcutError}>
              {shortcutError}
            </span>
          {/if}
          ...
          <div class="flex gap-1 shrink-0">
            {#if value && !(recording && recordingTarget === target)}
              <Button variant="outline" size="sm"
                      class="text-ui-base h-6 px-2 text-muted-foreground"
...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>{t("shortcutsTab.commandPalette")}</SectionHeader>
  </div>
    ...
    <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
      <div class="flex items-center gap-3 px-4 py-3">
        <div class="flex flex-col gap-1 flex-1">
          <span class="text-ui-lg font-semibold text-foreground">{t("shortcutsTab.cmdKTitle")}</span>
          <span class="text-ui-base text-muted-foreground leading-relaxed">
...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>{t("shortcutsTab.inAppShortcuts")}</SectionHeader>
  </div>
  ...
  <SettingsCard>
    <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
      ...
      {#each ([
        ...
        <div class="flex items-center gap-3 px-4 py-2.5">
          <span class="text-ui-md text-foreground/80 flex-1">{label}</span>
          <kbd class="inline-flex items-center gap-0.5 rounded-md border border-border
                      dark:border-white/[0.1] bg-muted dark:bg-white/[0.04]
                      px-2 py-0.5 font-mono text-ui-base font-medium
                      text-muted-foreground whitespace-nowrap shrink-0">
