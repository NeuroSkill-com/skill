...
<div class="flex flex-col gap-4 px-1">
  ...
  <div class="flex flex-col gap-1">
    <h2 class="text-ui-md font-bold tracking-tight text-foreground">{t("tokens.title")}</h2>
    <p class="text-ui-sm text-muted-foreground leading-relaxed">{t("tokens.desc")}</p>
  </div>
  ...
  <SettingsCard>
    <CardContent class="flex flex-col gap-2 py-3">
      ...
      <p class="text-ui-2xs text-muted-foreground">{t("tokens.defaultTokenDesc")}</p>
      {#if defaultTokenRevealed && defaultToken}
        <div class="flex flex-col gap-1">
          <span class="text-ui-2xs font-semibold text-amber-600 dark:text-amber-400">{t("tokens.copyWarning")}</span>
          <div class="flex items-center gap-2">
            <code class="flex-1 text-ui-sm font-mono bg-black/5 dark:bg-white/5 px-2 py-1 rounded select-all break-all">{defaultToken}</code>
            <Button variant="outline" size="sm" class="h-6 text-ui-xs shrink-0"
  ...
  <SettingsCard>
    <CardContent class="flex flex-col gap-3 py-3">
      <SectionHeader>{t("tokens.create")}</SectionHeader>

      <input type="text" bind:value={newName} aria-label={t("tokens.namePlaceholder")} placeholder={t("tokens.namePlaceholder")}
        class="h-7 rounded-md border border-border bg-background px-2 text-ui-base
               text-foreground placeholder:text-muted-foreground/40
               focus:outline-none focus:ring-1 focus:ring-ring" />

      <div class="flex flex-col gap-1">
        <span class="text-ui-xs text-muted-foreground">{t("tokens.acl")}</span>
        <div class="flex gap-1.5 flex-wrap">
          {#each ACL_OPTIONS as opt}
            <button
              ...
              <span class="text-ui-sm font-medium">{t(opt.label)}</span>
              <span class="text-ui-2xs text-muted-foreground/60">{t(opt.desc)}</span>
            </button>
          {/each}
        </div>
      </div>
      ...
      <div class="flex items-center gap-2">
        <span class="text-ui-xs text-muted-foreground">{t("tokens.expiry")}:</span>
        {#each EXPIRY_OPTIONS as opt}
          <button
            class="h-6 px-2 rounded-md text-ui-xs font-medium cursor-pointer transition-colors
                   {newExpiry === opt.key
      ...
      <Button size="sm" class="h-7 text-ui-sm self-start" disabled={!newName.trim() || creating}
        onclick={createToken}>
  ...
  {#if justCreated}
    <Card class="border-green-500/30 bg-green-500/5 gap-0 py-0 overflow-hidden">
      <CardContent class="flex flex-col gap-2 py-3">
        <span class="text-ui-sm font-semibold text-green-700 dark:text-green-400">{t("tokens.copyWarning")}</span>
        <div class="flex items-center gap-2">
          <code class="flex-1 text-ui-sm font-mono bg-black/5 dark:bg-white/5 px-2 py-1 rounded select-all break-all">
            {justCreated.token}
          </code>
  ...
  <!-- Token list -->
  {#if loading}
    <p class="text-ui-sm text-muted-foreground text-center py-6">Loading…</p>
  {:else if tokens.length === 0}
    <p class="text-ui-sm text-muted-foreground text-center py-6">{t("tokens.empty")}</p>
  {:else}
        ...
        <SettingsCard>
          <CardContent class="flex items-start gap-3 py-3">
            <div class="flex flex-col gap-1 flex-1 min-w-0">
              ...
              <div class="flex items-center gap-3 text-ui-2xs text-muted-foreground/60">
              ...
              <code class="text-ui-2xs font-mono text-muted-foreground/40">{tok.token_preview || tok.token || "stored_as_hash"}</code>
            </div>
            <div class="flex items-center gap-1 shrink-0">
              {#if !tok.revoked && tok.id !== "default"}
                <Button variant="ghost" size="sm" class="h-6 text-ui-xs px-2 text-amber-600 hover:text-amber-700"
                  onclick={() => revokeToken(tok.id)}>
                  {t("tokens.revoke")}
                </Button>
              {/if}
              {#if tok.id !== "default"}
                <Button variant="ghost" size="sm" class="h-6 text-ui-xs px-2 text-red-600 hover:text-red-700"
