...
{#if editingClient}
  ...
  >
    <div
      class="bg-surface-1 rounded-xl border border-border dark:border-white/[0.06] shadow-2xl w-full max-w-lg mx-4 mb-8"
    >
      ...
      <div class="p-4 flex flex-col gap-4 max-h-[70vh] overflow-y-auto">
        ...
        {#if editScope === "custom"}
          <div>
            <SectionHeader>Command Groups</SectionHeader>
          ...
          {#if dangerousGroups.length > 0}
            <div>
              <div class="flex items-center gap-2 mb-1.5">
                ...
                <span
                  class="text-ui-xs font-semibold tracking-widest uppercase text-red-600 dark:text-red-400"
                >
                  Dangerous
                </span>
              </div>
              {#if editHasDangerous}
                ...
                >
                  <p class="text-ui-xs text-red-600 dark:text-red-400 leading-relaxed">
        ...
        {#if editScope === "read"}
          <div class="text-ui-sm text-muted-foreground bg-muted/40 rounded-lg p-3 leading-relaxed">
      ...
      <div class="p-4 border-t border-border dark:border-white/[0.06] flex items-center justify-between">
        <Button
          variant="outline"
          size="sm"
  ...
  <SettingsCard>
    <CardContent class="p-4">
      ...
      {:else}
        ...
        <p class="mt-2 text-ui-xs text-muted-foreground font-mono break-all leading-relaxed">
...
{#if online}
  <!-- ── Pair a Device ──────────────────────────────────────────────────── -->
  <section class="flex flex-col gap-2">
    <SectionHeader>Pair a Device</SectionHeader>
    <SettingsCard>
      <CardContent class="p-4 flex flex-col gap-3">
        ...
        {:else if qr}
          <div class="flex flex-col items-center gap-3">
            ...
            <p class="text-ui-sm text-muted-foreground text-center max-w-56">
              ...
              <div class="flex flex-col items-center gap-1.5 w-full max-w-64">
                <button
                  ...
                  class="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-border dark:border-white/[0.1]
                         text-ui-sm text-muted-foreground hover:text-foreground hover:bg-muted
                ...
                <p class="text-ui-2xs text-muted-foreground/60 text-center leading-tight">
                  {t("invite.linkHint")}
                </p>
              </div>
            {/if}
          ...
          <div>
            ...
            <div class="flex gap-2 mt-1.5">
              {#each [["read", "Read Only"], ["custom", "Custom"], ["full", "Full Access"]] as [val, label]}
                <button
                  class="px-3 py-1.5 text-ui-sm rounded-md border transition-colors cursor-pointer
                         {inviteScope === val
                    ? val === 'full'
                      ? 'bg-red-600 text-white border-red-600'
                      : 'bg-violet-600 text-white border-violet-600'
  ...
  {#if devices.length > 0}
    <section class="flex flex-col gap-2">
      <SectionHeader>Paired Devices ({devices.length})</SectionHeader>
      <SettingsCard>
            ...
            <div class="flex items-start justify-between gap-3 px-4 py-3">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-1.5 flex-wrap">
                  <span class="text-ui-md font-semibold truncate">{d.name}</span>
                  <Badge
                  ...
                  {#if d.last_connected_at}
                    <span class="text-ui-2xs text-muted-foreground">{ago(d.last_connected_at)}</span>
                  {/if}
                </div>
                {#if phoneInfo && d.endpoint_id === phoneInfo.iroh_endpoint_id && (phoneInfo.phone_marketing_name || phoneInfo.phone_model)}
                  <div class="text-ui-xs text-muted-foreground mt-0.5 truncate">
                ...
                {:else}
                  <div class="text-ui-xs text-muted-foreground mt-0.5 font-mono truncate">
                    {d.endpoint_id}
                  </div>
                {/if}
                {#if d.last_ip || d.last_country || d.last_city}
                  <div class="text-ui-2xs text-muted-foreground/60 mt-0.5">
              ...
              <div class="flex gap-1 shrink-0 pt-0.5">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  class="text-ui-xs h-6 px-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"
  ...
  {#if revokedCount > 0}
    <section class="flex flex-col gap-2">
      <details class="group">
        <summary
          class="text-ui-xs font-semibold tracking-widest uppercase text-muted-foreground px-0.5 cursor-pointer select-none
        ...
        >
          <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
            {#each revokedClients as c}
              <div class="px-4 py-2">
                <div class="flex items-center gap-1.5">
                  <span class="text-ui-sm line-through">{c.name}</span>
                  {#if c.device_model}
                    <span class="text-ui-2xs text-muted-foreground/60">{c.device_model}</span>
                  {/if}
                  <span class="text-ui-2xs text-muted-foreground">revoked {fmt(c.revoked_at)}</span>
                </div>
              </div>
            {/each}
          </CardContent>
