...
<section class="flex flex-col gap-5">
  ...
  <div class="flex flex-col gap-2">
    ...
    {#if activeModel}
      <div class="rounded-xl border border-border dark:border-white/[0.06]
                  bg-surface-1 px-4 py-3 flex items-start gap-3">
        <div class="flex flex-col gap-1 flex-1 min-w-0">
          <div class="flex items-center gap-2 flex-wrap">
            <span class="text-ui-md font-semibold text-foreground truncate">
              {activeModel.code}
            </span>
            <span class="rounded border px-1.5 py-0 text-ui-xs font-semibold
                         {dimColor(activeModel.dim)}">
              {activeModel.dim}d
            </span>
          </div>
          <p class="text-ui-base text-muted-foreground/70 leading-snug">
  ...
  <!-- ── Info callout ────────────────────────────────────────────────────── -->
    ...
    <div class="flex flex-col gap-1">
      <p class="text-ui-base text-violet-600 dark:text-violet-400 leading-relaxed">
        {t("embeddings.info")}
      </p>
      <p class="text-ui-sm text-violet-500/70 leading-relaxed">
    ...
    <div class="flex items-center justify-between">
      <div class="flex flex-col gap-0.5">
        <div class="flex items-center gap-2">
          ...
          {#if staleCount > 0}
            <span class="rounded-full px-1.5 py-0 text-ui-xs font-semibold
  ...
  <div class="flex items-center gap-3 pt-1">
    <span class="text-ui-xs font-semibold uppercase tracking-wider text-muted-foreground/50 shrink-0">
    ...
    {/each}
    <span class="text-ui-xs text-muted-foreground/40">{t("embeddings.dimHint")}</span>
  </div>
  ...
  <div class="flex flex-col gap-3">
    ...
    <p class="text-ui-sm text-muted-foreground/60 leading-relaxed -mt-1">
    ...
    <div class="flex flex-col gap-2">
      <label class="flex items-center justify-between gap-2 cursor-pointer">
        <span class="text-ui-base text-foreground">{t("embeddings.autoReembed.labels")}</span>
        <input type="checkbox" bind:checked={reembedCfg.auto_labels} onchange={saveReembedConfig}
               class="w-8 h-4 rounded-full appearance-none bg-muted dark:bg-white/[0.08]
                      checked:bg-violet-500 relative cursor-pointer transition-colors
                      after:content-[''] after:absolute after:top-0.5 after:left-0.5
                      after:w-3 after:h-3 after:rounded-full after:bg-white after:transition-transform
                      checked:after:translate-x-4" />
      </label>
      <label class="flex items-center justify-between gap-2 cursor-pointer">
        <span class="text-ui-base text-foreground">{t("embeddings.autoReembed.eeg")}</span>
      ...
      <label class="flex items-center justify-between gap-2 cursor-pointer">
        <span class="text-ui-base text-foreground">{t("embeddings.autoReembed.screenshots")}</span>
    ...
    <div class="grid grid-cols-2 gap-3 mt-1">
      <div class="flex flex-col gap-1">
        <label for="batch-size" class="text-ui-sm text-muted-foreground/60">
      ...
      <div class="flex flex-col gap-1">
        <label for="batch-delay" class="text-ui-sm text-muted-foreground/60">
  ...
  <div class="flex flex-col gap-3">
    ...
    <label class="flex items-center justify-between gap-2 cursor-pointer">
      <span class="text-ui-base text-foreground">{t("embeddings.watchdog.enabled")}</span>
    ...
    {#if watchdogCfg.enabled}
      <div class="flex flex-col gap-1">
        <label for="watchdog-timeout" class="text-ui-sm text-muted-foreground/60">
