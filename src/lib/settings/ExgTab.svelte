...
import EegModelTab from "$lib/settings/EegModelTab.svelte";
...
<section class="flex flex-col gap-4">
  ...
  <div class="flex flex-col gap-2">
    <div class="flex items-center gap-2 px-0.5">
      <SectionHeader>{t("settings.signalProcessing")}</SectionHeader>
      {#if filterSaving}
    ...
    <SettingsCard>
      <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
        ...
        <div class="flex flex-col gap-2.5 px-4 py-4">
          <span class="text-ui-lg font-semibold text-foreground">{t("settings.powerlineNotch")}</span>
          <div class="flex gap-2">
            {#each ([["Hz60","🇺🇸",t("settings.us60Hz"),t("settings.us60HzSub")],["Hz50","🇪🇺",t("settings.eu50Hz"),t("settings.eu50HzSub")]] as const) as [val, flag, label, sub]}
              <button onclick={() => setNotch(val)}
                ...
                <span class="text-ui-sm text-muted-foreground">{sub}</span>
                {#if filter.notch === val}
            ...
            <button onclick={() => setNotch(null)}
              ...
              {#if filter.notch === null}
                <span class="text-ui-xs font-bold tracking-widest uppercase text-slate-500 mt-0.5">{t("common.active")}</span>
              {/if}
            </button>
          </div>
        </div>
        ...
        <div class="flex flex-col gap-2 px-4 py-3.5">
          <span class="text-ui-lg font-semibold text-foreground">{t("settings.highPassCutoff")}</span>
          <div class="flex items-center gap-1.5 flex-wrap">
            {#each ([null, 0.5, 1, 4, 8] as const) as hz}
              <button onclick={() => setHighPass(hz)}
                class="rounded-lg border px-3 py-1.5 text-ui-base font-semibold
                       transition-all cursor-pointer select-none
        ...
        <!-- Low-pass -->
        <div class="flex flex-col gap-2 px-4 py-3.5">
          <span class="text-ui-lg font-semibold text-foreground">{t("settings.lowPassCutoff")}</span>
          <div class="flex items-center gap-1.5 flex-wrap">
        ...
        <div class="flex items-center gap-2 flex-wrap px-4 py-3 bg-surface-3">
          <SectionHeader class="shrink-0">{t("settings.pipeline")}</SectionHeader>
          ...
          {/if}
          <span class="ml-auto text-ui-xs text-muted-foreground/60 shrink-0">{t("settings.gpuLatency")}</span>
        </div>
  ...
  <div class="flex flex-col gap-2">
    <div class="flex items-center gap-2 px-0.5">
      <SectionHeader>{t("settings.eegEmbedding")}</SectionHeader>
      {#if overlapSaving}
        <span class="text-ui-xs text-muted-foreground">saving…</span>
      {/if}
    </div>
      ...
      <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
        ...
        <div class="flex flex-col gap-2 px-4 py-3.5">
          <div class="flex items-baseline justify-between">
            <span class="text-ui-lg font-semibold text-foreground">{t("settings.epochOverlap")}</span>
          ...
          <ChipGroup
            items={OVERLAP_PRESETS}
            selected={OVERLAP_PRESETS.find(p => p[1] === overlapSecs) ?? OVERLAP_PRESETS[0]}
            onselect={(p) => setOverlap(p[1])}
            labelFn={(p) => p[0]}
          />
        </div>

        <div class="flex items-center gap-2 flex-wrap px-4 py-3 bg-surface-3">
          <SectionHeader class="shrink-0">Pipeline</SectionHeader>
          <Badge variant="outline"
            class="text-ui-xs py-0 px-1.5 bg-violet-500/10 text-violet-600 dark:text-violet-400 border-violet-500/20">
          ...
          <span class="ml-auto text-ui-xs text-muted-foreground/60 shrink-0">wgpu</span>
        </div>
