...
<section class="flex flex-col gap-2">
  <SectionHeader>{t("settings.geoProvider")}</SectionHeader>
  <SettingsCard>
    <CardContent class="flex flex-col gap-3 p-4">
      ...
      <div class="flex items-center justify-between gap-3">
        <span class="text-ui-md font-medium text-foreground">{t("settings.geoProviderLocal")}</span>
        <button
          aria-label={t("settings.geoProviderLocal")}
          class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors
                 {locationEnabled ? 'bg-violet-500' : 'bg-muted'}
      ...
      <!-- Status / test result -->
      {#if locationToggling}
        <p class="text-ui-base text-muted-foreground animate-pulse">{t("settings.geoProviderTesting")}</p>
      {:else if locationTestResult}
        {#if locationTestResult.ok}
          <div class="rounded-lg bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800/30 px-3 py-2
                      text-ui-base text-green-800 dark:text-green-300 leading-relaxed">
        ...
        {:else}
          <div class="rounded-lg bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800/30 px-3 py-2
                      text-ui-base text-red-800 dark:text-red-300 leading-relaxed">
            ❌ {locationTestResult.error}
          </div>
        {/if}
      {:else if !locationEnabled}
        <p class="text-ui-base text-muted-foreground">{t("settings.geoProviderOff")}</p>
      {/if}
    </CardContent>
  </SettingsCard>
</section>
...
<!-- ── Storage Format ───────────────────────────────────────────────────────── -->
<section class="flex flex-col gap-2">
  <SectionHeader>{t("settings.storageFormat")}</SectionHeader>
      ...
      <div class="flex gap-2">
        {#each (["csv", "parquet", "both"] as const) as fmt}
          <button onclick={async () => { storageFormat = fmt; await setStorageFormat(fmt); }}
            ...
            <span class="text-ui-lg font-bold leading-tight
                         {storageFormat === fmt ? 'text-violet-600 dark:text-violet-400' : 'text-foreground'}">
              {fmt === "csv" ? "CSV" : fmt === "parquet" ? "Parquet" : t("settings.storageFormatBoth")}
            </span>
            <span class="text-ui-xs text-muted-foreground text-center leading-tight">
            ...
            {#if storageFormat === fmt}
              <span class="text-ui-xs font-bold tracking-widest uppercase text-violet-500 mt-0.5">{t("common.active")}</span>
            {/if}
          </button>
        {/each}
      </div>
...
<!-- ── Inference Device preference ─────────────────────────────────────────── -->
<section class="flex flex-col gap-2">
  <SectionHeader>{t("settings.inferenceDevice")}</SectionHeader>
  <SettingsCard>
    <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
      <p class="px-4 py-2.5 text-ui-sm text-muted-foreground leading-relaxed">
        ...
        {#each (["gpu", "cpu"] as ("gpu" | "cpu")[]) as dev}
          ...
          <button
            ...
            class="flex-1 flex flex-col gap-0.5 items-start px-4 py-3 text-left transition-colors cursor-pointer
                   {isActive
                     ? 'bg-violet-500/5 dark:bg-violet-500/[0.08]'
                     : 'hover:bg-accent/50 dark:hover:bg-white/[0.02]'}">
            <div class="flex items-center gap-2">
              ...
              {#if isActive}
                <span class="text-ui-xs font-bold tracking-widest uppercase text-violet-500"
                  >Active</span
                >
              {/if}
      ...
      <p class="px-4 py-2 text-ui-sm text-amber-500/80 dark:text-amber-400/70">
...
{#if gpuStats}
  ...
  <section class="flex flex-col gap-2">
    <SectionHeader>{t("settings.gpuSection")} · {memLabel}</SectionHeader>
      ...
      <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
        ...
        {#if gpuStats.gpuName}
          <div class="flex items-center justify-between px-4 py-2.5">
            <span class="text-ui-md font-semibold text-foreground">{t("settings.gpuName")}</span>
            <span class="text-ui-base text-muted-foreground font-mono">{gpuStats.gpuName}</span>
          </div>
        {/if}
        ...
        {#if gpuStats.totalMemoryBytes}
          <div class="flex flex-col gap-2 px-4 py-3.5">
            <div class="flex items-baseline justify-between">
              <span class="text-ui-md font-semibold text-foreground">{memLabel}</span>
              {#if fmtBytes(gpuStats.totalMemoryBytes)}
                <span class="text-ui-base text-muted-foreground tabular-nums">
                  ...
                  {#if gpuStats.isUnifiedMemory}<span class="text-ui-xs ml-0.5 text-muted-foreground/60">total</span>{/if}
                </span>
              {/if}
            </div>
            ...
            {#if usedPct !== null && gpuStats.freeMemoryBytes !== null}
              <!-- Progress bar -->
              <div class="h-2 w-full rounded-full bg-muted dark:bg-white/[0.06] overflow-hidden">
                <div
              ...
              <div class="flex items-center justify-between text-ui-sm text-muted-foreground tabular-nums">
                <span>
            ...
            {#if gpuStats.isUnifiedMemory}
              <p class="text-ui-sm text-muted-foreground/60 leading-relaxed -mt-0.5">
        ...
        {#if gpuStats.overall > 0 || gpuStats.render > 0 || gpuStats.tiler > 0}
          <div class="flex items-center gap-4 px-4 py-3 bg-surface-3">
            <SectionHeader class="shrink-0">GPU Usage</SectionHeader>
            {#each ([
              ...
              <div class="flex items-center gap-1.5">
                <div class="h-1.5 w-16 rounded-full bg-muted dark:bg-white/[0.06] overflow-hidden">
...
<!-- ── Main Window ─────────────────────────────────────────────────────────── -->
<section class="flex flex-col gap-2">
  <SectionHeader>{t("settings.mainWindow")}</SectionHeader>

  <SettingsCard>
    <CardContent class="py-0 px-0">
      <ToggleRow
        checked={mainWindowAutoFit}
        label={t("settings.autoFitToggle")}
        description={t("settings.autoFitToggleDesc")}
        ontoggle={async () => {
...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>{t("settings.activityTracking")}</SectionHeader>
  ...
  <SettingsCard>
    <CardContent class="py-0 px-0">
      ...
      <ToggleRow
        ...
        description={t("settings.activeWindowToggleDesc")}
        ontoggle={async () => {
      ...
      {#if trackActiveWindow}
        <div class="border-t border-border dark:border-white/[0.06] px-4 py-3 flex flex-col gap-2 bg-muted/20 dark:bg-white/[0.02]">
          <span class="text-ui-xs font-semibold tracking-widest uppercase text-muted-foreground/70">
            ...
            <div class="flex flex-col gap-1.5">
              {#each ([
                ...
                <div class="flex items-baseline gap-2">
                  <span class="text-ui-xs text-muted-foreground/55 shrink-0 w-[4.5rem] text-right">{label}</span>
                  <span class="text-ui-base {cls} truncate">{value}</span>
                </div>
              {/each}
            </div>
          {:else}
            <p class="text-ui-sm text-muted-foreground/50 italic">{t("settings.activeWindowNone")}</p>
          {/if}
        </div>
      {/if}
      ...
      <ToggleRow
        ...
        description={t("settings.inputActivityToggleDesc")}
        ontoggle={async () => {
      ...
      {#if trackInputActivity}
        ...
        <div class="border-t border-border dark:border-white/[0.06] px-4 py-3 flex flex-col gap-2.5 bg-muted/20 dark:bg-white/[0.02]">
          ...
          <div class="flex flex-col gap-1.5">
            {#each ([
              ...
              <div class="flex items-baseline gap-2">
                <span class="text-ui-xs text-muted-foreground/55 shrink-0 w-[4.5rem] text-right">{label}</span>
                <span class="text-ui-base {ts > 0 ? 'text-foreground/80' : 'text-muted-foreground/40 italic'}">
          ...
          <p class="text-ui-xs text-muted-foreground/50 leading-relaxed">
...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>{t("settings.logging")}</SectionHeader>
    <span class="ml-auto text-ui-xs text-muted-foreground/60">{dataDirCurrent}/log_config.json</span>
  </div>
  ...
  <SettingsCard class="mt-2">
    <CardContent class="py-0 px-0">
      <ToggleRow
        checked={irohLogs}
        label={t("settings.logIroh")}
        description={t("settings.logIrohDesc")}
        ontoggle={async () => { irohLogs = !irohLogs; await setIrohLogs(irohLogs); }}
...
<!-- ── Data Directory ──────────────────────────────────────────────────────── -->
<section class="flex flex-col gap-2">
  <SectionHeader>{t("settings.dataDir")}</SectionHeader>
    ...
    <CardContent class="flex flex-col gap-3 py-3">
      ...
      <div class="flex items-center gap-2">
        <input type="text"
               ...
               class="flex-1 h-7 rounded-md border border-border bg-background px-2 text-ui-base
                      font-mono text-foreground placeholder:text-muted-foreground/40
                      focus:outline-none focus:ring-1 focus:ring-ring" />
        <Button variant="outline" size="sm"
                class="h-7 text-ui-sm px-2.5 border-border dark:border-white/10"
                onclick={async () => { await invoke("open_skill_dir"); }}>
          {t("settings.dataDirOpen")}
        </Button>
        {#if dataDirInput !== dataDirDefault}
          <Button variant="ghost" size="sm"
                  class="h-7 text-ui-sm px-2 text-muted-foreground hover:text-foreground"
      ...
      {#if dataDirChanged}
        <div class="flex items-center gap-2 rounded-lg bg-amber-500/10 border border-amber-500/20 px-3 py-2">
          <span class="text-ui-sm text-amber-600 dark:text-amber-400 flex-1">
...
<!-- ── WebSocket Server ──────────────────────────────────────────────────────── -->
<section class="flex flex-col gap-2">
  <SectionHeader>{t("settings.wsConfig")}</SectionHeader>
  ...
  <SettingsCard>
    <CardContent class="flex flex-col gap-3 py-3">
      ...
      <!-- Host selector -->
      <div class="flex flex-col gap-1">
        <span class="text-ui-sm font-medium text-foreground">{t("settings.wsHost")}</span>
        <div class="flex flex-col gap-1.5 mt-0.5">
          {#each [["127.0.0.1", t("settings.wsHostLoopback")], ["0.0.0.0", t("settings.wsHostLan")]] as [val, lbl]}
            <label class="flex items-center gap-2 cursor-pointer">
              ...
              <span class="text-ui-base text-foreground">{lbl}</span>
            </label>
          {/each}
        </div>
        {#if wsHost === "0.0.0.0"}
          <p class="text-ui-sm text-amber-600 dark:text-amber-400 leading-relaxed mt-0.5">
      ...
      <!-- Port input -->
      <div class="flex flex-col gap-1">
        <span class="text-ui-sm font-medium text-foreground">{t("settings.wsPort")}</span>
        <p class="text-ui-sm text-muted-foreground leading-relaxed">{t("settings.wsPortDesc")}</p>
        <div class="flex items-center gap-2">
          <input type="number" min="1024" max="65535"
                 ...
                 class="w-28 h-7 rounded-md border border-border bg-background px-2 text-ui-base
                        font-mono text-foreground focus:outline-none focus:ring-1 focus:ring-ring" />
          {#if wsPortError}
            <span class="text-ui-sm text-red-500">{wsPortError}</span>
          {/if}
        </div>
      </div>
      ...
      {#if wsChanged && !wsPortError}
        <div class="flex items-center gap-2 rounded-lg bg-cyan-500/10 border border-cyan-500/20 px-3 py-2">
          <span class="text-ui-sm text-cyan-600 dark:text-cyan-400 flex-1">
...
<section class="flex flex-col gap-2">
  <SectionHeader>{t("settings.hfEndpoint")}</SectionHeader>
  ...
  <SettingsCard>
    <CardContent class="flex flex-col gap-3 py-3">
      ...
      <p class="text-ui-xs text-muted-foreground/70 font-mono">{t("settings.hfEndpointCurrent")}: {hfEndpoint}</p>
    </CardContent>
  </SettingsCard>
</section>
...
<!-- ── API Token ────────────────────────────────────────────────────────────── -->
<section class="flex flex-col gap-2">
  <SectionHeader>{t("settings.apiToken")}</SectionHeader>

  <SettingsCard>
    <CardContent class="flex flex-col gap-3 py-3">
      <div class="flex flex-col gap-1">
        <span class="text-ui-sm font-medium text-foreground">{t("settings.apiTokenLabel")}</span>
        <p class="text-ui-sm text-muted-foreground leading-relaxed">{t("settings.apiTokenDesc")}</p>
        ...
        {#if apiToken}
          <p class="text-ui-xs text-emerald-600 dark:text-emerald-400 mt-0.5">
            {t("settings.apiTokenActive")}
          </p>
        {:else}
          <p class="text-ui-xs text-muted-foreground/60 mt-0.5">
