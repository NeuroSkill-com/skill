...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>{t("model.encoder")}</SectionHeader>
    <!-- Live status dot -->
    ...
    <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
        ...
        <div class="flex items-center gap-3 px-4 py-3.5">
          <div class="flex flex-col gap-0.5 min-w-0 flex-1">
            ...
            {#if modelStatus.encoder_describe}
              <span class="text-ui-base text-muted-foreground font-mono truncate">
                {modelStatus.encoder_describe}
              </span>
            {/if}
            {#if modelStatus.avg_embed_ms > 0}
              <span class="text-ui-sm text-muted-foreground/70 font-mono">
          ...
          <Badge variant="outline"
            class="shrink-0 text-ui-xs py-0 px-1.5
        ...
        <div class="flex flex-col gap-3 px-4 py-4">
          <div class="flex items-center justify-between gap-2">
            <div class="flex flex-col gap-0.5 min-w-0">
              <span class="text-ui-lg font-semibold text-foreground">{encoderName}</span>
              <span class="text-ui-base text-violet-600 dark:text-violet-400 truncate">
          ...
          {#if modelStatus.download_status_msg}
            <p class="text-ui-sm text-muted-foreground/70 leading-relaxed -mt-1">
          ...
          <div class="flex justify-end">
            <Button variant="outline" size="sm"
                    class="h-7 text-ui-base px-3 text-destructive border-destructive/30
      ...
      {:else if isAutoRetrying}
        <div class="flex flex-col gap-3 px-4 py-4">
          <div class="flex items-center justify-between gap-2">
            <div class="flex flex-col gap-0.5 min-w-0">
              <span class="text-ui-lg font-semibold text-foreground">{encoderName}</span>
              <span class="text-ui-base text-amber-600 dark:text-amber-400">
          ...
          {#if modelStatus.download_status_msg && modelStatus.download_status_msg !== "Download cancelled."}
            <p class="text-ui-sm text-destructive/80 font-mono break-all leading-relaxed
          ...
          <div class="flex items-center gap-3">
            <div class="relative w-8 h-8 shrink-0 flex items-center justify-center">
              ...
              <span class="text-ui-sm font-bold tabular-nums text-amber-600 dark:text-amber-400">
          ...
          <div class="flex justify-end gap-2">
            ...
            <Button size="sm" class="h-7 text-ui-base px-3" onclick={startDownload}>
              {t("model.retry")}
            </Button>
          </div>
        </div>
      ...
      {:else if hasFailed}
        <div class="flex flex-col gap-3 px-4 py-4">
          <div class="flex items-center justify-between gap-2">
            <div class="flex flex-col gap-0.5 min-w-0">
              <span class="text-ui-lg font-semibold text-foreground">{encoderName}</span>
              <span class="text-ui-base text-destructive truncate">
      ...
      {:else if wasCancelled}
        <div class="flex items-center gap-3 px-4 py-3.5">
          <div class="flex flex-col gap-0.5 min-w-0 flex-1">
            <span class="text-ui-lg font-semibold text-foreground">{encoderName}</span>
            <span class="text-ui-base text-muted-foreground">{t("model.downloadCancelled")}</span>
          </div>
          <Button size="sm" variant="outline"
                  class="shrink-0 h-7 text-ui-base px-3"
      ...
      {:else if needsRestart}
        <div class="flex flex-col gap-3 px-4 py-4">
          ...
          <div class="flex justify-end">
            <Button size="sm"
                    class="h-7 text-ui-base px-3 bg-amber-500 hover:bg-amber-600 text-white"
                    disabled={restarting}
                    onclick={restartApp}>
      ...
      {:else if weightsReadyNoSession}
        <div class="flex items-center gap-3 px-4 py-3.5">
          <div class="flex flex-col gap-0.5 min-w-0 flex-1">
            <span class="text-ui-lg font-semibold text-foreground">{encoderName}</span>
            <span class="text-ui-base text-muted-foreground/70">
      ...
      {:else if encoderLoading}
        <div class="flex flex-col gap-3 px-4 py-4">
          <div class="flex items-center justify-between gap-2">
            <div class="flex flex-col gap-0.5 min-w-0">
              <span class="text-ui-lg font-semibold text-foreground">{encoderName}</span>
              <span class="text-ui-base text-muted-foreground">{t("model.encoderLoading")}</span>
            </div>
            <Badge variant="outline"
              class="shrink-0 text-ui-xs py-0 px-1.5
        ...
        <div class="flex flex-col gap-3 px-4 py-4">
          ...
          <div class="flex items-center gap-2 rounded-lg bg-muted/30 px-3 py-2.5
            ...
            <div class="flex flex-col gap-0.5 flex-1 min-w-0">
              <span class="text-ui-sm text-muted-foreground/70 font-mono truncate">
                {modelConfig.model_backend === "luna" ? modelConfig.luna_hf_repo : modelConfig.hf_repo}
              </span>
            </div>
            <Button size="sm" class="shrink-0 h-7 text-ui-base px-3" onclick={startDownload}>
              {t("model.download")}
            </Button>
          </div>
        </div>
      ...
      {#if modelStatus.weights_path}
        <div class="flex flex-col gap-0.5 px-4 py-3">
          <span class="text-ui-sm font-medium text-foreground">{t("model.weightsPath")}</span>
          <span class="text-ui-sm text-muted-foreground font-mono break-all leading-relaxed">
      ...
      <!-- ── Today's storage ─────────────────────────────────────────────── -->
      <div class="flex items-start gap-6 flex-wrap px-4 py-3 bg-surface-3">
        <div class="flex flex-col gap-0.5 min-w-0 flex-1">
          <SectionHeader>{t("model.todaysDb")}</SectionHeader>
          <span class="text-ui-sm font-mono text-muted-foreground break-all leading-relaxed">
      ...
      {#if hnswRebuilt}
        <div class="flex items-center justify-between gap-2 px-4 py-2.5 bg-amber-500/8 border-t border-amber-500/20">
          <Badge variant="outline"
            class="text-ui-xs py-0 px-1.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20">
            {t("model.hnswRecoveredTitle")}
          </Badge>
          <span class="text-ui-sm text-amber-700/90 dark:text-amber-300/90 tabular-nums">
...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>{t("model.hnswIndex")}</SectionHeader>
    {#if modelConfigSaving}
      <span class="text-ui-xs text-muted-foreground">{t("common.saving")}</span>
    {/if}
    <span class="ml-auto text-ui-xs text-muted-foreground/60">{t("model.takesEffectRestart")}</span>
  </div>
    ...
    <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
      ...
      <div class="flex flex-col gap-2 px-4 py-3.5">
        <div class="flex items-baseline justify-between">
          <span class="text-ui-lg font-semibold text-foreground">
            {t("model.connectivity")} <code class="text-ui-md text-muted-foreground">M</code>
          </span>
          <span class="text-ui-base text-muted-foreground">{t("model.currently", { n: modelConfig.hnsw_m })}</span>
        </div>
      ...
      <div class="flex flex-col gap-2 px-4 py-3.5">
        <div class="flex items-baseline justify-between">
          <span class="text-ui-lg font-semibold text-foreground">
            {t("model.buildQuality")} <code class="text-ui-md text-muted-foreground">ef</code>
          </span>
          <span class="text-ui-base text-muted-foreground">{t("model.currently", { n: modelConfig.hnsw_ef_construction })}</span>
        </div>
        <p class="text-ui-base text-muted-foreground leading-relaxed -mt-0.5">
      ...
      <div class="flex items-center gap-2 flex-wrap px-4 py-3 bg-surface-3">
        <SectionHeader class="shrink-0">{t("model.index")}</SectionHeader>
        <Badge variant="outline"
          class="text-ui-xs py-0 px-1.5 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20">
...
<section class="flex flex-col gap-2">
  <SectionHeader>{t("model.reembed")}</SectionHeader>
    ...
    <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
      ...
      {#if reembedEstimate && reembedEstimate.total_epochs > 0}
        <div class="flex flex-col gap-2.5 px-4 py-3.5">
          <div class="flex items-center justify-between gap-2">
            <SectionHeader>{t("model.embeddingCoverage")}</SectionHeader>
            <Badge variant="outline"
              class="text-ui-xs py-0 px-1.5
          ...
          <div class="flex items-center justify-between gap-2">
            <span class="text-ui-sm text-muted-foreground font-mono">
            ...
            {#if reembedEstimate.missing > 0 && reembedEstimate.eta_secs > 0}
              <span class="text-ui-xs text-muted-foreground/70 font-mono">
                {t("model.coverageEta", { eta: fmtEta(reembedEstimate.eta_secs) })}
              </span>
            {/if}
          </div>
          {#if reembedEstimate.missing > 0}
            <span class="text-ui-sm text-amber-600 dark:text-amber-400">
              {t("model.coverageMissing", { missing: reembedEstimate.missing.toLocaleString() })}
            </span>
          {:else}
            <span class="text-ui-sm text-emerald-600 dark:text-emerald-400">
      ...
      {#if reembedEstimate?.idle_reembed}
        ...
        {#if ir.active}
          <div class="flex items-center gap-3 px-4 py-2.5 bg-blue-500/5">
            ...
            <div class="flex flex-col gap-0.5 min-w-0 flex-1">
              <span class="text-ui-sm font-medium text-blue-600 dark:text-blue-400">
                {t("model.idleReembedActive")}
              </span>
              {#if ir.current_day}
                <span class="text-ui-xs text-muted-foreground font-mono">
            ...
            {#if ir.total > 0}
              <Badge variant="outline"
                class="shrink-0 text-ui-xs py-0 px-1.5 bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20">
      ...
      <div class="flex flex-col gap-3 px-4 py-3.5">
          ...
          <div class="flex flex-col gap-2">
            ...
            <div class="flex items-center justify-between gap-2">
              ...
              {#if (reembedProgress.status === "processing" || reembedProgress.status === "running") && etaRemain > 0}
                <span class="text-ui-xs text-muted-foreground/50 font-mono tabular-nums">
        ...
        {:else}
          ...
          <div class="flex items-center justify-between gap-2">
            {#if reembedEstimate && reembedEstimate.total_epochs > 0}
              <span class="text-ui-base text-muted-foreground font-mono">
                {t("model.reembedEstimate", { days: String(reembedEstimate.date_dirs), rows: reembedEstimate.total_epochs.toLocaleString() })}
              </span>
            {:else}
              <span class="text-ui-base text-muted-foreground/70">{t("model.reembedNoData")}</span>
            {/if}
            <Button size="sm" variant="outline"
                    class="shrink-0 h-7 text-ui-base px-3"
      ...
      {#if reembedEstimate && reembedEstimate.per_day && reembedEstimate.per_day.length > 0}
        <div class="flex flex-col px-4 py-2.5">
          <button
            class="flex items-center gap-1.5 text-ui-sm text-muted-foreground/70 hover:text-foreground transition-colors cursor-pointer select-none"
          ...
          {#if perDayExpanded}
            <div class="mt-2 max-h-48 overflow-y-auto rounded border border-border dark:border-white/[0.06]">
              <table class="w-full text-ui-xs">
                <thead class="sticky top-0 bg-muted dark:bg-surface-3">
                ...
                <tbody>
                  {#each reembedEstimate.per_day as day}
                    ...
                    <tr class="border-t border-border/50 dark:border-white/[0.04]">
      ...
      <div class="flex flex-col gap-3 px-4 py-3.5">
        <div class="flex items-center justify-between gap-4">
          <div class="flex flex-col gap-0.5">
            <span class="text-ui-md font-medium text-foreground">{t("model.gpuPrecision")}</span>
            <span class="text-ui-xs text-muted-foreground/70">{t("model.gpuPrecisionDesc")}</span>
          </div>
          <select
            ...
            class="text-ui-base rounded border border-border dark:border-white/[0.08]
                   bg-background dark:bg-surface-1 px-2 py-1
        ...
        <div class="flex items-center justify-between gap-4">
          <div class="flex flex-col gap-0.5">
            <span class="text-ui-md font-medium text-foreground">{t("model.idleReembed")}</span>
            <span class="text-ui-xs text-muted-foreground/70">{t("model.idleReembedDesc")}</span>
          </div>
          <input type="checkbox"
        ...
        {#if reembedConfig.idle_reembed_enabled}
          <div class="flex items-center justify-between gap-4">
            <span class="text-ui-base text-muted-foreground">{t("model.idleDelay")}</span>
            <select
              ...
              class="text-ui-sm rounded border border-border dark:border-white/[0.08]
                     bg-background dark:bg-surface-1 px-2 py-0.5
                     text-foreground focus:outline-none focus:ring-1 focus:ring-blue-500/50">
...
<!-- ── Model source ──────────────────────────────────────────────────────────── -->
<section class="flex flex-col gap-2">
  <SectionHeader>{t("model.modelSource")}</SectionHeader>

  <SettingsCard>
    <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">

      <div class="flex items-center justify-between gap-4 px-4 py-3.5">
        <span class="text-ui-lg font-semibold text-foreground">{t("model.hfRepo")}</span>
        <span class="text-ui-base font-mono text-muted-foreground">
          {modelConfig.model_backend === "luna" ? modelConfig.luna_hf_repo : modelConfig.hf_repo}
        </span>
      </div>
      ...
      {#if modelConfig.model_backend === "zuna"}
        <div class="flex items-center justify-between gap-4 px-4 py-3.5">
          <div class="flex flex-col gap-0.5">
            <span class="text-ui-lg font-semibold text-foreground">{t("model.dataNormalisation")}</span>
            <span class="text-ui-base text-muted-foreground">{t("model.dataNormDesc")}</span>
          </div>
          <span class="text-ui-lg font-mono font-semibold text-foreground">{modelConfig.data_norm}</span>
        </div>
      {/if}
