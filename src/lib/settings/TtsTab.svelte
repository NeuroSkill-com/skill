...
<div class="flex flex-col gap-5 px-4 py-4 pb-8">
  ...
  <!-- ── 2. Engine status ───────────────────────────────────────────────────── -->
  <section class="flex flex-col gap-2">
    <SectionHeader>{t("ttsTab.statusSection")}</SectionHeader>
      ...
      <CardContent class="flex flex-col gap-3 px-4 py-3.5">
          ...
          <div class="ml-auto flex items-center gap-1.5">
            <button
              ...
              class="rounded-lg border border-border dark:border-white/[0.08]
                     bg-muted dark:bg-surface-2 px-2.5 py-1 text-ui-sm font-semibold
        ...
        {#if enginePhase === "loading"}
          <div class="flex flex-col gap-1">
            <div class="flex items-center justify-between">
              <span class="text-ui-sm text-muted-foreground truncate max-w-[80%]" title={loadLabel}>
        ...
        {#if enginePhase === "error" && errorMsg}
          <div class="rounded-lg border border-rose-200 dark:border-rose-800/60
            ...
            <p class="text-ui-sm font-semibold text-rose-700 dark:text-rose-400 mb-0.5">
              {t("ttsTab.errorTitle")}
            </p>
            <p class="text-ui-sm text-rose-600/80 dark:text-rose-400/70 font-mono break-all leading-relaxed">
              {errorMsg}
            </p>
          </div>
        {/if}
  ...
  {#if activeBackend === "kitten"}
    <section class="flex flex-col gap-2">
      <SectionHeader>{t("ttsTab.kittenConfigSection")}</SectionHeader>
      ...
      <SettingsCard>
        <CardContent class="flex flex-col gap-3 px-4 py-3.5">
            ...
            <div class="flex flex-wrap gap-1.5">
              {#each kittenVoices as v}
                <button
                  ...
                  class="rounded-lg border px-3 py-1.5 text-ui-base font-medium transition-colors
                         {kittenVoice === v
          ...
          <!-- Model info -->
          <p class="text-ui-sm text-muted-foreground/60 font-mono">
  ...
  {#if activeBackend === "neutts"}
    <section class="flex flex-col gap-2">
      <SectionHeader>{t("ttsTab.neuttsConfigSection")}</SectionHeader>
      ...
      <SettingsCard>
        <CardContent class="py-0 px-0 flex flex-col divide-y divide-border dark:divide-white/[0.05]">
          ...
          <div class="px-4 py-3.5 flex flex-col gap-2">
            ...
            {#if selectedModel()}
              ...
              <div class="rounded-lg border border-border dark:border-white/[0.06]
                ...
                <span class="text-ui-sm text-emerald-600 dark:text-emerald-400">✓ {m.pros}</span>
                <span class="text-ui-sm text-muted-foreground/50">✗ {m.cons}</span>
              </div>
            {/if}
          </div>
          ...
          <div class="px-4 py-3.5 flex flex-col gap-2">
            ...
            <!-- Preset voice grid -->
            <div class="flex flex-col gap-1">
              <span class="text-ui-sm font-medium text-muted-foreground/70 uppercase tracking-wider">
              ...
              <div class="grid grid-cols-3 gap-1.5">
                {#each PRESET_VOICES as pv}
                  <button
                    ...
                    <span class="text-ui-xs text-muted-foreground/50 leading-none mt-0.5">
                      {pv.gender} {pv.lang}
                    </span>
                  </button>
                {/each}
                ...
                <button
                  ...
                  <span class="text-ui-xs text-muted-foreground/40 leading-none mt-0.5">WAV</span>
                </button>
              </div>
            </div>
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <div class="flex items-center gap-2">
                    <span class="flex-1 truncate rounded-lg border border-border dark:border-white/[0.08]
                                 bg-muted dark:bg-surface-2 px-2.5 py-1.5 text-ui-sm
                    ...
                    <button
                      ...
                      class="shrink-0 rounded-lg border border-border dark:border-white/[0.08]
                             bg-muted dark:bg-surface-2 px-2.5 py-1.5 text-ui-sm font-semibold
                             text-muted-foreground hover:text-foreground transition-colors">
          ...
          <div class="px-4 py-3 flex justify-end">
            <button
              ...
              class="relative overflow-hidden rounded-lg px-3.5 py-1.5 text-ui-base font-semibold
                     transition-all duration-300
                     {neuttsSaved
                       ? 'bg-emerald-500 text-white border border-emerald-500'
                       : neuttsDirty
                         ? 'bg-indigo-500 hover:bg-indigo-600 text-white border border-indigo-500'
                         : 'bg-muted dark:bg-surface-2 text-muted-foreground border border-border dark:border-white/[0.08] opacity-40 cursor-not-allowed'}">
              {#key neuttsSaved}
  ...
  <!-- ── 4. Test voice ──────────────────────────────────────────────────────── -->
  <section class="flex flex-col gap-2">
    <SectionHeader>{t("ttsTab.testSection")}</SectionHeader>
    <p class="text-ui-base text-muted-foreground/70 -mt-1">
      {t("ttsTab.testDesc")}
    </p>
    <TtsTestWidget />
  </section>
  ...
  <!-- ── 5. API snippets ────────────────────────────────────────────────────── -->
  <section class="flex flex-col gap-2">
    <SectionHeader>{t("ttsTab.apiSection")}</SectionHeader>
    <p class="text-ui-base text-muted-foreground/70 -mt-1">
      {t("ttsTab.apiDesc")}
    </p>
  ...
  -d '{"text":"Eyes closed. Relax."}'`],
        ...
        <div class="px-3 py-2.5 flex flex-col gap-1">
          ...
          <pre class="text-ui-base font-mono text-foreground/80 whitespace-pre-wrap leading-relaxed">{code}</pre>
        </div>
      {/each}
    </div>
  </section>
  ...
  <!-- ── 6. Startup behaviour ─────────────────────────────────────────────── -->
  <section class="flex flex-col gap-2">
    <SectionHeader>{t("ttsTab.startupSection")}</SectionHeader>

    <SettingsCard>
      <ToggleRow
        checked={ttsPreload}
        label={t("ttsTab.preloadOnStartup")}
        description={t("ttsTab.preloadOnStartupDesc")}
        ontoggle={() => {
  ...
  <!-- ── 7. Debug logging ───────────────────────────────────────────────────── -->
  <section class="flex flex-col gap-2">
    <SectionHeader>{t("ttsTab.loggingSection")}</SectionHeader>

    <SettingsCard>
      <ToggleRow
        checked={logConfig.tts}
        label={t("ttsTab.loggingLabel")}
        description={t("ttsTab.loggingDesc")}
        ontoggle={toggleTtsLog}
