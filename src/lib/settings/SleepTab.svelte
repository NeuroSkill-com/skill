      ...
      <div class="flex items-center gap-6 flex-wrap">
        ...
        <div class="flex-1 flex flex-col gap-4 min-w-[180px]">
          ...
          <div class="flex flex-col gap-1.5">
            <label for="bedtime" class="text-ui-base font-semibold text-muted-foreground/70 uppercase tracking-wider flex items-center gap-1.5">
            ...
            <input id="bedtime" type="time" value={config.bedtime}
                   ...
                   class="w-full rounded-lg border border-border dark:border-white/[0.08]
                          ...
                          text-ui-lg font-semibold tabular-nums text-foreground
                          focus:outline-none focus:ring-2 focus:ring-primary/30" />
          </div>

          <!-- Wake time -->
          <div class="flex flex-col gap-1.5">
            <label for="waketime" class="text-ui-base font-semibold text-muted-foreground/70 uppercase tracking-wider flex items-center gap-1.5">
          ...
          <div class="flex items-center gap-2 px-1">
            ...
            {#if saving}
              <span class="text-ui-xs text-muted-foreground animate-pulse ml-auto">{t("common.saving")}</span>
            {/if}
          </div>
        ...
        {#each PRESETS as p}
          ...
          <button
            ...
            class="flex items-center gap-3 rounded-xl border px-4 py-3
                   ...
                   {active
                     ? 'border-violet-500/50 bg-violet-500/[0.06] dark:bg-violet-500/[0.08]'
                     : 'border-border dark:border-white/[0.06] bg-surface-1 hover:bg-accent/50 dark:hover:bg-white/[0.02]'}">
            ...
            <div class="flex-1 flex flex-col gap-0.5 min-w-0">
              <span class="text-ui-md font-semibold {active ? 'text-foreground' : 'text-foreground/80'}">
                {p.label()}
              </span>
            ...
            <span class="shrink-0 text-ui-sm font-mono tabular-nums
                         {active ? 'text-violet-600 dark:text-violet-400' : 'text-muted-foreground/40'}">
              {p.bedtime} — {p.wake}
            </span>
          </button>
        {/each}
