  ...
  <Card class="border-amber-500/30 bg-amber-500/5 dark:bg-amber-500/5 gap-0 py-0 overflow-hidden">
    <CardContent class="flex items-center gap-3 p-4">
      ...
      <div class="flex flex-col gap-0.5 flex-1 min-w-0">
        <span class="text-ui-md font-semibold text-amber-700 dark:text-amber-300 leading-tight">
      ...
      <span
        class="text-ui-2xs font-bold tracking-widest uppercase text-amber-600/60 dark:text-amber-400/60"
      >
        {t("lsl.switchHint")}
      </span>
    </CardContent>
...
{#if isLslSessionActive}
  <Card class="border-emerald-500/30 bg-emerald-500/5 dark:bg-emerald-500/5 gap-0 py-0 overflow-hidden">
    <CardContent class="flex items-center gap-3 p-4">
      ...
      <div class="flex flex-col gap-0.5 flex-1 min-w-0">
        <span class="text-ui-md font-semibold text-emerald-700 dark:text-emerald-300 leading-tight">
          {t("lsl.sessionActive")}
        </span>
        <span class="text-ui-sm text-emerald-600/70 dark:text-emerald-400/70 truncate">
      ...
      <span
        class="text-ui-xs font-bold tracking-widest uppercase text-emerald-600 dark:text-emerald-400"
      >
      ...
      <CardContent class="py-0 px-0">
        ...
        >
          {#each sessionChannelNames as ch, i}
            <div class="flex flex-col gap-0.5 px-3 py-2 bg-surface-1">
              <span class="text-ui-xs tabular-nums text-muted-foreground/40 font-mono">#{i + 1}</span>
              <span class="text-ui-md font-semibold text-foreground truncate">{ch}</span>
            </div>
          {/each}
        </div>
      </CardContent>
...
{#if secondarySessions.length > 0}
  <section class="flex flex-col gap-2">
    <SectionHeader>{t("lsl.backgroundSessions")} ({secondarySessions.length})</SectionHeader>
    <SettingsCard>
      <CardContent class="py-0 px-0">
        {#each secondarySessions as sess (sess.id)}
          ...
          >
            <span class="relative flex h-2 w-2 shrink-0">
              <span
                class="animate-ping absolute inline-flex h-full w-full rounded-full bg-violet-500/60 opacity-75"
              ></span>
            ...
            <button
              class="text-muted-foreground/40 hover:text-red-500 cursor-pointer text-ui-md shrink-0"
...
<!-- ── Auto-Connect Toggle ────────────────────────────────────────────────── -->
<section class="flex flex-col gap-2">
  <SectionHeader>{t("lsl.autoConnect")}</SectionHeader>

  <SettingsCard>
    <CardContent class="py-0 px-0">
      <ToggleRow
        checked={autoConnect}
        label={t("lsl.autoConnectToggle")}
        description={t("lsl.autoConnectDesc")}
      ...
      {#if pairedStreams.length > 0}
        <div
          class="border-t border-border dark:border-white/[0.06] px-4 py-3 flex flex-col gap-2 bg-muted/20 dark:bg-white/[0.02]"
        >
          ...
          <div class="flex flex-col gap-1.5">
            {#each pairedStreams as paired}
              <div class="flex items-center gap-2 text-ui-sm">
                ...
                {#if paired.name}
                  <span class="text-muted-foreground/40 font-mono text-ui-2xs truncate"
                ...
                <button
                  class="ml-auto text-muted-foreground/40 hover:text-red-500 cursor-pointer shrink-0 text-ui-sm"
...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>{t("lsl.idleTimeout")}</SectionHeader>
    {#if idleTimeoutSaving}
      <span class="text-ui-xs text-muted-foreground/60">saving…</span>
    {/if}
  </div>
      ...
      <div class="flex flex-col gap-3 px-4 py-3.5">
        ...
        <div class="flex items-center gap-1.5 flex-wrap">
          <button
            ...
            class="rounded-lg border px-2.5 py-1.5 text-ui-base font-semibold transition-all cursor-pointer select-none
...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>Virtual test source</SectionHeader>
    <span class="ml-auto text-ui-2xs text-muted-foreground/40">
      SkillVirtualEEG · 32ch · 256 Hz
    </span>
  </div>

  <SettingsCard class={virtualSourceRunning ? 'ring-1 ring-emerald-500/30' : ''}>
    <CardContent class="flex items-center gap-3 px-4 py-3.5">
      <!-- Status badge -->
      <span class="shrink-0 inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-ui-xs font-semibold
      ...
      {#if virtualSourceRunning}
        <Button
          variant="outline"
          size="sm"
          class="h-7 text-ui-sm px-3 border-red-500/30 text-red-500 hover:bg-red-500/10"
      ...
      {:else}
        <Button
          size="sm"
          class="h-7 text-ui-sm px-3 bg-emerald-600 hover:bg-emerald-700 text-white"
          disabled={virtualSourceBusy}
    ...
    {#if virtualSourceError}
      <div class="px-4 pb-3 text-ui-sm text-red-500 leading-relaxed">{virtualSourceError}</div>
    {/if}
    <!-- Link to full Virtual Devices window -->
    <div class="px-4 pb-3 border-t border-border dark:border-white/[0.06]">
      <p class="text-ui-xs text-muted-foreground/50 leading-relaxed mt-2.5">
        ...
        <button
          class="text-violet-600 dark:text-violet-400 hover:underline cursor-pointer font-semibold"
...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    <SectionHeader>{t("lsl.localStreams")}</SectionHeader>
    {#if lastScanLabel}
      <span class="ml-auto text-ui-2xs text-muted-foreground/40">{lastScanLabel}</span>
    {/if}
  </div>
  ...
  <SettingsCard>
    <CardContent class="flex flex-col gap-3 p-4">
      <div class="flex items-center gap-2">
        ...
        {#if autoConnect}
          <span
            class="ml-auto flex items-center gap-1.5 text-ui-xs font-semibold text-emerald-600 dark:text-emerald-400"
          >
      ...
      {#if scanError}
        <p class="text-ui-sm text-red-500 leading-relaxed">{scanError}</p>
      {/if}
          ...
          {#each sortedStreams as stream (stream.source_id || stream.name)}
            ...
            >
              ...
              <div class="flex flex-col gap-1 flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  <span class="text-ui-md font-semibold text-foreground truncate"
                    >{stream.name}</span
                  >
                  <span
                    class="text-ui-2xs font-bold tracking-widest uppercase px-1.5 py-0.5 rounded
                               bg-violet-500/10 text-violet-600 dark:text-violet-400">{stream.type}</span
                  >
                  {#if stream.paired}
                    <span
                      class="text-[0.46rem] font-bold tracking-widest uppercase px-1.5 py-0.5 rounded
              ...
              <div class="flex items-center gap-1.5 shrink-0">
                {#if stream.paired}
                  <Button
                    variant="ghost"
                    size="sm"
                    class="h-7 text-ui-xs px-2 text-muted-foreground/60 hover:text-red-500"
                ...
                {:else}
                  <Button
                    variant="ghost"
                    size="sm"
                    class="h-7 text-ui-xs px-2 text-muted-foreground"
                    onclick={() => togglePair(stream)}
                  >
                    {t("lsl.pair")}
                  </Button>
      ...
      {:else if scanning}
        <div class="flex items-center justify-center py-6 gap-2">
          <span
            class="h-4 w-4 border-2 border-violet-500/40 border-t-primary rounded-full animate-spin"
          ></span>
          <span class="text-ui-sm text-muted-foreground">{t("lsl.scanningNetwork")}</span>
        </div>
      {:else}
        <!-- Empty state -->
        <div
...
<section class="flex flex-col gap-2">
  ...
  >
    ...
    <SectionHeader>{t("lsl.irohRemote")}</SectionHeader>
      ...
      <CardContent class="flex flex-col gap-3 p-4">
        ...
        <div class="flex items-center gap-2">
          {#if irohStatus.running}
            ...
            <span class="text-ui-sm font-semibold text-emerald-600 dark:text-emerald-400">
              {t("lsl.irohRunning")}
            </span>
            <Button
              variant="outline"
              size="sm"
              class="h-7 text-ui-sm px-3 ml-auto border-red-500/30 text-red-500 hover:bg-red-500/10"
              onclick={stopIroh}
            >
              {t("lsl.irohStop")}
            </Button>
          {:else}
            ...
            <Button
              variant="outline"
              size="sm"
              class="h-7 text-ui-sm px-3 ml-auto"
        ...
        {#if irohError}
          <p class="text-ui-sm text-red-500 leading-relaxed">{irohError}</p>
        {/if}
        ...
        {#if irohStatus.endpoint_id}
          ...
          >
            <div class="flex items-center gap-2">
              <span
                class="text-ui-xs font-semibold tracking-widest uppercase text-cyan-600 dark:text-cyan-400"
              >
                {t("lsl.irohEndpointId")}
              </span>
              <button
                aria-label="Copy endpoint ID"
                class="ml-auto text-ui-xs font-semibold px-2 py-0.5 rounded
            ...
            <code
              class="text-ui-sm font-mono text-foreground break-all select-all leading-relaxed"
            >
              {irohStatus.endpoint_id}
            </code>
            <p class="text-ui-xs text-muted-foreground/60 leading-relaxed">
