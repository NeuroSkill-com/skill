...
<section class="flex flex-col gap-2">
  <div class="flex items-center gap-2 px-0.5">
    ...
    <span class="text-ui-xs text-muted-foreground/50">tools diagnostics</span>
  </div>

  <SettingsCard>
    <CardContent class="flex flex-col gap-2.5 px-4 py-3.5">
      <div class="flex items-center gap-2 text-ui-base">
        <span class="text-muted-foreground">Permission:</span>
        <span class="rounded-full border px-2 py-0.5 text-ui-sm font-semibold
        ...
        <button
          class="ml-auto rounded-md border border-border px-2 py-1 text-ui-sm font-semibold text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
      ...
      <div class="flex flex-wrap items-center gap-2">
        <button
          class="rounded-md border border-border px-2.5 py-1 text-ui-sm font-semibold text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
          onclick={async () => {
        ...
        <button
          class="rounded-md border border-blue-500/30 bg-blue-500/10 px-2.5 py-1 text-ui-sm font-semibold text-blue-600 dark:text-blue-400 hover:bg-blue-500/15 transition-colors disabled:opacity-50"
      ...
      {#if calendarTestError}
        <p class="text-ui-sm text-red-600 dark:text-red-400 leading-relaxed">{calendarTestError}</p>
      {/if}

      {#if calendarEvents.length > 0}
        <div class="rounded-lg border border-border/60 dark:border-white/[0.08] bg-muted/40 dark:bg-surface-3 px-2.5 py-2 max-h-44 overflow-y-auto">
          <p class="text-ui-xs uppercase tracking-wider text-muted-foreground/70 mb-1.5">Upcoming ({calendarEvents.length})</p>
          <div class="flex flex-col gap-1">
            {#each calendarEvents.slice(0, 20) as ev}
              <div class="text-ui-sm leading-relaxed">
