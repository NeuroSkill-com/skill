...
<section class="flex flex-col gap-4">
  {#if daemonStatus.state !== 'connected'}
    <div class="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 flex items-center gap-2.5">
      ...
      <span class="text-ui-base text-amber-600 dark:text-amber-400 font-medium">
  ...
  {#if !editing}
    ...
    <div class="flex items-center justify-between">
      <span class="text-ui-lg font-semibold text-foreground">{t("calibration.profiles")}</span>
      <Button size="sm" variant="outline" class="text-ui-base h-7 px-2.5 gap-1"
      ...
      {#each profiles as p}
        ...
        <div class="rounded-xl border transition-all
                    {isActive
                      ...
                      : 'border-border dark:border-white/[0.06] bg-surface-1'}">
          <div class="flex items-center gap-3 px-3 py-2.5">
            ...
            <div class="flex flex-col gap-0.5 flex-1 min-w-0">
              <span class="text-ui-md font-semibold truncate">{p.name}</span>
              <div class="flex items-center gap-1.5 flex-wrap">
                {#each p.actions as a}
                  <Badge variant="outline"
                    class="text-ui-2xs py-0 px-1 bg-muted border-border/50 text-muted-foreground">
                    {a.label}
                  </Badge>
                {/each}
              ...
              {:else}
                <span class="text-ui-2xs text-amber-600/60 dark:text-amber-400/50 italic">
    ...
    <div class="flex items-center gap-3 pt-1">
      <Button size="sm" class="text-ui-md h-9 px-5 gap-1.5" onclick={openCalibration}>
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
  ...
  {:else}
    ...
    <div class="flex items-center gap-2">
      ...
      <span class="text-ui-lg font-semibold">
    ...
    <div class="flex flex-col gap-1.5">
      <span class="text-ui-sm font-semibold text-muted-foreground/60 uppercase tracking-wider">
    ...
    <SettingsCard>
      <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
        ...
        <div class="flex flex-col gap-1.5 px-4 py-3">
          <label for="calibration-profile-name" class="text-ui-sm font-semibold text-muted-foreground uppercase tracking-wider">
          ...
          <input id="calibration-profile-name" type="text" bind:value={editing.name}
                 ...
                 class="rounded-lg border border-border dark:border-white/[0.08] bg-muted dark:bg-surface-2
                        px-2.5 py-1.5 text-ui-md text-foreground
        ...
        <div class="flex flex-col gap-2.5 px-4 py-3.5">
          <div class="flex items-center justify-between">
            <span class="text-ui-lg font-semibold text-foreground">{t("settings.actionLabels")}</span>
            <button onclick={addAction}
                  aria-label="Add action"
                  class="flex items-center gap-1 text-ui-sm font-semibold text-violet-600 dark:text-violet-400
                    hover:text-violet-600/80 transition-colors">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                   stroke-linecap="round" stroke-linejoin="round" class="w-3 h-3">
                <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
              </svg>
            ...
            <div class="flex items-center gap-2">
              ...
              <input type="text" bind:value={action.label}
                     ...
                     class="flex-1 rounded-lg border border-border dark:border-white/[0.08]
                            bg-muted dark:bg-surface-2 px-2.5 py-1.5
                  text-ui-md text-foreground focus:outline-none focus:ring-1 focus:ring-ring/50" />
              <!-- Duration -->
              <ChipGroup
                items={[5,10,15,20,30]}
                selected={[5,10,15,20,30].find(s => s === action.duration_secs) ?? 5}
                onselect={(secs) => action.duration_secs = secs}
                labelFn={(secs) => `${secs}s`}
              />
              <!-- Remove -->
        ...
        <!-- Break + loops -->
        <div class="flex flex-col gap-2.5 px-4 py-3.5">
          <span class="text-ui-lg font-semibold text-foreground">{t("settings.timing")}</span>
          <div class="flex gap-4">
            <div class="flex flex-col gap-1 flex-1">
              ...
              <ChipGroup
                items={[3,5,10,15,20]}
                selected={[3,5,10,15,20].find(s => s === editing!.break_duration_secs) ?? 5}
                onselect={(secs) => editing!.break_duration_secs = secs}
                labelFn={(secs) => `${secs}s`}
              />
            </div>
            <div class="flex flex-col gap-1 flex-1">
              ...
              <ChipGroup
                items={[1,2,3,5,10]}
                selected={[1,2,3,5,10].find(n => n === editing!.loop_count) ?? 3}
                onselect={(n) => editing!.loop_count = n}
                labelFn={(n) => `${n}\u00d7`}
              />
            </div>
          </div>
        ...
        <!-- Auto-start -->
        <ToggleRow
          checked={editing.auto_start}
          label={t("settings.autoStartOnLaunch")}
          description={t("settings.autoStartDesc")}
          ontoggle={() => editing!.auto_start = !editing!.auto_start}
        ...
        <div class="flex items-center gap-2 flex-wrap px-4 py-3 bg-surface-3">
          <SectionHeader class="shrink-0">{t("settings.summary")}</SectionHeader>
          {#each editing.actions as action, i}
            {@const colors = ["bg-violet-500/10 text-violet-600 dark:text-violet-400 border-violet-500/20",
                              "bg-violet-500/10 text-violet-600 dark:text-violet-400 border-violet-500/20",
                              "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20",
                              "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20",
                              "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20"]}
            <Badge variant="outline" class="text-ui-xs py-0 px-1.5 {colors[i % colors.length]}">
              {action.label || "?"}
            </Badge>
          {/each}
          <Badge variant="outline"
          ...
          <span class="ml-auto text-ui-xs text-muted-foreground/60 shrink-0">
    ...
    <div class="flex items-center gap-2">
      <Button onclick={saveEdit} disabled={saving || !editing.name.trim() || editing.actions.length === 0}
              class="text-ui-md h-8 px-5">
        {saving ? t("common.saving") : t("common.save")}
      </Button>
      <Button variant="outline" onclick={cancelEdit} class="text-ui-md h-8 px-4">
        {t("common.cancel")}
      </Button>
    </div>
