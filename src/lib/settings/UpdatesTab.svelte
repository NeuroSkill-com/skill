...
<section class="flex flex-col gap-4">
  ...
  <div class="rounded-2xl border border-border dark:border-white/[0.06]
    ...
    {#if phase === "ready"}
      <span class="text-emerald-500 font-bold text-ui-md">
        ✅ {t("updates.readyToRestart")}
      </span>
    {:else if phase === "downloading"}
      <span class="text-blue-500 font-semibold text-ui-base tabular-nums">{progress}%</span>
        ...
        {#if phase === "ready"}
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col gap-0.5 flex-1">
              <span class="text-ui-lg font-semibold text-emerald-600 dark:text-emerald-400">
        ...
        {:else if phase === "downloading"}
          ...
          <div class="flex flex-col gap-2.5">
            <div class="flex items-center gap-2">
              ...
              <span class="ml-auto text-ui-md font-bold text-blue-500 tabular-nums">
                {progress}%
              </span>
            </div>
            ...
            {#if available?.body}
              <p class="text-ui-sm text-muted-foreground/70 line-clamp-3">{available.body}</p>
            {/if}
          </div>
        ...
        {:else}
          ...
          <div class="flex items-center gap-3">
            {#if phase === "error" && available}
              ...
              <div class="flex flex-col gap-0.5 flex-1">
                ...
                <span class="text-ui-base text-red-600 dark:text-red-400">
          ...
          {#if phase === "error" && error}
            <div class="rounded-lg border border-red-400/30 bg-red-50 dark:bg-[#1a0a0a] px-3 py-2">
              <span class="text-ui-base text-red-600 dark:text-red-400 break-all">{error}</span>
            </div>
          {/if}
    ...
    <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
      <div class="flex flex-col gap-3 px-4 py-4">
        ...
        <ChipGroup
          ...
          selected={INTERVAL_OPTIONS.find(p => p[0] === checkIntervalSecs) ?? INTERVAL_OPTIONS[0]}
          onselect={(p) => setCheckInterval(p[0])}
          labelFn={(p) => t(p[1])}
        />

        {#if checkIntervalSecs === 0}
          <p class="text-ui-sm text-amber-600 dark:text-amber-400 leading-relaxed">
    ...
    <CardContent class="py-0 px-0">
      <button
        ...
        <div class="relative shrink-0 w-8 h-4 rounded-full transition-colors
                    {autostartEnabled ? 'bg-violet-500' : 'bg-muted dark:bg-white/[0.08]'}">
          {#if autostartSaving}
