...
import { applyPreferred } from "$lib/settings/devices-logic";
...
<section class="flex flex-col gap-4">
  ...
  <div class="rounded-2xl border border-border dark:border-white/[0.06]
    ...
    <div class="flex flex-col gap-0.5">
      <span class="text-ui-lg font-bold">{t("devices.title")}</span>
  ...
  <div class="flex flex-col gap-2">
    <SectionHeader>{t("devices.pairedDevices")}</SectionHeader>
    ...
    {#if hasNewUnpaired}
      <div class="flex items-start gap-2.5 rounded-xl
        ...
        <div class="flex flex-col gap-0.5 flex-1 min-w-0">
          ...
          <span class="text-ui-base text-amber-700/70 dark:text-amber-400/70 leading-relaxed">
  ...
  <!-- ── Supported Devices ─────────────────────────────────────────────────── -->
  <div class="flex flex-col gap-2">
    <SectionHeader>{t("settings.supportedDevices.title")}</SectionHeader>
    ...
    <div class="flex flex-col gap-2">
      <input
        ...
        class="text-ui-md px-2.5 py-1.5 rounded-md border border-border bg-background text-foreground placeholder-muted-foreground focus:outline-none focus:ring-1 focus:ring-muted-foreground/50" />
    </div>
          ...
          <div class="flex flex-col">
            ...
            >
              ...
              <div class="flex flex-col items-start gap-0 flex-1 min-w-0">
                <span class="text-ui-md font-semibold text-foreground">{t(company.name_key)}</span>
            ...
            {#if supportedCompanyExpanded === company.id}
              <div class="px-3.5 pb-3 flex flex-col gap-2">
                  ...
                  {#each company.devices as item (item.name_key)}
                    <div
                      class="flex flex-col items-stretch gap-1 rounded-md border border-border/50
                             dark:border-white/[0.06] bg-background/60 px-1.5 py-1.5"
                    >
                ...
                <div class="rounded-md border border-border/50 dark:border-white/[0.06] bg-muted/30 px-2.5 py-2">
                  <p class="text-ui-sm font-medium text-foreground/80 mb-0.5">{t("settings.supportedDevices.howToConnect")}</p>
                  <div class="flex flex-col gap-0.5">
                    {#each company.instruction_keys as lineKey (lineKey)}
                      <p class="text-ui-xs text-muted-foreground leading-relaxed">• {t(lineKey)}</p>
                    {/each}
                  </div>
                </div>
              </div>
  ...
  <div class="flex flex-col gap-2">
    <div class="flex items-center gap-2 px-0.5">
      <SectionHeader>{t("settings.deviceApi.title")}</SectionHeader>
    </div>
    ...
    <SettingsCard>
      <CardContent class="flex flex-col gap-3 p-4">
        ...
        >
          <span class="text-ui-lg font-semibold text-foreground">{t("settings.deviceApi.openbciTitle")}</span>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
               stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
               class="w-3 h-3 text-muted-foreground/50 transition-transform duration-200
        ...
        {#if openbciExpanded}
          ...
          <div class="flex flex-col gap-1.5">
            <p class="text-ui-base font-medium text-foreground/80">{t("settings.openbciBoard")}</p>
            <div class="grid grid-cols-2 gap-x-4 gap-y-1">
              {#snippet boardRadio(val: OpenBciBoard, key: string)}
                <label class="flex items-center gap-1.5 cursor-pointer select-none text-ui-md leading-snug">
          ...
          {#if isBle}
            <div class="flex items-center gap-3">
              <label for="openbci-scan-timeout" class="text-ui-base font-medium text-foreground/80 shrink-0">
              ...
              <input id="openbci-scan-timeout"
                ...
                class="w-16 text-ui-md text-center px-2 py-1 rounded-md border border-border
                       bg-background text-foreground tabular-nums" />
              <span class="text-ui-base text-muted-foreground">{t("settings.openbciScanTimeoutSuffix")}</span>
            </div>
            <Separator class="bg-border dark:bg-white/[0.04]" />
          {/if}
          ...
          {#if isSerial}
            <div class="flex flex-col gap-1.5">
              <p class="text-ui-base font-medium text-foreground/80">{t("settings.openbciSerialPort")}</p>
              <div class="flex gap-2 items-center">
                {#if serialPorts.length > 0}
                  <select bind:value={openbci.serial_port} aria-label={t("settings.openbciSerialPort")} onchange={() => { openbciChanged = true; }}
                    class="flex-1 min-w-0 text-ui-md px-2 py-1 rounded-md border border-border bg-background text-foreground">
                ...
                {:else}
                  <input type="text" bind:value={openbci.serial_port} aria-label={t("settings.openbciSerialPort")} oninput={() => { openbciChanged = true; }}
                    ...
                    class="flex-1 min-w-0 text-ui-md font-mono px-2 py-1 rounded-md border border-border bg-background text-foreground" />
                {/if}
                <Button size="sm" variant="outline"
                  class="text-ui-base h-7 px-2.5 shrink-0 border-border dark:border-white/10"
                  onclick={loadSerialPorts} disabled={portsLoading}>
                  {portsLoading ? "…" : t("settings.openbciRefreshPorts")}
                </Button>
              </div>
              <span class="text-ui-sm text-muted-foreground">{t("settings.openbciSerialPortHint")}</span>
            </div>
            <Separator class="bg-border dark:bg-white/[0.04]" />
          {/if}
          ...
          {#if isWifi}
            <div class="flex flex-col gap-2">
              <div class="flex items-center gap-3">
                <img src="/devices/openbci-wifi-shield.png" alt="OpenBCI WiFi Shield"
                     class="h-16 w-16 object-cover rounded-lg shrink-0
                            bg-muted/30 dark:bg-white/[0.02]
                            border border-border dark:border-white/[0.06]" />
                <p class="text-ui-base font-medium text-foreground/80">{t("settings.openbciWifiShieldIp")}</p>
              </div>
              <input type="text" bind:value={openbci.wifi_shield_ip} aria-label={t("settings.openbciWifiShieldIp")} oninput={() => { openbciChanged = true; }}
                ...
                class="text-ui-md font-mono px-2 py-1 rounded-md border border-border bg-background text-foreground" />
              <div class="flex items-center gap-3 mt-1">
                <label for="openbci-local-port" class="text-ui-base font-medium text-foreground/80 shrink-0">
                ...
                <input id="openbci-local-port"
                  ...
                  oninput={() => { openbciChanged = true; }}
                  class="w-20 text-ui-md text-center px-2 py-1 rounded-md border border-border
                         bg-background text-foreground tabular-nums" />
              </div>
            </div>
            <Separator class="bg-border dark:bg-white/[0.04]" />
          ...
          {#if isGalea}
            <div class="flex flex-col gap-1.5">
              <p class="text-ui-base font-medium text-foreground/80">{t("settings.openbciGaleaIp")}</p>
          ...
          <!-- Channel labels -->
          <div class="flex flex-col gap-2">
            <span class="text-ui-base font-medium text-foreground/80">
            ...
            <div class="flex items-center gap-2">
              <select
                ...
                class="flex-1 min-w-0 text-ui-base h-7 px-2 rounded border border-border
              ...
              <button onclick={() => applyPreset("__none__")}
                class="shrink-0 text-ui-sm h-7 px-2.5 rounded border border-border
            ...
            <div class="grid grid-cols-4 gap-x-2 gap-y-2">
              {#each Array.from({ length: channelCount }, (_, i) => i) as i}
                <div class="flex flex-col gap-0.5 min-w-0">
                  <span class="text-ui-sm text-muted-foreground tabular-nums text-center">{i + 1}</span>
                  <input type="text"
                    ...
                    class="w-full min-w-0 text-ui-md font-mono text-center px-1 py-0.5 rounded
            ...
            <span class="text-ui-sm text-muted-foreground">{t("settings.openbciChannelLabelsHint")}</span>
          </div>
          ...
          <div class="flex items-center justify-end gap-2">
            <Button size="sm"
              ...
              class="text-ui-base h-7 px-3
                     {openbciSaved ? 'text-green-600 dark:text-green-400 border-green-500/30' :
                      openbciChanged ? 'border-violet-500/50 text-violet-600 dark:text-violet-400' :
            ...
            {#if !isBle}
              <Button size="sm" variant="default"
                class="text-ui-base h-7 px-3 bg-violet-600 hover:bg-violet-700 text-white"
          ...
          {#if openbciError}
            <p class="text-ui-sm text-destructive">{openbciError}</p>
          {/if}
        {/if}
        ...
        >
          <span class="text-ui-lg font-semibold text-foreground">{t("settings.deviceApi.emotivTitle")}</span>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
               stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
               class="w-3 h-3 text-muted-foreground/50 transition-transform duration-200
        ...
        {#if emotivApiExpanded}
          ...
          <div class="flex flex-col gap-1.5">
            <label for="emotiv-client-id" class="text-ui-base font-medium text-foreground/80">{t("settings.deviceApi.clientId")}</label>
            <input
              id="emotiv-client-id"
              type="text"
              ...
              class="text-ui-md px-2 py-1 rounded-md border border-border bg-background text-foreground" />
          </div>

          <div class="flex flex-col gap-1.5">
            <label for="emotiv-client-secret" class="text-ui-base font-medium text-foreground/80">{t("settings.deviceApi.clientSecret")}</label>
            <div class="flex items-center gap-2">
              <input
          ...
          <div class="flex justify-end">
            <Button size="sm"
              ...
              class="text-ui-base h-7 px-3
                ...
                emotivApiChanged ? 'border-violet-500/50 text-violet-600 dark:text-violet-400' :
                'border-border dark:border-white/10 text-muted-foreground'}"
          ...
          {#if emotivApiError}
            <p class="text-ui-sm text-destructive">{emotivApiError}</p>
          {/if}
        {/if}
        ...
        >
          <span class="text-ui-lg font-semibold text-foreground">{t("settings.deviceApi.idunTitle")}</span>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
               stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
               class="w-3 h-3 text-muted-foreground/50 transition-transform duration-200
        ...
        {#if idunApiExpanded}
          ...
          <a
            ...
            class="text-ui-sm text-violet-600 dark:text-violet-400 hover:underline w-fit">
            {t("settings.deviceApi.idunDashboard")}
          </a>

          <div class="flex flex-col gap-1.5">
            <label for="idun-api-token" class="text-ui-base font-medium text-foreground/80">{t("settings.deviceApi.apiToken")}</label>
            <div class="flex items-center gap-2">
              <input
                id="idun-api-token"
          ...
          <div class="flex justify-end">
            <Button size="sm"
              ...
              class="text-ui-base h-7 px-3
                ...
                idunApiChanged ? 'border-violet-500/50 text-violet-600 dark:text-violet-400' :
                'border-border dark:border-white/10 text-muted-foreground'}"
              onclick={saveIdunApi}
          ...
          {#if idunApiError}
            <p class="text-ui-sm text-destructive">{idunApiError}</p>
          {/if}
        {/if}
        ...
        >
          <span class="text-ui-lg font-semibold text-foreground">{t("settings.deviceApi.neurosityTitle")}</span>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
               stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
               class="w-3 h-3 text-muted-foreground/50 transition-transform duration-200
        ...
        {#if neurosityApiExpanded}
          ...
          <div class="flex flex-col gap-1.5">
            <label for="neurosity-email" class="text-ui-base font-medium text-foreground/80">{t("settings.deviceApi.neurosityEmail")}</label>
            <input
              id="neurosity-email"
              type="text"
          ...
          <div class="flex flex-col gap-1.5">
            <label for="neurosity-password" class="text-ui-base font-medium text-foreground/80">{t("settings.deviceApi.neurosityPassword")}</label>
            <div class="flex items-center gap-2">
              <input
          ...
          <div class="flex flex-col gap-1.5">
            <label for="neurosity-device-id" class="text-ui-base font-medium text-foreground/80">{t("settings.deviceApi.neurosityDeviceId")}</label>
            <input
          ...
          {#if neurosityApiError}
            <p class="text-ui-sm text-destructive">{neurosityApiError}</p>
          {/if}
        {/if}
        ...
        >
          <span class="text-ui-lg font-semibold text-foreground">{t("settings.deviceApi.brainmasterTitle")}</span>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
               stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
               class="w-3 h-3 text-muted-foreground/50 transition-transform duration-200
        ...
        {#if brainmasterApiExpanded}
          ...
          <div class="flex flex-col gap-1.5">
            <label for="brainmaster-model" class="text-ui-base font-medium text-foreground/80">{t("settings.deviceApi.brainmasterModel")}</label>
            <select
          ...
          {#if brainmasterApiError}
            <p class="text-ui-sm text-destructive">{brainmasterApiError}</p>
          {/if}
        {/if}
        ...
        >
          <span class="text-ui-lg font-semibold text-foreground">{t("settings.deviceApi.ouraTitle")}</span>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
               stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
               class="w-3 h-3 text-muted-foreground/50 transition-transform duration-200
        ...
        {#if ouraApiExpanded}
          ...
          <div class="flex flex-col gap-1.5">
            <label for="oura-access-token" class="text-ui-base font-medium text-foreground/80">{t("settings.deviceApi.ouraAccessToken")}</label>
            <div class="flex items-center gap-2">
              <input
          ...
          <div class="flex items-center justify-between gap-2">
            ...
            <Button size="sm"
              ...
              class="text-ui-base h-7 px-3
                ...
                ouraApiChanged ? 'border-violet-500/50 text-violet-600 dark:text-violet-400' :
                'border-border dark:border-white/10 text-muted-foreground'}"
              onclick={saveOuraApi}
          ...
          {#if ouraApiError}
            <p class="text-ui-sm text-destructive">{ouraApiError}</p>
          {/if}
          {#if ouraSyncError}
            <p class="text-ui-sm text-destructive">{ouraSyncError}</p>
          {/if}
        {/if}
      </CardContent>
    </SettingsCard>
  ...
  <!-- ── Scanner Backends ──────────────────────────────────────────────────── -->
  <div class="flex flex-col gap-2">
    <SectionHeader>{t("settings.scanner.title")}</SectionHeader>
    <p class="text-ui-sm text-muted-foreground/70 px-0.5 leading-relaxed">
      ...
      <CardContent class="flex flex-col divide-y divide-border dark:divide-white/[0.05] py-0 px-0">
        ...
        <label class="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-muted/30">
          <input type="checkbox" bind:checked={scannerConfig.ble}
            ...
            class="accent-violet-500 shrink-0" />
          <div class="flex flex-col gap-0.5 flex-1 min-w-0">
            <span class="text-ui-md font-semibold text-foreground">{t("settings.scanner.ble")}</span>
            <span class="text-ui-sm text-muted-foreground">{t("settings.scanner.bleDesc")}</span>
          </div>
          <Badge variant="outline"
            class="text-ui-2xs py-0 px-1.5 shrink-0
        ...
        <label class="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-muted/30">
          <input type="checkbox" bind:checked={scannerConfig.usb_serial}
            ...
            class="accent-violet-500 shrink-0" />
          <div class="flex flex-col gap-0.5 flex-1 min-w-0">
            <span class="text-ui-md font-semibold text-foreground">{t("settings.scanner.usbSerial")}</span>
            <span class="text-ui-sm text-muted-foreground">{t("settings.scanner.usbSerialDesc")}</span>
          </div>
          <Badge variant="outline"
            class="text-ui-2xs py-0 px-1.5 shrink-0
        ...
        <label class="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-muted/30">
          <input type="checkbox" bind:checked={scannerConfig.cortex}
            ...
            class="accent-violet-500 shrink-0" />
          <div class="flex flex-col gap-0.5 flex-1 min-w-0">
            <span class="text-ui-md font-semibold text-foreground">{t("settings.scanner.cortex")}</span>
            <span class="text-ui-sm text-muted-foreground">{t("settings.scanner.cortexDesc")}</span>
          </div>
          {#if scannerConfig.cortex && deviceApi.emotiv_client_id && deviceApi.emotiv_client_secret}
            {#if cortexWsState === "connected"}
              <Badge variant="outline"
                class="text-ui-2xs py-0 px-1.5 shrink-0 flex items-center gap-1
      ...
      <Button size="sm"
        ...
        class="text-ui-base h-7 px-3
               {scannerSaved ? 'text-green-600 dark:text-green-400 border-green-500/30' :
                scannerChanged ? 'border-violet-500/50 text-violet-600 dark:text-violet-400' :
                'border-border dark:border-white/10 text-muted-foreground'}"
  ...
  <div class="flex flex-col gap-2">
    ...
    >
      <SectionHeader>{t("settings.deviceLog.title")}</SectionHeader>
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
           stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
           class="w-3 h-3 text-muted-foreground/50 transition-transform duration-200
      ...
      <SettingsCard>
        <CardContent class="p-0">
          {#if deviceLog.length === 0}
            <p class="text-ui-base text-muted-foreground/50 text-center py-6">
              {t("settings.deviceLog.empty")}
            </p>
          {:else}
            <div class="max-h-[260px] overflow-y-auto font-mono text-ui-sm leading-relaxed">
              {#each deviceLog.toReversed() as entry (entry.ts + entry.tag + entry.msg)}
                <div class="flex gap-2 px-3 py-1 border-b border-border/30 dark:border-white/[0.04]
                  ...
                  <span class="shrink-0 w-[52px] font-semibold
                    {entry.tag === 'ble'     ? 'text-blue-500' :
                     ...
                     entry.tag === 'cortex'  ? 'text-violet-600 dark:text-violet-400' :
...
{#snippet deviceRow(dev: DiscoveredDevice)}
  ...
  <div class="flex items-center gap-3 px-4 py-3
              transition-colors hover:bg-accent/50 dark:hover:bg-white/[0.02]
    ...
    <div class="flex flex-col gap-0.5 min-w-0 flex-1">
      <div class="flex items-center gap-1.5 min-w-0">
        ...
        {#if dev.is_paired}
          <Badge variant="outline"
            class="text-ui-xs tracking-wide uppercase py-0 px-1 shrink-0
                   bg-green-500/10 text-green-600 dark:text-green-400 border-green-500/20">
            {t("settings.paired")}
          </Badge>
      ...
      <div class="flex items-center gap-2">
        <span class="font-mono text-ui-sm text-muted-foreground truncate">{dev.id}</span>
        {#if dev.last_rssi !== 0}
          <span class="text-ui-sm shrink-0" style="color:{colorForRssi(dev.last_rssi)}">
            {fmtRssi(dev.last_rssi)}
          </span>
        {/if}
        <span class="text-ui-sm text-muted-foreground shrink-0">
          {fmtLastSeen(dev.last_seen)}
        </span>
      </div>

      {#if !dev.is_paired}
        <span class="text-ui-sm text-amber-600/80 dark:text-amber-400/70 leading-tight mt-0.5">
          {t("settings.pairToConnect")}
        </span>
      {/if}

      {#if dev.transport === "cortex"}
        <span class="text-ui-sm text-violet-600 dark:text-violet-400/80 dark:text-violet-500/70 leading-tight mt-0.5">
        ...
        <div class="flex items-center gap-3 flex-wrap">
          {#if connected.serial_number}
            <button
              ...
              class="font-mono text-ui-sm text-muted-foreground/80 hover:text-muted-foreground
      ...
      {#if dev.is_paired}
        <Button
          ...
          class={dev.is_preferred
            ? "text-ui-base h-7 px-2.5 bg-yellow-500/15 text-yellow-600 dark:text-yellow-400 border-yellow-500/30 hover:bg-yellow-500/25"
            : "text-ui-base h-7 px-2.5 border-border dark:border-white/10 text-muted-foreground hover:text-foreground"}
          onclick={() => setPreferred(dev.id)}>
          {dev.is_preferred ? t("settings.defaultDevice") : t("settings.setDefault")}
        </Button>
        <Button size="sm" variant="ghost"
          class="text-ui-base h-7 px-2 text-muted-foreground hover:text-red-500"
