...
<script lang="ts">
  ...
  import {
    ...
    Points, PerspectiveCamera, Raycaster, Vector2, Vector3, Color, Group, Line,
    LineBasicMaterial, LineDashedMaterial, SphereGeometry,
    MeshBasicMaterial, Mesh, GridHelper, AmbientLight,
  } from "three";
  ...
  // ── Constants ────────────────────────────────────────────────────────────
  const COLOR_A = 0x3b82f6, COLOR_B = 0xf59e0b, LINK_COLOR = 0xffffff;
  const SCALE   = 15;
  const ANIM_MS = 1800;
  ...
  function clearLinks() {
    if (!linkGroup) return;
    linkGroup.traverse((o: any) => { o.geometry?.dispose(); o.material?.dispose(); });
  ...
  function showLinks(label: string) {
    ...
    const dm = new LineDashedMaterial({ color: LINK_COLOR, transparent: true, opacity: 0.2, dashSize: 0.3, gapSize: 0.2 });
    for (const idx of matching) {
  ...
  function buildCloud(pts: UmapPoint[], positions: Float32Array) {
    const n = pts.length;
    const colA = new Color(COLOR_A), colB = new Color(COLOR_B);
    ...
    if (mainCloud)  { scene.remove(mainCloud);  mainCloud.geometry.dispose();  (mainCloud.material as PointsMaterial).dispose(); }
    if (labelCloud) { scene.remove(labelCloud); labelCloud.geometry.dispose(); (labelCloud.material as PointsMaterial).dispose(); labelCloud = null; }
    clearLinks();
    ...
    mainCloud = new Points(g, new PointsMaterial({
      size: ps, vertexColors: true, transparent: true, opacity: 0.65,
    ...
    if (lPos.length) {
      ...
      labelCloud = new Points(lg2, new PointsMaterial({
        size: ps * 2.5, vertexColors: true, transparent: true, opacity: 0.95, sizeAttenuation: true,
      }));
  ...
  function applyPositions(pos: Float32Array) {
    ...
    if (labelCloud && labeledIdx.length) {
      ...
      for (let li = 0; li < labeledIdx.length; li++) {
        ...
        la.array[li*3] = pos[i*3]; la.array[li*3+1] = pos[i*3+1]; la.array[li*3+2] = pos[i*3+2];
      }
      la.needsUpdate = true;
    }
    curPositions = pos;
  ...
  useTask(() => {
    ...
    if (fromPos && toPos && mainCloud) {
      ...
      applyPositions(buf);
      if (t >= 1) { fromPos = null; toPos = null; }
    }
  ...
  function onPointerUp(e: PointerEvent) {
    ...
    if (label) { activeLabel === label ? clearLinks() : showLinks(label); }
    else { clearLinks(); }
  }

  function onPointerLeave() { tooltip = null; }
  ...
  onDestroy(() => {
    ...
    controls?.dispose();
    if (mainCloud)  { scene.remove(mainCloud);  mainCloud.geometry.dispose();  (mainCloud.material as PointsMaterial).dispose(); }
    if (labelCloud) { scene.remove(labelCloud); labelCloud.geometry.dispose(); (labelCloud.material as PointsMaterial).dispose(); }
