...
<script lang="ts">
  ...
  let streamStartMs  = $state(0);       // performance.now() when first token arrived
  let streamTokens   = $state(0);       // approximate token count during streaming
  ...
  function editAndResend(msg: Message) {
    ...
    tick().then(() => { autoResizeInput(); inputEl?.focus(); });
  }
...
<div class="flex h-full min-h-0 bg-background text-foreground overflow-hidden rounded-b-[10px]">
  ...
  <!-- ── Parameters panel (compact inline) ──────────────────────────────── -->
  {#if showSettings}
            ...
            <div class="flex flex-col gap-1 max-w-[82%]">
              ...
              {#if msg.content.trim()}
                <div class="group/bubble flex flex-col gap-0.5">
                  ...
                  <!-- Actions: copy + regenerate — visible on hover once generation is done -->
  ...
  <footer class="relative shrink-0 border-t border-border dark:border-white/[0.06]
    ...
    <div class="flex items-end gap-2 rounded-xl border border-border dark:border-white/[0.08]
      ...
      {#if generating}
        <!-- Abort button with "Aborting…" feedback -->
        <button
          onclick={abort}
          disabled={aborting}
