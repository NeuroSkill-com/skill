...
<script lang="ts">
  ...
  interface QueryEntry   { timestamp: number; timestamp_unix: number; neighbors: NeighborEntry[]; }
  ...
  function simPct(d: number, max: number) {
    if (max <= 0) return "100%";
    return ((1 - d / max) * 100).toFixed(1) + "%";
  }
  function simWidth(d: number, max: number) {
    if (max <= 0) return 1;
    return Math.max(0, 1 - d / max);
  }

  // EEG metric chips
  function metricChips(met: NeighborMetrics) {
    return [
  ...
  const searchAnalysis = $derived.by((): SearchAnalysis | null => {
  ...
  const temporalHeatmap = $derived.by((): number[][] | null => {
    if (!result || result.results.length === 0) return null;
  ...
  function heatColor(count: number, max: number) {
    ...
    const t2 = count / max;
    return `rgba(${Math.round(59+(1-t2)*180)},${Math.round(130+(1-t2)*100)},246,${0.15+t2*0.75})`;
  }
  ...
  function genPlaceholder(nA: number, nB: number): UmapResult {
    const pts: UmapPoint[] = [];
    for (let i = 0; i < nA; i++) pts.push({ x: -3+gaussRand()*3, y: gaussRand()*3, z: gaussRand()*3, session: 0, utc: 0 });
    for (let i = 0; i < nB; i++) pts.push({ x:  3+gaussRand()*3, y: gaussRand()*3, z: gaussRand()*3, session: 1, utc: 0 });
    return { points: pts, n_a: nA, n_b: nB, dim: 0 };
  }
  function fireUmap() {
    ...
    umapPlaceholder = genPlaceholder(Math.min(result.query_count, 200), Math.min(allNb.length, 200));
  ...
  const ixDisplayGraph = $derived.by(() => {
    ...
    // Build text → canonical node map for found_labels
    const canonMap = new Map<string, string>(); // text → canonical node id
    const keepNodes: GraphNode[] = [];

    for (const n of ixNodes) {
      if (n.kind !== "found_label") { keepNodes.push(n); continue; }
      const key = (n.text ?? "").trim().toLowerCase();
      if (!canonMap.has(key)) {
        canonMap.set(key, n.id);  // first occurrence becomes canonical
        keepNodes.push(n);
      }
      // subsequent duplicates are dropped — edges will be remapped below
    }

    // Remap edges: any to_id pointing to a dropped node → redirect to canonical
    // Also build a deduplicated edge set (no duplicate from+to pairs)
    const idToText = new Map<string, string>(); // node_id → text key
    for (const n of ixNodes) {
  ...
  const ixTimeHeatmap = $derived.by(() => {
    const eeg = ixNodes.filter(n => n.kind === "eeg_point" && n.timestamp_unix != null);
    ...
    // Absolute time range for jet coloring of cells
  ...
  // Turbo/jet colormap in JS (mirrors InteractiveGraph3D)
  function ixTurbo(t: number): string {
  ...
  function ixHeatColor(dayIdx: number, hour: number, count: number, heatmap: NonNullable<typeof ixTimeHeatmap>): string {
    if (count === 0) return "transparent";
    // Time-position t for this cell's midpoint
    ...
    // Extract RGB from turbo CSS hex
    const hex = ixTurbo(t);
   ...
   */
  function buildTextKnnGraph(results: LabelNeighbor[], query: string): UmapResult {
    const GRAPH_SCALE = 12;
    const pts: UmapPoint[] = [];

    // Query anchor — blue, center, labeled so it shows prominently
    ...
    // Golden angle ≈ 137.5° ensures even angular coverage on a sphere
    const golden = Math.PI * (3 - Math.sqrt(5));
    const n = sorted.length;

    for (let i = 0; i < n; i++) {
      const nb = sorted[i];
      // Normalise distance to [0,1] → shell radius [0.18, 0.88] of GRAPH_SCALE
      const t      = n === 1 ? 0.5 : (nb.distance - minD) / range;
      const radius = (0.18 + t * 0.70) * GRAPH_SCALE;
      // Fibonacci sphere parametrisation: y spread uniformly, angle by golden ratio
            ...
            <div class="flex items-center gap-2 px-3.5 py-2 border-b border-border dark:border-white/[0.06]">
              ...
              <div class="ml-auto flex items-center gap-1.5">
                ...
                <div class="w-14 h-1.5 rounded-full"
                     style="background:linear-gradient(to right,{ixTurbo(0)},{ixTurbo(0.25)},{ixTurbo(0.5)},{ixTurbo(0.75)},{ixTurbo(1)})">
                </div>
